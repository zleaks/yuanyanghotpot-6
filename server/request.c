...
static const char * const server_socket_name = "/tmp/socket";   /* name of the socket file */
...
char *server_socket_name_final = NULL;
...
static void socket_cleanup(void)
{
    ...
    if (!do_it_once++) unlink( server_socket_name_final );
}
...
static char *create_server_dir( int force )
{
    ...
    size_t len_socket = sizeof(server_socket_name) + 2 * sizeof(st.st_dev) + 2 * sizeof(st.st_ino) + 16;
    server_socket_name_final = malloc(len_socket);
...
#else
    len += sizeof("/opt/winux/server/.wine-") + 12;
    if (!(server_dir = malloc( len ))) fatal_error( "out of memory\n" );
    sprintf( server_dir, "/opt/winux/server/.wine-%u", getuid() );
    sprintf( server_socket_name_final, "/tmp/socket-%u-", getuid() );
#endif
    ...
    q = server_socket_name_final + strlen( server_socket_name_final );

    if (st.st_dev != (unsigned long)st.st_dev) {
        p += sprintf( p, "%lx%08lx-", (unsigned long)((unsigned long long)st.st_dev >> 32),
                      (unsigned long)st.st_dev );
        q += sprintf( q, "%lx%08lx-", (unsigned long)((unsigned long long)st.st_dev >> 32),
                      (unsigned long)st.st_dev );
    }
    else {
        p += sprintf( p, "%lx-", (unsigned long)st.st_dev );
        q += sprintf( q, "%lx-", (unsigned long)st.st_dev );
    }

    if (st.st_ino != (unsigned long)st.st_ino) {
        sprintf( p, "%lx%08lx", (unsigned long)((unsigned long long)st.st_ino >> 32),
                 (unsigned long)st.st_ino );
        sprintf( q, "%lx%08lx", (unsigned long)((unsigned long long)st.st_ino >> 32),
                 (unsigned long)st.st_ino );
    }
    else {
        sprintf( p, "%lx", (unsigned long)st.st_ino );
        sprintf( q, "%lx", (unsigned long)st.st_ino );
    }
...
static void acquire_lock(void)
{
    ...
    if (fcntl( fd, F_SETLK, &fl ) != -1)
    {
        ...
        if (stat( server_socket_name_final, &st ) != -1 &&   /* there is a leftover socket */
        ...
        unlink( server_socket_name_final ); /* we got the lock, we can safely remove the socket */
        got_lock = 1;
    ...
    strcpy( addr.sun_path, server_socket_name_final );
    ...
    //chmod( server_socket_name_final, 0600 );  /* make sure no other user can connect */
    chmod( server_socket_name_final, 0666 );  /* make sure no other user can connect */
