...
static void on_filemanagestyle_change(HWND dialog)
{
	...
	{
	    case 1:
	    	set_reg_key_dword(HKEY_CURRENT_USER,szKeyStyle, "File Manage Style", file_manage_style[selection].dwStyleID);
		break;
...
update_comboboxes(HWND dialog)
{
	...
	if (( res = RegOpenKeyW(HKEY_CURRENT_USER, szKeyStyle, &style_key))) 
	{
		style_key = 0;
		styleID = 0;
	}
...
DriveDlgProc (HWND dialog, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        case WM_INITDIALOG:
            ...
            set_advanced(dialog);
	//    init_comboboxes(dialog); //add by xiaoya
	...
	/*	case IDC_NENO:
		{
		    ...
		    on_filemanagestyle_change(dialog);
		    break;
		}*/
            }
            break;
        ...
        case WM_NOTIFY:
            switch (((LPNMHDR)lParam)->code)
            {
                ...
                case LVN_ITEMCHANGED:
                {
                    ...
                    update_controls(dialog);
	//   update_comboboxes(dialog); //add by xiaoya
                    break;
                }
            }
            break;
