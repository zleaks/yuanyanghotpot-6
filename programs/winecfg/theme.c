...
/* 20210428 wanghg add for 配置默认应用 --> */
static const WCHAR szKeyStyle[] = {'S','o','f','t','w','a','r','e','\\','W','i','n','e','\\','F','i','l','e','O','p','e','n','A','s','s','o','c','i','a','t','i','o','n','s',0};
static const WCHAR szKeyFileOpenAssociations[] = {'S','o','f','t','w','a','r','e','\\','W','i','n','e','\\','F','i','l','e','O','p','e','n','A','s','s','o','c','i','a','t','i','o','n','s','\\','%','s',0};

/*
static void init_comboboxes(HWND dialog)
{
    WCHAR strtemp[256]={0};
    WCHAR *buff;
    WCHAR string1W[]={'a','a','a','\0',0};
    WCHAR string2W[]={'b','b','b','%','s',0};
    WCHAR s[]={',',0};
    WCHAR WhiteList[]={'W','h','i','t','e','L','i','s','t',0};
    WCHAR keyValue[256] = { 0 };
    const WCHAR stateN[] = {'N',0};
    const WCHAR stateY[] = {'Y',0};
    WCHAR Enabled[] = {'E','n','a','b','l','e','d',0};
    DWORD length=0,index=0;
    DWORD len;
    DWORD type;
    //int index=0;
    WCHAR lpValues[256]={0};
    WCHAR lpValue[256]={0};
    WCHAR List[25][25]={0};
    DWORD ListIndex1=0;
    DWORD ListIndex2=0;

	TRACE("init comboboxes\n");
	int res;
    DWORD dwIndexs,charLength; //需要返回子项的索引
    HKEY FileType_key;
    WCHAR keyName[25] = { 0 }; //保存子键的名称
    dwIndexs = 0;
    charLength = 256;  //想要读取多少字节并返回实际读取到的字符长度

    if (( res = RegOpenKeyW(HKEY_CURRENT_USER, szKeyStyle, &FileType_key)))
	{
		FileType_key = 0;
    }
    disable(IDC_MIME_ADDDOC);
    disable(IDC_MIME_REMOVEDOC);
     while (RegEnumKeyExW(FileType_key, dwIndexs, keyName, &charLength, NULL, NULL, NULL, NULL) == ERROR_SUCCESS)
	{
        sprintfW(strtemp,szKeyFileOpenAssociations,keyName);
        if(get_reg_keyW(HKEY_CURRENT_USER,strtemp, Enabled, NULL)==NULL)
        {
            set_reg_keyW(HKEY_CURRENT_USER,strtemp, Enabled, stateN);
        }
        buff=get_reg_keyW(HKEY_CURRENT_USER,strtemp, Enabled, stateN);
        if(strcmpW(buff,'N')==0)
        {
            SendDlgItemMessageW(dialog, IDC_MIME_ASSCOMBO, CB_ADDSTRING, 0, keyName);
        }
		else
        {
           SendDlgItemMessageW(dialog, IDC_MIME_LIST, LB_ADDSTRING, 0, keyName);
        }
        ++dwIndexs;
		charLength = 256;
	}
    if (FileType_key != NULL)
	{
		RegCloseKey(FileType_key);
	}
        /*
    if(FileType_key)
	{
        if(get_reg_keyW(HKEY_CURRENT_USER,szKeyStyle, WhiteList, NULL)!=NULL)
        {
           res = RegQueryValueExW(FileType_key,WhiteList,NULL,&type,NULL,&len);
           if (type == REG_MULTI_SZ) { printf(("\n::::::::SUCCESS 0x%x,cbValues=%u\n"), res,len);}
           printf("\n>>>>>>>>>>>>>>>test1\n");
           res = RegQueryValueExW(FileType_key,WhiteList,0,0,(BYTE *)lpValues,&len);
           printf(("KKKKKKKKlpValue=%s\n"),debugstr_w(lpValues));
           if (ERROR_SUCCESS != res) { printf(("ERROR 0x%x\n"), res);}
           else printf(("SUCCESS\n,hhhhhhhhhhhhhlength=%d"),strlenW(lpValues));
           strcpyW(lpValue,lpValues);

            while(!('\0' == lpValues[index]&&'\0' == lpValues[index+1]))
          {
               if('\0' == lpValues[index])
               {
               //SendDlgItemMessageW(dialog, IDC_MIME_LIST, LB_ADDSTRING, 0, List[ListIndex1]);
                sprintfW(strtemp,szKeyFileOpenAssociations,List[ListIndex1]);
                set_reg_keyW(HKEY_CURRENT_USER,strtemp, Enabled, stateY);
                printf("\n<<<<<<index=%d,ListIndex1=%d,ListIndex2=%d,temp=%s\n",index,ListIndex1,ListIndex2,debugstr_w(List[ListIndex1]));
                index++;
                ListIndex1++;
                ListIndex2=0;
                }
               printf(("KKKKKKKKlpValue=%c\n"), lpValues[index]);
               List[ListIndex1][ListIndex2]=lpValues[index];
               index++;
               ListIndex2++;
           }
           SendDlgItemMessageW(dialog, IDC_MIME_LIST, LB_ADDSTRING, 0, List[ListIndex1]);
...
static void init_comboboxes(HWND dialog)
{
	...
	{
        if(get_reg_keyW(HKEY_CURRENT_USER,szKeyStyle, WhiteList, NULL)==NULL)
            RegSetValueExW(FileType_key,WhiteList,0,REG_MULTI_SZ,NULL,NULL);
        disable(IDC_MIME_ADDDOC);
        disable(IDC_MIME_REMOVEDOC);
        while (RegEnumKeyExW(FileType_key, dwIndexs, keyName, &charLength, NULL, NULL, NULL, NULL) == ERROR_SUCCESS)
        {
            ...
            if(get_reg_keyW(HKEY_CURRENT_USER,strtemp, Enabled, NULL)==NULL)
            {
                set_reg_keyW(HKEY_CURRENT_USER,strtemp, Enabled, state);
            }
            buff=get_reg_keyW(HKEY_CURRENT_USER,strtemp, Enabled, state);
            if(strcmpW(buff,L"Y"))
            {
                SendDlgItemMessageW(dialog, IDC_MIME_ASSCOMBO, CB_ADDSTRING, 0, keyName);
            }
...
static void enable_ADD_button(HWND dialog)
{
        enable(IDC_MIME_ADDDOC);
}
static void enable_Remove_button(HWND dialog)
{
        enable(IDC_MIME_REMOVEDOC);
}
/*
static void add_file(HWND dialog)
{
    WCHAR strtemp[256];
    WCHAR buff[25]={0};
    const WCHAR state[] = {'Y',0};
    WCHAR Enabled[] = {'E','n','a','b','l','e','d',0};
    int lbItem;
    int res;
    WCHAR WhiteList[]={'W','h','i','t','e','L','i','s','t',0};
    ...
    lbItem=(int)SendDlgItemMessageW(dialog, IDC_MIME_ASSCOMBO,CB_GETCURSEL, 0, 0);
    SendDlgItemMessageW(dialog, IDC_MIME_ASSCOMBO,CB_GETLBTEXT, lbItem, (LPARAM)buff);
    res = RegQueryValueExW(FileType_key,WhiteList,NULL,&type,NULL,&len);
           if (type == REG_MULTI_SZ) { printf(("\n::::::::SUCCESS 0x%x,cbValues=%u\n"), res,len);}
           printf("\n>>>>>>>>>>>>>>>test1\n");
           if (!(res = HeapAlloc(GetProcessHeap(), 0, len)))  printf(("ffffffERROR 0x%x\n"), res);
           res = RegQueryValueExW(FileType_key,WhiteList,0,0,(BYTE *)lpValues,&len);
           printf(("KKKKKKKKlpValue=%s\n"),debugstr_w(lpValues));
           if (ERROR_SUCCESS != res) { printf(("ERROR 0x%x\n"), res);}
           else printf(("SUCCESS\n,hhhhhhhhhhhhhlength=%d"),strlenW(lpValues));
    ...
    sprintfW(strtemp,szKeyFileOpenAssociations,buff);
    printf(">>>>>>>>>>>buff1=%s",debugstr_w(buff));
    set_reg_keyW(HKEY_CURRENT_USER,strtemp, Enabled, state);
    SendDlgItemMessageW(dialog, IDC_MIME_LIST,LB_ADDSTRING, 0, (LPARAM)buff);
     printf(">>>>>>>>>>>buff2=%s",debugstr_w(buff));
    SendDlgItemMessageW(dialog, IDC_MIME_ASSCOMBO,CB_DELETESTRING, lbItem,(LPARAM)buff);
     printf(">>>>>>>>>>>buff3=%s",debugstr_w(buff));
    disable(IDC_MIME_ADDDOC);
}
...
static void add_file(HWND dialog)
{
    ...
    if(FileType_key)
	{
        if(get_reg_keyW(HKEY_CURRENT_USER,szKeyStyle, WhiteList, NULL)!=NULL)
        {
            ...
            res = RegQueryValueExW(FileType_key,WhiteList,0,0,(BYTE *)lpValues,&len);
            if (ERROR_SUCCESS != res) { printf(("ERROR 0x%x\n"), res);}
            else printf(("SUCCESS\n,hhhhhhhhhhhhhstrlenW(lpValues)=%d\n"),strlenW(lpValues));
            printf(">>>>>>>>>>>>>>strlenW(buff)=%d\n",strlenW(buff));
            index=0;
            //if WhiteList is NUll
            if(strlenW(lpValues)==0)
            {
                strcpyW(lpValues,buff);
                lpValues[strlenW(lpValues)] ='\0';
                lpValues[strlenW(lpValues)+1] ='\0';
                RegSetValueExW(FileType_key,WhiteList,0,REG_MULTI_SZ,(const BYTE *)buff,(2+strlenW(buff))*2);
            }
            else  //if WhiteList is Not NUll
            {
                //get old string
                while(!('\0' == lpValues[index]&&'\0' == lpValues[index+1]))
                {
                ...
                //  printf(("\nindex=%d,KKKKKKKKlpValue[]=%c\n"), index,lpValues[index]);
                    List[ListIndex1][ListIndex2]=lpValues[index];
                    index++;
                    ListIndex2++;
                }
                ...
                index=0;
                //add new string to the old string
                while(index<strlenW(buff))
                {
                    //printf(("\nKKKKKKKK111lpValue=%c%c%c%c%c\n"), lpValues[length-6],lpValues[length-5],lpValues[length-4],lpValues[length-2],lpValues[length-1]);
                    lpValues[length]=buff[index];
                    //   printf(("KKKKKKKKlpValue=%c\n"), lpValues[length]);
                    //    printf(("KKKKKKKKlpValue=%c\n"), buff[index]);
                    index++;
                    length++;
                ...
                lpValues[length]='\0';
                lpValues[length++]='\0';
                //add new string to the register
                RegSetValueExW(FileType_key,WhiteList,0,REG_MULTI_SZ,(const BYTE *)lpValues,(length+1+strlenW(buff))*2);
                printf("\n<<<<<<index=%d,ListIndex1=%d,ListIndex2=%d,temp=%s\n",index,ListIndex1,ListIndex2,debugstr_w(List[ListIndex1]));
            }
        }
    }
...
/*
static void remove_file(HWND dialog)
{
    WCHAR strtemp[256];
    WCHAR buff[25]={0};
    HKEY hSubKey;
    ...
    WCHAR Enabled[] = {'E','n','a','b','l','e','d',0};
    int lbItem;
    lbItem = (int)SendDlgItemMessageW(dialog, IDC_MIME_LIST, LB_GETANCHORINDEX, 0,0);
    SendDlgItemMessageW(dialog, IDC_MIME_LIST,LB_GETTEXT, lbItem, (LPARAM)buff);
    sprintfW(strtemp,szKeyFileOpenAssociations,buff);
    set_reg_keyW(HKEY_CURRENT_USER,strtemp, Enabled, state);
    SendDlgItemMessageW(dialog, IDC_MIME_LIST,LB_DELETESTRING, lbItem, (LPARAM)buff);
    SendDlgItemMessageW(dialog, IDC_MIME_ASSCOMBO,CB_ADDSTRING, 0, (LPARAM)buff);
    disable(IDC_MIME_REMOVEDOC);
}
*/
static void remove_file(HWND dialog)
    ...
    if(FileType_key)
	{
        if(get_reg_keyW(HKEY_CURRENT_USER,szKeyStyle, WhiteList, NULL)!=NULL)
        {
            ...
            while(!('\0' == lpValues[index]&&'\0' == lpValues[index+1]))
            {
                ...
                {
                    printf("\n<<<<<<index=%d,ListIndex1=%d,ListIndex2=%d,temp=%s\n",index,ListIndex1,ListIndex2,debugstr_w(List[ListIndex1]));
                    if(strcmpW(List[ListIndex1],buff)==0)
                    {
                        printf("\nffffffffffff---index=%d,List[%d]=%s\n",index,ListIndex1,debugstr_w(List[ListIndex1]));
                        for(i=index+1;!('\0' == lpValues[i]&&'\0' == lpValues[i+1]);i++)
                        {
                            lpValues[i-strlenW(buff)-1]=lpValues[i];
                            printf("---------lpValues[%d]=%c\n",i-strlenW(buff)-1,lpValues[i-strlenW(buff)-1]);
                        }
                        // lpValues[]='\0';
                        index=i;
                        length=i-strlenW(buff)-1;
                        i=i-strlenW(buff)-1;
                        printf("\nssssssssss---i-strlenW(buff)-1=%d,index=%d\n",i,index);
                        for(;i<=(index+1);i++)
                            lpValues[i]='\0';
                        index=length;
                    }
        ...
        printf("*****************List[ListIndex1]=%s\n",debugstr_w(List[ListIndex1]));
        length=index-strlenW(buff);
        isMatch=1;
        for(i=0;i<strlenW(buff);i++)
        {
            if(lpValues[length]!=buff[i])
              isMatch=0;
            length++;
        }
        if(isMatch==1)
        ...
        RegSetValueExW(FileType_key,WhiteList,0,REG_MULTI_SZ,(const BYTE *)lpValues,index*2);
        printf(">>>>>>>>>>>>>>index=%d",index);
        }
    }
}
            ...
            switch(HIWORD(wParam)) {
                case CBN_SELCHANGE: {
                    ...
                    switch (LOWORD(wParam))
                    {
                        ...
                        /* 20210428 wanghg add for 配置默认应用 --> */
                        case IDC_MIME_ASSCOMBO: enable_ADD_button(hDlg);break;
                        case IDC_MIME_LIST: enable_Remove_button(hDlg);break;
                        /* 20210428 wanghg add for 配置默认应用 <-- */
                ...
                case BN_CLICKED:
                    switch (LOWORD(wParam))
                    {
                        ...
                        case IDC_ENABLE_FILE_ASSOCIATIONS:
                            ...
                            break;
                        /* 20210428 wanghg add for 配置默认应用 --> */
                        case IDC_MIME_ADDDOC: add_file(hDlg);break;
                        case IDC_MIME_REMOVEDOC:remove_file(hDlg);break;
                        /* 20210428 wanghg add for 配置默认应用 <-- */
                    }
                    break;
            }
