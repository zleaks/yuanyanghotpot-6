...
//<yangxinruo><202010407><for file assocation individual configuration in winecfg> add
WINE_DECLARE_DEBUG_CHANNEL(yyyxxxrrr);
...
//<yangxinruo><202010407><for file assocation individual configuration in winecfg> add
static char *in_container_xdg_data_dir;
...
// wanghg 20210906 add for hotpot building start
static void getAppNameAndVersion(const char *path, char* appName, char* appVersion) {
    if (path && strstr(path, "/opt/winux/hotpot/appstore/")) {
        // init prefix buff
        char *prefix = (char*) malloc(strlen(path) + 1);
        memset(prefix, 0, sizeof(prefix));
        strcpy(prefix, path);

        // get app version from prefix
        char *version = strrchr(prefix, '-');
        if (version && strlen(version) > 1) {
            version++;
            strcpy(appVersion, version);
            // printf("222 appVersion : %s \n", appVersion);
        }

        // get app name from prefix
        char * name = strrchr(prefix, '/');
        if (name && strlen(name) > 1) {
            name++;
            char *pathend = strrchr(prefix, '-');
            if (pathend) *pathend = '\0';
            if (name && strlen(name) > 0) strcpy(appName, name);
            // printf("222 appName : %s \n", appName);
        }

        free(prefix);
    }
}
// wanghg 20210906 add for hotpot building end
                                ...
                                const char *workdir, const char *icon, const char *wmclass)
{
    ...
    /*notepad 创建桌面快捷方式 出现desktop文件*/
    if(prefix)
    {
        if(strstr(prefix,"/opt/winux/hotpot/appstore/notepad-win-u"))
        {
            static char *check_path;
            check_path = &args[strlen(args) - 6];
            if( strcmp(check_path , "exe\\\\\"") && strcmp(check_path , "lnk\\\\\""))
            {
                return TRUE;
            }
        }
	//begin 内网通首次启动不在桌面创建.lnk文件
        if(strstr(prefix,"/opt/winux/hotpot/appstore/shiyeline-win-u"))
	{
            char check_path[128];
	    strcpy(check_path, args + (strlen(args) - 4));
            WINE_TRACE("check_path:%s\n", check_path);
	    if(strcmp(check_path, ".lnk") == 0)
	    {
	        return TRUE;
	    }
	}
    ...
    fprintf(file, "Name=%s\n", linkname);
    if (prefix) {
        // char* appName = (char*) malloc(50);
        // char* appVersion = (char*) malloc(50);
        // memset(appName, 0, sizeof(appName));
        // memset(appVersion, 0, sizeof(appVersion));
        char appName[256], appVersion[256];
        getAppNameAndVersion(prefix, appName, appVersion);
        if (strlen(appName) > 0 && strlen(appVersion) > 0) {
            fprintf(file, "Exec=/opt/winux/tools/appmanager/wine-appmanager %s %s start %s %s\n", appName, appVersion, path, args);
        } else {
...
//<yangxinruo><202010407><for file assocation individual configuration in winecfg> add begin
static WCHAR* reg_get_valsW(HKEY key, LPCWSTR subkey, LPCWSTR name)
{
    DWORD size;
    if (RegGetValueW(key, subkey, name, RRF_RT_REG_MULTI_SZ, NULL, NULL, &size) == ERROR_SUCCESS)
    {
        WCHAR *ret = HeapAlloc(GetProcessHeap(), 0, size);
        if (ret)
        {
            if (RegGetValueW(key, subkey, name, RRF_RT_REG_MULTI_SZ, NULL, ret, &size) == ERROR_SUCCESS)
                return ret;
        }
        HeapFree(GetProcessHeap(), 0, ret);
    }
...
//<yangxinruo><202010407><for file assocation individual configuration in winecfg> add end
...
//<renzhengyi><202010607><for merge multiple desktop file into one> add begin
static BOOL isAddWhitelist(LPCWSTR extensionW,LPCWSTR whitelistW,LPCWSTR whitelistLastW)
{
    WCHAR *p;
    char *whitelist = NULL;
    int isInWhitelist = 0, isInwhitelast = 0;

    /* if the whitelist is empty, it will not be added */
    if( !strncmp(debugstr_w(whitelistW), "L\"\"", strlen("L\"\"")) || !whitelistW){
        return FALSE;
    }

    for(p = whitelistW; *p; p += lstrlenW(p) + 1){
        //TRACE("[whitelistW][%d]===>p:%s\n", __LINE__, wchars_to_utf8_chars(p));
        if(strcmpW(extensionW,p)==0){
            isInWhitelist = TRUE;
            break;
        }
    }
    ...
    if(whitelistLastW){
        for (p = whitelistLastW; *p; p += lstrlenW(p) + 1)
        {
            //TRACE("[whitewhitelistLastWlistW][%d]===>p:%s\n", __LINE__, wchars_to_utf8_chars(p));
            if(strcmpW(extensionW,p)==0){
                isInwhitelast = TRUE;
                break;
            }
    ...
    if(!isInwhitelast && isInWhitelist){//Indecates add the whitelist
        return TRUE;
    }
...
static BOOL isDelWhitelist(LPCWSTR extensionW,LPCWSTR whitelistW,LPCWSTR whitelistLastW)
{
    WCHAR *p;
    char *whitelist = NULL;
    int isInWhitelist = 0, isInwhitelast = 0;

    /* if the whitelist is empty, delete it */
    if( !strncmp(debugstr_w(whitelistW), "L\"\"", strlen("L\"\"")) || !whitelistW){
        return TRUE;
    }

    for(p = whitelistW; *p; p += lstrlenW(p) + 1){
        //TRACE("[whitelistLastW][%d]===>p:%s\n", __LINE__, wchars_to_utf8_chars(p));
        if(strcmpW(extensionW,p)==0){
            isInWhitelist = TRUE;
            break;
        }
    ...
    if(!isInWhitelist && isInwhitelast){//Indecates delete the whitelist
        return TRUE;
    }
...
static BOOL remove_mimetype(const char *xdg_data_home, WCHAR *extensionW)
{
    int matchType = 0;
    char *tmpfile = NULL;
    char *mimeTypeA = NULL;
    ...
    tmpfile = heap_printf("%s/applications/tmp.txt", in_container_xdg_data_dir);
    if (tmpfile == NULL)
    {
        WINE_ERR("out of memory\n");
        return FALSE;
    ...
    char *desktopPath = heap_printf("%s/applications/wine-extension-%s.desktop", in_container_xdg_data_dir, friendlyAppNameA);
    if(!desktopPath)
        return FALSE;
    ...
    desktopFp = fopen(desktopPath, "r");
    desktopFpW = fopen(tmpfile, "w");
    if(!desktopFp || !desktopFpW){
        WINE_TRACE("===>line[%d]==>file open fail\n", __LINE__);
        return FALSE;
    }
    while(fgets(tmp_buff, 1024, desktopFp)){
        if(mimeTypeA)
            ret = strstr(tmp_buff,mimeTypeA);
        if(ret){
            int len,offset_len;
            len = ret-tmp_buff;
            matchType = 1;
            
            offset_len = len+strlen(mimeTypeA)+1;
            memcpy(mimeString, tmp_buff,len);
            memcpy(mimeString+len, tmp_buff+offset_len, strlen(tmp_buff)-offset_len);
            
            memset(tmp_buff, 0, sizeof(tmp_buff));
            memcpy(tmp_buff, mimeString, strlen(mimeString));

            if(!strcmp(tmp_buff, "MimeType=\n")){//delete file
                WINE_TRACE("=====>line[%d]====>remove file\n",__LINE__);
                matchType = 0;
                remove(desktopPath);
                break;
            }
        }
        fwrite(tmp_buff, strlen(tmp_buff), 1, desktopFpW);
        memset(mimeString, 0, sizeof(mimeString));
        memset(tmp_buff, 0, sizeof(tmp_buff));
    }
    ...
    if(matchType){
        remove(desktopPath);
        if(rename(tmpfile,desktopPath) != 0){
            WINE_TRACE("=====>line[%d]====>rename file fail\n",__LINE__);
            return FALSE;
        }
    }
    else{
...
//<renzhengyi><202010607><for merge multiple desktop file into one> add end

//<yangxinruo><202010407><for file assocation individual configuration in winecfg> add begin
static DWORD reg_GetMultiStringCharacterLen(LPCWSTR multi)
{
    DWORD ret;
...
static BOOL has_whitelist_changed(LPCWSTR extensionW,LPCWSTR whilelistW,LPCWSTR whitelistLastW){
    if(!whilelistW&&!whitelistLastW){
        return FALSE;
    }
    WCHAR *p;
    if(whilelistW&&whitelistLastW){
        int foundCount=0;
        for (p = whilelistW; *p; p += lstrlenW(p) + 1)
        {
            if(strcmpW(extensionW,p)==0){
                foundCount++;
                break;
...
static BOOL cleanup_associations(void)
{
    ...
    static const WCHAR DesktopFileW[] = {'D','e','s','k','t','o','p','F','i','l','e',0};
    //<yangxinruo><202010407><for file assocation individual configuration in winecfg> add begin
    static const WCHAR AsoWhitelistW[] = {'W','h','i','t','e','L','i','s','t',0};
    static const WCHAR AsoWhitelistLastW[] = {'W','h','i','t','e','l','i','s','t','L','a','s','t',0};
    //<yangxinruo><202010407><for file assocation individual configuration in winecfg> add end
    HKEY assocKey;
    ...
    {
        //<yangxinruo><202010407><for file assocation individual configuration in winecfg> add begin
        WCHAR* aso_whitelistW = NULL;
        WCHAR* aso_lastwhiteLastW = NULL;
        aso_whitelistW = reg_get_valsW(assocKey, NULL, AsoWhitelistW);
        aso_lastwhiteLastW = reg_get_valsW(assocKey,NULL,AsoWhitelistLastW);
        //<yangxinruo><202010407><for file assocation individual configuration in winecfg> add end
        ...
        for (i = 0; !done;)
        {
            ...
            if (ret == ERROR_SUCCESS)
            {
                ...
                //<yangxinruo><202010407><for file assocation individual configuration in winecfg> modify begin
				/*
                if (command == NULL)
                {
				...
				*/
                ...
                // BOOL isInWhitelistLast=FALSE;
                // WCHAR* p;
                // if(aso_lastwhiteLastW){
                //     for (p = aso_lastwhiteLastW; *p; p += lstrlenW(p) + 1)
                //     {
                //         if (strcmpW(extensionW,p)==0){
                //             isInWhitelistLast=TRUE;
                //             break;
                //         }
                //     }
                // }
                // if (command == NULL||!isInWhitelist)
                if(command == NULL || isDelWhitelist( extensionW, aso_whitelistW, aso_lastwhiteLastW) )
                {
                    char *desktopFile = reg_get_val_utf8(assocKey, extensionW, DesktopFileW);
                    if (desktopFile)
                    {
                        WINE_TRACE("removing file type association for %s\n", wine_dbgstr_w(extensionW));
                        //<renzhengyi><202010607><for merge multiple desktop file into one> modify begin
                        remove_mimetype(xdg_data_dir, extensionW);//ray modify
                        //remove(desktopFile);
                        //<renzhengyi><202010607><for merge multiple desktop file into one> modify end
                    }
                    if(command==NULL){
                        hasChanged = TRUE;
                        RegDeleteKeyW(assocKey, extensionW);
                    }else{
                        if(has_whitelist_changed(extensionW,aso_whitelistW,aso_lastwhiteLastW)){
                            hasChanged = TRUE;
                        }
                        i++;
                    }
                ...
                //<yangxinruo><202010407><for file assocation individual configuration in winecfg> modify end
                else
                    i++;
        ...
        //<yangxinruo><202010407><for file assocation individual configuration in winecfg> add begin
        HeapFree(GetProcessHeap(), 0, aso_whitelistW);
        HeapFree(GetProcessHeap(), 0, aso_lastwhiteLastW);
        //<yangxinruo><202010407><for file assocation individual configuration in winecfg> add end
        RegCloseKey(assocKey);
    }
    else
                                                ...
                                                const char *progId, const char *openWithIcon)
{
    ...
    FILE *tmpfp;
    int len = 0;
    char mime_buff[1024] = {0};
    char mimeString[1024] = {0};
    char mimeValue[256] = {0};
    const char *prefix = getenv("WINEPREFIX");
    const char *home = getenv("HOME");
    const char *desktopTmp = heap_printf("%s/xdg-data/applications/desktopTmp.txt", prefix);
    ...
    //<renzhengyi><202010607><for merge multiple desktop file into one> modify begin
    desktopFpW = fopen(desktopPath, "r");//ray add
    if(desktopFpW){
        tmpfp = fopen(desktopTmp, "w");
        if(tmpfp){

            while(fgets(mime_buff, 1024, desktopFpW)){
                len += strlen(mime_buff);
                if(len > 1024){
                    WINE_TRACE("====>line[%d]=>mimetype greater than 1024\n", __LINE__);
                    return ret;
                }
                if(strstr(mime_buff,"MimeType=")){
                    
                    sscanf(mimeType, "%s;\n", mimeValue);
                    memcpy(mimeString, mime_buff, strlen(mime_buff));

                    if(strlen(mime_buff) + strlen(mimeValue) > 1024){
                        WINE_TRACE("====>line[%d]=>mimetype greater than 1024\n", __LINE__);
                        return ret;
                    }
                    memcpy(mimeString+strlen(mime_buff)-1, mimeValue, strlen(mimeValue));
                    
                    fprintf(tmpfp,"%s;\n",mimeString);
                    memset(mime_buff, 0, strlen(mime_buff));
                    memset(mimeString, 0, strlen(mimeString));

                    ret = TRUE;
                    continue;

                }
                else if(strstr(mime_buff,"Exec=env")){
                    if (prefix) {
                        // char* appName = (char*) malloc(50);
                        // char* appVersion = (char*) malloc(50);
                        // memset(appName, 0, sizeof(appName));
                        ...
                        getAppNameAndVersion(prefix, appName, appVersion);
                        if (strlen(appName) > 0 && strlen(appVersion) > 0) {
                            fprintf(tmpfp, "Exec=/opt/winux/tools/appmanager/wine-appmanager %s %s start start /ProgIDOpen %s %%f\n", appName, appVersion, progId);
                        } else {
                            fprintf(tmpfp, "Exec=env WINEPREFIX=\"%s\" wine start /ProgIDOpen %s %%f\n", prefix, progId);
                        }
                        // free(appName);
                        // free(appVersion);
                    }
                        ...
                        fprintf(tmpfp, "Exec=env WINEPREFIX=\"%s/.wine\" wine start /ProgIDOpen %s %%f\n", home, progId);
                    else
                        fprintf(tmpfp, "Exec=wine start /ProgIDOpen %s %%f\n", progId);
                    ret = TRUE;
                    continue;
                }
                fwrite(mime_buff, strlen(mime_buff), 1,tmpfp);
                memset(mime_buff, 0, sizeof(char)*1024);
            }
            len = 0;
            fclose(tmpfp);
            fclose(desktopFpW);
            ...
            remove(desktopPath);
             if(rename(desktopTmp, desktopPath) == -1){
                 WINE_TRACE("rename fail, error:%s\n", strerror(errno));
                 ret = FALSE;
             }
        ...
        if (desktop)
        {
            ...
            if (prefix) {
                ...
                getAppNameAndVersion(prefix, appName, appVersion);
                if (strlen(appName) > 0 && strlen(appVersion) > 0) {
                    fprintf(desktop, "Exec=/opt/winux/tools/appmanager/wine-appmanager %s %s start start /ProgIDOpen %s %%f\n", appName, appVersion, progId);
                } else {
    ...
    HeapFree(GetProcessHeap(), 0, desktopTmp);
    return ret;
}
...
static BOOL generate_associations(const char *xdg_data_home, const char *packages_dir, const char *applications_dir)
{
    ...
    {
        aso_whitelistW=reg_get_valsW(hkey,NULL,AsoWhitelistW);
        aso_lastwhiteLastW=reg_get_valsW(hkey,NULL,AsoWhitelistLastW);
        RegCloseKey( hkey );
        WINE_TRACE("YYYXXXRRR---> get aso whitelist = %s, lastlist=%s\n",debugstr_w(aso_whitelistW),debugstr_w(aso_lastwhiteLastW));
    }
    //<yangxinruo><202010407><for file assocation individual configuration in winecfg> add end
    for (i = 0; ; i++)
    {
        ...
        if (ret == ERROR_SUCCESS && extensionW[0] == '.' && !is_extension_banned(extensionW))
        {
            ...
            if (friendlyAppNameW)
            {
                ...
                //<yangxinruo><20210206><for do not export file assocaition of fileopenbylinux to linux system!!> add begin
                if(strcmp(friendlyAppNameA,"fileopenbylinux")==0){
                    TRACE("NEVER export fileopenbylinux to linux system!! for ext=%s\n",extensionA);
                    goto end;
                }
                //<yangxinruo><20210206><for do not export file assocaition of fileopenbylinux to linux system!!> add end
            }
            else
            {
            ...
            //assume every config changed
			//<yangxinruo><202010407><for file assocation individual configuration in winecfg> modify
            if (has_association_changed(extensionW, mimeTypeA, progIdW, friendlyAppNameA, openWithIconA)
                || has_whitelist_changed(extensionW,aso_whitelistW,aso_lastwhiteLastW))
            {
                //<renzhengyi><202010607><for merge multiple desktop file into one> modify begin
                //char *desktopPath = heap_printf("%s/wine-extension-%s.desktop", applications_dir, &extensionA[1]);
                char *desktopPath = heap_printf("%s/wine-extension-%s.desktop", applications_dir, friendlyAppNameA);//ray modify
                if (desktopPath)
                {
                    if (!isAddWhitelist( extensionW, aso_whitelistW, aso_lastwhiteLastW)){//ray add:if the whitelist is not add, skip "write_freedesktop_association_entry"
                        hasChanged = TRUE;
                        update_association(extensionW, mimeTypeA, progIdW, friendlyAppNameA, desktopPath, openWithIconA);
                    }
                    else if (write_freedesktop_association_entry(desktopPath, extensionA, friendlyAppNameA, mimeTypeA, progIdA, openWithIconA))
                    {
                        hasChanged = TRUE;
                        update_association(extensionW, mimeTypeA, progIdW, friendlyAppNameA, desktopPath, openWithIconA);
                    }
...
static void refresh_whitelist( void ){
    ...
    if ((assocKey = open_associations_reg_key()))
    {
        WCHAR* aso_whitelistW;
        aso_whitelistW = reg_get_valsW(assocKey, NULL, AsoWhitelistW);
        if(aso_whitelistW){
            DWORD len=reg_GetMultiStringCharacterLen(aso_whitelistW);
            RegSetValueExW(assocKey, AsoWhitelistLastW, 0, REG_MULTI_SZ, (const BYTE *)aso_whitelistW,len * sizeof (WCHAR));
        }
        HeapFree(GetProcessHeap(), 0, aso_whitelistW);
        RegCloseKey(assocKey);
    }
...
static void RefreshFileTypeAssociations(void)
{
    ...
    char *homeapp_dir;
    char *winexdg_dir;
    homeapp_dir = heap_printf("%s/applications", xdg_data_dir);
    if(homeapp_dir == NULL){
        WINE_ERR("out of memory\n");
        goto end;
    }
    create_directories(homeapp_dir);

    winexdg_dir = heap_printf("%s/wine-xdg", xdg_data_dir);
    if(winexdg_dir == NULL){
        WINE_ERR("out of memory\n");
        goto end;
    }
    create_directories(winexdg_dir);
    //<renzhengyi><202010607><for merge multiple desktop file into one> modify end
    //<yangxinruo><202010407><for file assocation individual configuration in winecfg> modify

    applications_dir = heap_printf("%s/applications", in_container_xdg_data_dir);
    ...
    if (hasChanged)
    {
        ...
        // _spawnvp( _P_DETACH, argv[0], argv );
        char *xdg_collect_tool = NULL;
        xdg_collect_tool = heap_printf("/usr/local/bin/collector.py");
        //xdg_collect_tool = heap_printf("%s/.local/share/wine-xdg/collector.py", getenv("HOME"));
        if(xdg_collect_tool){
            argv[0] = "/usr/bin/python";
            argv[1] = xdg_collect_tool;
            argv[2] = applications_dir;
            argv[3] = NULL;
            TRACE_(yyyxxxrrr)("YYYXXXRRR---> to call script p0:%s p1:%s p2:%s\n",argv[0],argv[1],argv[2]);
            _spawnvp( _P_DETACH, argv[0], argv );
        }
        HeapFree(GetProcessHeap(), 0, xdg_collect_tool);
        //<yangxinruo><202010407><for file assocation individual configuration in winecfg> modify end
    }
    ...
    HeapFree(GetProcessHeap(), 0, homeapp_dir);
    HeapFree(GetProcessHeap(), 0, winexdg_dir);
}
...
static BOOL init_xdg(void)
{
    ...
    //<yangxinruo><202010407><for file assocation individual configuration in winecfg> add begin
    in_container_xdg_data_dir=heap_printf("%s/xdg-data", getenv("WINEPREFIX"));
    if(in_container_xdg_data_dir){
        TRACE_(yyyxxxrrr)("YYYXXXRRR---> create in container xdg config dir\n");
        create_directories(in_container_xdg_data_dir);
    }
    //<yangxinruo><202010407><for file assocation individual configuration in winecfg> add end
...
{
    //<yangxinruo><20210223><set winecfg associations_enabled default val to FALSE> modify begin
    BOOL ret = TRUE;
    //BOOL ret = FALSE;//ray modify
    //<yangxinruo><20210223><set winecfg associations_enabled default val to FALSE> modify end
    HKEY hkey;
    BYTE buf[32];
    DWORD len;
