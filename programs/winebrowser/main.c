...
static int launch_app( const WCHAR *candidates, const WCHAR *argv1 )
{
    ...
    static const WCHAR xdg_open[] ={'x','d','g','-','o','p','e','n',0};
    if (!(cmdline = strdup_unixcp( argv1 ))) return 1;

    while (*candidates)
    {
        WCHAR **args = CommandLineToArgvW( candidates, &count );
        //<yangxinruo><20201225><for wechat cannot open external broswer by no http:// url> add begin
        //our xdg_open do not know url without scheme
        // if(strcmp(candidates,xdg_open)==0){
        //     if(!strstr(cmdline,"://")){
        //         int strLen=strlen(cmdline);
        //         cmdline=HeapReAlloc(GetProcessHeap(), 0,cmdline, strLen+8);
        //         int ii=0;
        //         for(ii=strLen+7-1;ii>=7;--ii){
        //             cmdline[ii]=cmdline[ii-7];
        //         }
        //         cmdline[strLen+7]='\0';
        //         memcpy(cmdline,"http://",7);
        //     }
        // }
        //<yangxinruo><20201225><for wechat cannot open external broswer by no http:// url> add end
