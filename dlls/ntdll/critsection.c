...
NTSTATUS WINAPI RtlpWaitForCriticalSection( RTL_CRITICAL_SECTION *crit )
{
    ...
    for (;;)
    {
        ...
        if ( status == STATUS_TIMEOUT )
        {
            ...
            /* <--20210421 wanghg add for 云视讯退出会议，画面无响应-->  <--20210928 lidong add for 企业微信语音通话卡死--> <--20211214 zhangyanhui add for 有道云笔记偶尔死锁导致界面卡死-->*/ 
            FILE *f;
            char cParam[100];
            char buff[256];
            sprintf(cParam,"/proc/%d/cmdline",getpid());
            if ((f = fopen(cParam, "r"))) {
                fgets(buff, 255, f);
                fclose(f);
            }
            if (strstr(buff, "ViLin.exe") || strstr(buff, "WXWork.exe") || strstr(buff, "CAXA")|| strstr(buff,"YoudaoNote.exe") || strstr(buff, "TdxW.exe") || strstr(buff, "DWGView.exe")
                    || strstr(buff, "QQ.exe")) {
                RtlLeaveCriticalSection( crit );
                break;
            }
            /* 20210421 wanghg add for 云视讯退出会议，画面无响应-->  <--20210928 lidong add for 企业微信语音通话卡死--> <--20211214 zhangyanhui add for 有道云笔记偶尔死锁导致界面卡死--><--20211217 liujunlong add for 申万证券登录后界面卡死--> */
