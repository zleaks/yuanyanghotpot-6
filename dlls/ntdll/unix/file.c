                                           ...
                                           UINT disposition )
{
    ...
    NTSTATUS status;
    //BOOLEAN check_case = !(attr->Attributes & OBJ_CASE_INSENSITIVE); //zcg

    if (!attr->RootDirectory)  /* without root dir fall back to normal lookup */
        return nt_to_unix_file_name( attr->ObjectName, name_ret, disposition );
       // return nt_to_unix_file_name( attr->ObjectName, name_ret, disposition ,check_case);//zcg
...
//NTSTATUS nt_to_unix_file_name( const UNICODE_STRING *nameW, char **unix_name_ret, UINT disposition ,BOOLEAN check_case) //zcg
{
...
NTSTATUS CDECL wine_nt_to_unix_file_name( const UNICODE_STRING *nameW, char *nameA, SIZE_T *size,
                                          UINT disposition) //zcg
{
    char *buffer = NULL;
    NTSTATUS status = nt_to_unix_file_name( nameW, &buffer, disposition );
    //NTSTATUS status = nt_to_unix_file_name( nameW, &buffer, disposition, check_case); //zcg
                              ...
                              ULONG options, void *ea_buffer, ULONG ea_length )
{
    ...
    //add by xdl 解决Carsim8.02导出合并文件时路径错误
    const char *prefix = getenv("WINEPREFIX");
    if(prefix && strstr(prefix, "carsim8.02-win-u"))
    {
        if(strstr(debugstr_us(attr->ObjectName), "Z:\\\\users\\\\") && strstr(debugstr_us(attr->ObjectName), "\\\\Temp\\\\CarSim_SaveAs"))
        {
            attr->ObjectName->Buffer[4] = 'C';
        }
    }
    //end
