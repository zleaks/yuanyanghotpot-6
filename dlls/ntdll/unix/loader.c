...
static char *getHotpotAppDllPath() {
    char *path = getenv( "HOTPOT_APPDLLPATH" );
    
    //printf("111 HOTPOT_APPDLLPATH : %s \n", path);
    if (path) return path;

    path = getenv( "WINEPREFIX" );
    if (path && strstr(path, "/opt/winux/hotpot/appstore/")) {
        char *prefix = (char*) malloc(strlen(path) + 1);
        char *dllpath = (char*) malloc(255);
        memset(prefix, 0, sizeof(prefix));
        strcpy(prefix, path);
        memset(dllpath, 0, sizeof(dllpath));
        strcpy(dllpath, "/opt/winux/hotpot6/usr/local/share/wine/dlls/");
        char * pathstart = strrchr(prefix, '/');
        pathstart++;
        char *pathend = strrchr(prefix, '-');
        if (pathend) *pathend = '\0';
//        printf("111 path2 : %s \n", pathstart);
        if (!pathstart || strlen(pathstart) == 0) return NULL;
        dllpath = strcat(dllpath, pathstart);
//        printf("111 path222 : %s \n", dllpath);
        free(prefix);
        return dllpath;
    }
    return NULL;
...
static void set_dll_path(void)
{
    ...
    int i, count = 0;
// wanghg 20210820 add for hotpot building start
    int count1 = 0, count2 = 0;
    char *path2 = getHotpotAppDllPath();
// wanghg 20210820 add for hotpot building end

// wanghg 20210820 modify for hotpot building start
    // if (path) for (p = path, count = 1; *p; p++) if (*p == ':') count++;

    // dll_paths = malloc( (count + 2) * sizeof(*dll_paths) );

    if (path) for (p = path, count1 = 1; *p; p++) if (*p == ':') count1++;
    if (path2) for (p = path2, count2 = 1; *p; p++) if (*p == ':') count2++;
    dll_paths = malloc( (count1 + count2 + 2) * sizeof(*dll_paths) );
// wanghg 20210820 modify for hotpot building end
    count = 0;
...
// wanghg 20210820 add for hotpot building start
    ...
    {
        path2 = strdup(path2);
        for (p = strtok( path2, ":" ); p; p = strtok( NULL, ":" )) dll_paths[count++] = strdup( p );
        free( path2 );
    }
// wanghg 20210820 add for hotpot building end
...
static NTSTATUS CDECL load_so_dll( UNICODE_STRING *nt_name, void **module )
{
   ...
   // if (nt_to_unix_file_name( nt_name, &unix_name, FILE_OPEN ,FALSE)) return STATUS_DLL_NOT_FOUND; //zcg
