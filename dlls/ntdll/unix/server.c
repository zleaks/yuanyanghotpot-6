...
#define SOCKETNAME "/tmp/socket"        /* name of the socket file */
#define LOCKNAME   "lock"          /* name of the lock file */

static const BOOL is_win64 = (sizeof(void *) > sizeof(int));

static const char *server_dir;
static const char *socket_name_final;
...
static const char *init_server_dir( dev_t dev, ino_t ino )
{
    ...
    size_t len_socket = sizeof(SOCKETNAME) + 2 * sizeof(dev) + 2 * sizeof(ino) + 16;
    socket_name_final = malloc(len_socket);
    ...
    dir = malloc( len );
    sprintf( dir, "/opt/winux/server/.wine-%u/server-", getuid() );
    sprintf( socket_name_final, "/tmp/socket-%u-", getuid() );
#endif
    p = dir + strlen( dir );
    q = socket_name_final + strlen( socket_name_final );
    if (dev != (unsigned long)dev) {
        p += sprintf( p, "%lx%08lx-", (unsigned long)((unsigned long long)dev >> 32), (unsigned long)dev );
        q += sprintf( q, "%lx%08lx-", (unsigned long)((unsigned long long)dev >> 32), (unsigned long)dev );
    }
    else {
        p += sprintf( p, "%lx-", (unsigned long)dev );
        q += sprintf( q, "%lx-", (unsigned long)dev );
    }

    if (ino != (unsigned long)ino) {
        sprintf( p, "%lx%08lx", (unsigned long)((unsigned long long)ino >> 32), (unsigned long)ino );
        sprintf( q, "%lx%08lx", (unsigned long)((unsigned long long)ino >> 32), (unsigned long)ino );
    }
    else {
        sprintf( p, "%lx", (unsigned long)ino );
        sprintf( q, "%lx", (unsigned long)ino );
    }
    return dir;
}
...
static int server_connect(void)
{
    ...
    for (retry = 0; retry < 6; retry++)
    {
        ...
        if (retry)
        {
            ...
            start_server( TRACE_ON(server) );
            if (lstat( socket_name_final, &st ) == -1) continue;  /* still no socket, wait a bit more */
        }
        else if (lstat( socket_name_final, &st ) == -1) /* check for an already existing socket */
        {
            if (errno != ENOENT) fatal_perror( "lstat %s/%s", server_dir, socket_name_final );
            start_server( TRACE_ON(server) );
            if (lstat( socket_name_final, &st ) == -1) continue;  /* still no socket, wait a bit more */
        }
        ...
        if (!S_ISSOCK(st.st_mode) && !S_ISFIFO(st.st_mode))
            fatal_error( "'%s/%s' is not a socket\n", server_dir, socket_name_final );
        //if (st.st_uid != getuid())
        //    fatal_error( "'%s/%s' is not owned by you\n", server_dir, socket_name_final );

        /* try to connect to it */
        addr.sun_family = AF_UNIX;
        strcpy( addr.sun_path, socket_name_final );
