...
static NTSTATUS MODULE_InitDLL( WINE_MODREF *wm, UINT reason, LPVOID lpReserved )
{
    ...
    //<yangxinruo><20210707><for Zoom sdk(Vilin etc.) freeze when exit meeting> add begin
    if(wcscmp( wm->ldr.BaseDllName.Buffer, L"CptShare.dll" )==0&&reason==0){
        WARN("YYYXXXRRR--->WARNING!!! we try to skip calling PROCESS DETACH entry of cptshare.dll!!!\n");
        return STATUS_DLL_INIT_FAILED;
    }

    // 修改打开语音会议高概率卡死的问题
    if(wcscmp( wm->ldr.BaseDllName.Buffer, L"xnn_core.dll" )==0&&reason==DLL_THREAD_ATTACH){
        WARN("YYYXXXRRR--->WARNING!!! we try to skip calling DLL_THREAD_ATTACH entry of xnn_core.dll!!!\n");
        return STATUS_DLL_INIT_FAILED;
    }
    //<yangxinruo><20210707><for Zoom sdk(Vilin etc.) freeze when exit meeting> add end
