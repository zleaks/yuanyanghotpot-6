...
#define DRAG_TARGET_TYPE_WINE 0
#define DRAG_TARGET_TYPE_XDND 1
  ...
  DWORD       *pdwEffect)    /* [out] ptr to effects of the source */
{
  ...
  trackerInfo.curTargetIdentifier = NULL;
  trackerInfo.curTargetType = DRAG_TARGET_TYPE_WINE;

  POINT initPoint;
  if(GetCursorPos(&initPoint)){
    HWND hwndInitSource = WindowFromPoint(initPoint);
    LONG style = GetWindowLongW( hwndInitSource, GWL_STYLE );
    while(style&WS_CHILD){
      hwndInitSource=GetParent(hwndInitSource);
      style = GetWindowLongW( hwndInitSource, GWL_STYLE );
    }
    TRACE_(xdnd)("set source window at pt(%d,%d) hwnd=%p\n",initPoint.x,initPoint.y,hwndInitSource);
    if(hwndInitSource){
      pXDndDragInit(hwndInitSource);
    }
  }  
  ...
  if (hwndTrackWindow)
  {
     ...
     */
    pXDndDragDeinit(hwndTrackWindow);
...
static IDropTarget* WrapXDndDropTarget( INT xWindowId);

static void drag_enter( TrackerWindowInfo *info, void* new_target_ident,int newTargetType )
{
    HRESULT hr;

    info->curTargetIdentifier = new_target_ident;
    info->curTargetType=newTargetType;

    if(info->curTargetType==DRAG_TARGET_TYPE_WINE){
      info->curTargetHWND = (HWND)new_target_ident;
      HWND new_target = (HWND)new_target_ident;
    ...
    }else if(info->curTargetType==DRAG_TARGET_TYPE_XDND){
      info->curDragTarget = WrapXDndDropTarget( (int)new_target_ident );
    }
...
static HRESULT WINAPI DropTargetXDndWrapper_DragEnter(IDropTarget* iface,
                                                  IDataObject* pDataObj,
                                                  DWORD grfKeyState,
                                                  POINTL pt,
                                                  DWORD* pdwEffect)
{
  ...
  DropTargetXDndWrapper* This = impl_from_IDropTarget(iface);
  INT status = pXDndTargetEnter(This->xWindowId,pDataObj);

  return r;
}

static HRESULT WINAPI DropTargetXDndWrapper_DragOver(IDropTarget* iface,
                                                 DWORD grfKeyState,
                                                 POINTL pt,
                                                 DWORD* pdwEffect)
{
  ...
  *pdwEffect = DROPEFFECT_NONE;
  INT status = pXDndTargetOver(This->xWindowId,pt.x,pt.y);
  if(status == 0 ){
...
static HRESULT WINAPI DropTargetXDndWrapper_DragLeave(IDropTarget* iface)
{
  HRESULT r = NO_ERROR;
  DropTargetXDndWrapper* This = impl_from_IDropTarget(iface);
  pXDndTargetLeave(This->xWindowId);
  // if(This->pDataObj){
  //   TRACE_(yyyxxxrrr)("YYYXXXRRR--->XDND release IDataObject(%p) in DragLeave!\n",This->pDataObj);
  //   IDataObject_Release(This->pDataObj);
  //   This->pDataObj=NULL;
  // }
  return r;
}

static HRESULT WINAPI DropTargetXDndWrapper_Drop(IDropTarget* iface,
                                             IDataObject* pDataObj,
                                             DWORD grfKeyState,
                                             POINTL pt,
                                             DWORD* pdwEffect)
{
  HRESULT r = NO_ERROR;
  DropTargetXDndWrapper* This = impl_from_IDropTarget(iface);
  pXDndTargetDrop(This->xWindowId,pt.x,pt.y,pDataObj);
  return r;
}

// static ULONG WINAPI DropTargetXDndWrapper_Release(IDropTarget* iface)
// {
//     DropTargetXDndWrapper* This = impl_from_IDropTarget(iface);
//     ULONG refs = InterlockedDecrement(&This->refs);
//     if (!refs) {
//       if(This->pDataObj){
//         TRACE_(yyyxxxrrr)("YYYXXXRRR--->XDND release IDataObject(%p) in Release!\n",This->pDataObj);
//         IDataObject_Release(This->pDataObj);
//         This->pDataObj=NULL;
//       }
//       HeapFree(GetProcessHeap(), 0, This);
...
static const IDropTargetVtbl DropTargetXDndWrapperVTbl =
{
    DropTargetWrapper_QueryInterface,
    DropTargetWrapper_AddRef,
    DropTargetWrapper_Release,
    DropTargetXDndWrapper_DragEnter,
    DropTargetXDndWrapper_DragOver,
    DropTargetXDndWrapper_DragLeave,
    DropTargetXDndWrapper_Drop
};

static IDropTarget* WrapXDndDropTarget( INT xWindowId )
{
    DropTargetXDndWrapper* This = HeapAlloc(GetProcessHeap(), 0, sizeof(*This));

    if (This)
    {
        This->IDropTarget_iface.lpVtbl = &DropTargetXDndWrapperVTbl;
...
static void OLEDD_TrackStateChange(TrackerWindowInfo* trackerInfo)
{
  ...
  POINT pt;
  void* newTargetIdentifier = NULL;
  int newTargetType = DRAG_TARGET_TYPE_WINE;
  ...
  hwndNewTarget = WindowFromPoint_internal(pt,CWP_SKIPINVISIBLE|CWP_SKIPTRANSPARENT);
#if defined __HOTPOT_160ZIPTOOL__ || __HOTPOT_SPEEDZIP__
  if(GetPropA( hwndNewTarget, foreign_window_prop ))
#else
  if(GetPropA( hwndNewTarget, foreign_window_prop )||GetDesktopWindow()==hwndNewTarget)
#endif
  {
    newTargetType=DRAG_TARGET_TYPE_XDND;
    newTargetIdentifier = (void*)pXDndPickUpTargetWindow(pt.x,pt.y);
  }else{
    newTargetType=DRAG_TARGET_TYPE_WINE;
    newTargetIdentifier = (void*)hwndNewTarget;
  }

  // TRACE_(yyyxxxrrr)("YYYXXXRRR---> fetch a newTargetIdentifier=%p target type= %d\n",newTargetIdentifier,newTargetType);

  trackerInfo->returnValue = IDropSource_QueryContinueDrag(trackerInfo->dropSource,
                                                           trackerInfo->escPressed,
                                                           trackerInfo->dwKeyState);

  // // if(!hwndNewTarget||GetPropA( hwndNewTarget, foreign_window_prop )){
  //   // TRACE_(yyyxxxrrr)("YYYXXXRRR---> OLEDD_TrackStateChange we may outof wine windows\n");
  // trackerInfo->isForeignWindow=pDoDragDropPickUpTarget(GetPropA( hwndNewTarget, foreign_window_prop )?TRUE:FALSE,pt.x,pt.y,trackerInfo->returnValue);
  // // }
  // TRACE_(yyyxxxrrr)("YYYXXXRRR---> foreignWindowStatus = %d\n",trackerInfo->isForeignWindow);

  if (((trackerInfo->curTargetType!=newTargetType||trackerInfo->curTargetIdentifier != newTargetIdentifier)) &&
      (trackerInfo->returnValue == S_OK ||
       trackerInfo->returnValue == DRAGDROP_S_DROP))
  {
    if (trackerInfo->curDragTarget)
    {
      TRACE_(xdnd)("YYYXXXRRR---> curDragTarget changed!! type(%d->%d) identfier(%08x->%08x)\n",trackerInfo->curTargetType,newTargetType,trackerInfo->curTargetIdentifier, newTargetIdentifier);
    ...
    if (newTargetIdentifier)
        drag_enter( trackerInfo, newTargetIdentifier, newTargetType );
  ...
  if (trackerInfo->returnValue == S_OK)
  {
    if (trackerInfo->curDragTarget)
    {
      ...
      *trackerInfo->pdwEffect &= trackerInfo->dwOKEffect;
    // }else{
    //   if(trackerInfo->isForeignWindow == 0 ){
    //     *trackerInfo->pdwEffect |= DROPEFFECT_COPY;
    //     *trackerInfo->pdwEffect &= trackerInfo->dwOKEffect;
    //     TRACE_(yyyxxxrrr)("YYYXXXRRR---> reset pdwEffect=%08x dwOKEffect=%08x\n",*trackerInfo->pdwEffect,trackerInfo->dwOKEffect);
    //   }
    }
    give_feedback( trackerInfo );
  }
