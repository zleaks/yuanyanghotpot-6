                                       ...
                                       STGMEDIUM *med)
{
    ...
    //<yangxinruo><for wechat can not copy attachment and video> add begin
    if(fmt->cfFormat==CF_HDROP){
        TRACE("YYYXXXRRR---> FOR CF_HDROP USE TYMED_HGLOBAL\n");
        mask|=TYMED_HGLOBAL;
    }
    //<yangxinruo><for wechat can not copy attachment and video> add end
    if(!mask)
    {
        hr = DV_E_TYMED;
        goto end;
 ...
 * source data object.
 */
HRESULT internal_render_format(IDataObject *data, LPFORMATETC fmt,HANDLE *clip_data)
{
    HRESULT hr;
    /* Embed source hack */
    if(fmt->cfFormat == embed_source_clipboard_format)
...
void internal_release_rendered_data(LPFORMATETC fmt,HANDLE *clip_data){
    if(!(*clip_data)){
        return;
    }
    if(fmt->tymed & TYMED_MFPICT)
