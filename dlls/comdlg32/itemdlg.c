...
typedef struct FileDialogImpl {
  ...
  static void debug_filedialogimpl(FileDialogImpl *This)
 {
	 ...
	 */
	 ...
	 LPWSTR set_filename;//默认名字
	 LPWSTR default_ext;//默认后缀
	 LPWSTR custom_title;
	 LPWSTR custom_okbutton;
	 LPWSTR custom_cancelbutton;
	 LPWSTR custom_filenamelabel;
	 FILEOPENDIALOGOPTIONS options;
	 COMDLG_FILTERSPEC *filterspecs;//过滤条件及描述
	 UINT filterspec_count;
	 UINT filetypeindex;
	 */
	 int filterspec_index=0;
	 WCHAR extbuf[MAX_PATH], *newext = NULL;
 
	 memset(extbuf,0,sizeof(extbuf));
	 for(filterspec_index=0;filterspec_index<This->filterspec_count;filterspec_index++)
	 {
		 newext = get_first_ext_from_spec(extbuf, This->filterspecs[filterspec_index].pszSpec);
		 TRACE("%s%s%s\n",debugstr_w(newext),debugstr_w(This->filterspecs[filterspec_index].pszSpec),debugstr_w(This->filterspecs[filterspec_index].pszName));
		 memset(extbuf,0,sizeof(extbuf));
	 }
	 ...
	 TRACE("(%s)(%s)(%s)(%s)(%s)(%s)(%s)(%d)(%d)\n", 
	 debugstr_w(This->set_filename),debugstr_w(This->default_ext),
	 debugstr_w(This->custom_title),debugstr_w(This->custom_okbutton),
	 debugstr_w(This->custom_cancelbutton),debugstr_w(This->custom_filenamelabel),
	 debugstr_w(newext),This->filterspec_count,
	 This->filetypeindex);
 
 }

static void init_OPENFILENAMEW(FileDialogImpl *This)
{
	...
	int filterspec_index=0;
	WCHAR extbuf[MAX_PATH], *newext = NULL;
	LPWSTR lpstrPos,lpstrPosStart;
    int  filter_len=0;
	for(filterspec_index=0;filterspec_index<This->filterspec_count;filterspec_index++)
	{
		 filter_len+=(lstrlenW(This->filterspecs[filterspec_index].pszName));
		 filter_len+=(lstrlenW(This->filterspecs[filterspec_index].pszSpec));
		 filter_len+=2;//中间空字符空间
	}

	if(filter_len>0)
	{
	
		filter_len+=1;//最后空字符空间
		This->psg_ofn->lpstrFilter=(LPWSTR)HeapAlloc(GetProcessHeap(), 0, sizeof(WCHAR)*filter_len);
		memset(This->psg_ofn->lpstrFilter,0,sizeof(WCHAR)*filter_len);
		lpstrPos=This->psg_ofn->lpstrFilter;
		lpstrPosStart=lpstrPos;
		for(filterspec_index=0;filterspec_index<This->filterspec_count;filterspec_index++)
		{
			
			lstrcpyW(lpstrPos,This->filterspecs[filterspec_index].pszName);
			lpstrPos+=lstrlenW(lpstrPos);
			*lpstrPos=(WCHAR)('\0');
			lpstrPos+=1;
			lstrcpyW(lpstrPos,This->filterspecs[filterspec_index].pszSpec);
			//newext = get_first_ext_from_spec(extbuf, This->filterspecs[filterspec_index].pszSpec);
			lpstrPos+=lstrlenW(lpstrPos);
			*lpstrPos=(WCHAR)('\0');
			lpstrPos+=1;
			TRACE("fileter::<%s><%s>\n",debugstr_w(This->filterspecs[filterspec_index].pszSpec),debugstr_w(This->filterspecs[filterspec_index].pszName));
		}
		*(lpstrPos)=(WCHAR)('\0');
		TRACE("filter_len(%p)(%p)(%d)(%d)\n",lpstrPosStart,lpstrPos,lpstrPos-lpstrPosStart,filter_len);
	}
	//初始化相关选项
	if(This->options&FOS_OVERWRITEPROMPT){
		This->psg_ofn->Flags|=OFN_OVERWRITEPROMPT;
	}

	//默认名字
	if((This->set_filename)&&lstrlenW(This->set_filename)){
		lstrcpyW(This->psg_ofn->lpstrFile,This->set_filename);
		TRACE("set_filename::<%s>\n",debugstr_w(This->set_filename));
	}

	//默认后缀
	if((This->default_ext)&&lstrlenW(This->default_ext)){
		This->psg_ofn->lpstrDefExt=HeapAlloc(GetProcessHeap(), 0, sizeof(WCHAR) * (lstrlenW(This->default_ext)+1));
		lstrcpyW(This->psg_ofn->lpstrDefExt,This->default_ext);
		TRACE("default_ext::<%s>\n",debugstr_w(This->default_ext));
	}
...
HRESULT  gtk_set_opened_file_name(FileDialogImpl *This)
{
    ...
    LPITEMIDLIST *pidla;
	UINT file_count;
    if((0==This->psg_ofn->lpstrFile[0])||This->psg_ofn->FlagsEx)//没有有效的文件路径
		return S_OK;
    file_count =1;
	...
	for(i = 0; i < file_count; i++){
		...
		//TRACE("GTK_DIALOG:(%s,%d)\n",debugstr_w(sg_ofn.lpstrFile),sg_ofn.nMaxFile);
		//gtk_unix_path_to_win_path(sg_ofn.lpstrFile,sg_ofn.nMaxFile);
		pidla[i] = COMDLG32_SHSimpleIDListFromPathAW(This->psg_ofn->lpstrFile);
	...
	This->psia_results = NULL;
	hr = SHCreateShellItemArray(NULL, NULL, file_count, (PCUITEMID_CHILD_ARRAY)pidla,
								&This->psia_results);
	This->psg_ofn->FlagsEx=1;
	if(This->psg_ofn->nFilterIndex>0){
		This->filetypeindex=This->psg_ofn->nFilterIndex-1;
		events_OnTypeChange(This);
	}
	TRACE("GTK_DIALOG:(%s,%d,%d)\n",debugstr_w(This->psg_ofn->lpstrFile),This->psg_ofn->nMaxFile,This->filetypeindex);
 ...
 *		
 */
static BOOL  GtkEndDialog(FileDialogImpl *This)
{
    HWND owner;

    TRACE("\n");
    if (!(This&&This->psg_ofn&&This->psg_ofn->hwndOwner))
		return FALSE;

	TRACE("%p\n", This->psg_ofn->hwndOwner);
	owner=This->psg_ofn->hwndOwner;
...
static HRESULT WINAPI IFileDialog2_fnShow(IFileDialog2 *iface, HWND hwndOwner)
{
    ...
    This->opendropdown_has_selection = FALSE;
    This->hfont_opendropdown = NULL;
	/*************<zyh><2021-10-14> <add gtk funcs> begin*********/
	#ifdef GTK_DIALOG
    if(FILESTYLE_GTK==get_FileManage_Style(0)){
		    BOOL bret=FALSE;
			HRESULT res=HRESULT_FROM_WIN32(ERROR_CANCELLED);
		    Gtk_GetPofn(&(This->psg_ofn));
			init_OPENFILENAMEW(This);
			This->psg_ofn->hwndOwner=hwndOwner;
			TRACE("%p (%p)\n", This->psg_ofn,This->psg_ofn->hwndOwner);
	        if(ITEMDLG_TYPE_OPEN==This->dlg_type){
	          bret=Gtk_GetOpenFileNameW_wrapper(This->psg_ofn);
			  if(bret){
			    gtk_set_opened_file_name(This);
				res=(HRESULT)(S_OK);
			  }
			  GtkEndDialog(This);
	        }
	        else if(ITEMDLG_TYPE_SAVE==This->dlg_type){
	          bret=Gtk_GetSaveFileNameW_wrapper(This->psg_ofn);
			  if(bret){
			    gtk_set_opened_file_name(This);				
				res=(HRESULT)(S_OK);
			  }
	...
	/*************<zyh><2021-10-14> <add gtk funcs> end*********/
