...
static void init_filedlg_infoA(OPENFILENAMEA *ofn, FileOpenDlgInfos *info)
{
	...
	TRACE("ofn->lpstrFile:%s\n", ofn->lpstrFile);
	if (strcmp(ofn->lpstrFile,"*.cpar") == 0
            || strcmp(ofn->lpstrFile, "*.cvt_cfg") == 0
	    || strcmp(ofn->lpstrFile, "*_bin.cvt_cfg") == 0
	    || strcmp(ofn->lpstrFile, "*_csv.cvt_cfg") == 0
	    || strcmp(ofn->lpstrFile, "*outputs_tab.txt") == 0
	    || strcmp(ofn->lpstrFile, "*.ind") == 0
	    || strcmp(ofn->lpstrFile, "No Data.cpar") == 0
	    || strcmp(ofn->lpstrFile, "*.par") == 0)

        {
            len = MultiByteToWideChar(CP_ACP, 0, ofn->lpstrFile, -1, NULL, 0);
        }
        else
        {
...
static INT_PTR FILEDLG95_Handle_GetFilePath(HWND hwnd, DWORD size, LPVOID result)
{
    ...
    char * tmpUnixFullPath = wine_get_unix_file_name(result);
    total =  MultiByteToWideChar( CP_ACP, 0, tmpUnixFullPath, -1, NULL, 0) ;
    MultiByteToWideChar( CP_ACP, 0,tmpUnixFullPath, -1, buffer, total) ;
#endif
    if (len)
    {
    ...
    {
#if defined __HOTPOT_AUTOCAD2008__
        if (result) lstrcpynW( result, buffer, total );
#else
...
/*************<zyh><2021-10-28> <add gtk funcs> begin*********/

/**********************************************************************
*
*
*/
static BOOL Gtk_GetOpenFileNameA(OPENFILENAMEA *ofn)
{

	BOOL b_ret=FALSE;
	OPENFILENAMEW* p_ofnw=Gtk_Get_OPENFILENAMEW_From_OPENFILENAMEA(ofn);//转换成unicode结构体
	if(!p_ofnw)
		return b_ret;
	b_ret=Gtk_GetOpenFileNameW(p_ofnw);
	if(b_ret)
		b_ret=Gtk_Get_OPENFILENAMEA_From_OPENFILENAMEW(ofn,p_ofnw);//转换成多字节结构体
	Gtk_FreePofn(p_ofnw);//释放unicode结构体及其内部内存
	return b_ret;
}
static BOOL Gtk_GetSaveFileNameA(OPENFILENAMEA *ofn)
{
	BOOL b_ret=FALSE;
	OPENFILENAMEW* p_ofnw=Gtk_Get_OPENFILENAMEW_From_OPENFILENAMEA(ofn);//转换成unicode结构体
	if(!p_ofnw)
		return b_ret;
	b_ret=Gtk_GetSaveFileNameW(p_ofnw);
	if(b_ret)
		b_ret=Gtk_Get_OPENFILENAMEA_From_OPENFILENAMEW(ofn,p_ofnw);//转换成多字节结构体
	Gtk_FreePofn(p_ofnw);//释放unicode结构体及其内部内存
	return b_ret;
}
/*************<zyh><2021-10-28> <add gtk funcs> end*********/
...
BOOL WINAPI GetOpenFileNameA(OPENFILENAMEA *ofn)
{
    ...
    {
		/*************<zyh><2021-10-28> <add gtk funcs>*********/
        if(FILESTYLE_GTK==get_FileManage_Style(ofn->Flags))
        {
			return Gtk_GetOpenFileNameA(ofn);
        }
		/*************<zyh><2021-10-28> <add gtk funcs> end*********/
...
static BOOL (CDECL *pGTKGetSaveFileNameW)(LPOPENFILENAMEW ofn) = NULL;
static BOOL (CDECL *pGTKGetOpenFileNameW)(OPENFILENAMEW *ofn) = NULL;

 static HMODULE load_gtk_graphics_driver(void)
{
...
//<yangxinruo><20210220><for wechat ungrabpointer before call GTK file dialog> add end
 BOOL callSystemGetOpenFileNameW(OPENFILENAMEW *ofn){
  if(!pGTKGetOpenFileNameW){
      HMODULE module = load_gtk_graphics_driver();
      pGTKGetOpenFileNameW = (void *)GetProcAddress(module, "GTKGetOpenFileNameW");
  }
  if(!pGTKGetOpenFileNameW){
      TRACE("YYYXXXRRR---> load system func GTKGetOpenFileNameW failedf!!\n");
      return FALSE;
  }   
  
  TRACE("(%p)(%p)(%x)\n",ofn,ofn->hwndOwner,ofn->hwndOwner);
  return pGTKGetOpenFileNameW(ofn);
}

 BOOL callSystemGetSaveFileNameW(LPOPENFILENAMEW ofn){
  if(!pGTKGetSaveFileNameW){
      HMODULE module = load_gtk_graphics_driver();
      pGTKGetSaveFileNameW = (void *)GetProcAddress(module, "GTKGetSaveFileNameW");
  }
  if(!pGTKGetSaveFileNameW){
      TRACE("YYYXXXRRR---> load system func GTKGetSaveFileNameW failedf!!\n");
      return FALSE;
  }      
  
  TRACE("(%p)(%x)\n",ofn->hwndOwner,ofn->hwndOwner);
  return pGTKGetSaveFileNameW(ofn);
}
#endif
/***********************************************************************
...
BOOL WINAPI GetOpenFileNameW(OPENFILENAMEW *ofn)
{
    ...
    BOOL bret=FALSE;
    TRACE("flags 0x%08x\n", ofn->Flags);
    //get current user select file Manager style
    if (!valid_struct_size( ofn->lStructSize ))
    {
        COMDLG32_SetCommDlgExtendedError( CDERR_STRUCTSIZE );
        return FALSE;
    ...
    else
    {
        ...
        TRACE("ccxxyy------select  style11 \n");
        file_style=get_FileManage_Style(ofn->Flags);
        switch(file_style)
        {
        	#ifdef GTK_DIALOG
            case FILESTYLE_GTK:
	    	  bret= Gtk_GetOpenFileNameW((void*)ofn);
			  TRACE("ccxxyy------select Linux style(%d) \n",bret);
			  return bret;
            case FILESTYLE_WINE:
			#endif
            default:
              TRACE("ccxxyy------use wine style\n");
              return GetFileDialog95(&info, OPEN_DIALOG); 
        }
    }
}
...
BOOL WINAPI GetSaveFileNameA(OPENFILENAMEA *ofn)
{
    ...
    else
    {
        ...
        if(FILESTYLE_GTK==get_FileManage_Style(ofn->Flags))
        {
			return 	 Gtk_GetSaveFileNameA(ofn);
        }
		/*************<zyh><2021-10-28> <add gtk funcs> end*********/
...
{  
    ...
    else
    {
        ...
        init_filedlg_infoW(ofn, &info);
	      TRACE("ccxxyy------select style \n");
        file_style=get_FileManage_Style(ofn->Flags);
        switch( file_style)
        {
        	#ifdef GTK_DIALOG
            case FILESTYLE_GTK:
			  return Gtk_GetSaveFileNameW((void*)ofn);
            case FILESTYLE_WINE:
			#endif
            default:
                TRACE("ccxxyy------use wine style\n");
