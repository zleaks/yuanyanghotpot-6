...
//<yangxinruo><20210125><for wechat browser use GPU accelerate> add begin
static void FBO_2_PPM_file(char *prefix,int id,int output_width,int output_height,BOOLEAN needRead,int buffer)
{
    FILE    *output_image;
    TRACE("YYYXXXRRR--->dupmy texture wxh=%dx%d\n",output_width,output_height);

    char fName[256];
    sprintf(fName, "_%s%d", prefix,id);
...
//<yangxinruo><20210125><for wechat browser use GPU accelerate> add end
     ...
     RECT *dst_rect)
{
    ...
    // if(src_location!=WINED3D_LOCATION_DRAWABLE){
    //     FBO_2_PPM_file("_acheck_",(context_gl&&context_gl->current_fbo)?context_gl->current_fbo->id:-1,dst_rect->right-dst_rect->left,dst_rect->bottom-dst_rect->top,false,NULL);
    // }
    ...
    // if(src_location!=WINED3D_LOCATION_DRAWABLE){
    //     FBO_2_PPM_file("_bsbis_",(context_gl&&context_gl->current_fbo)?context_gl->current_fbo->id:-1,dst_rect->right-dst_rect->left,dst_rect->bottom-dst_rect->top,true,buffer);
    // }
