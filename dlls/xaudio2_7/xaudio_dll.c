        ...
        const XAUDIO2_EFFECT_CHAIN *pEffectChain)
{
    XA2VoiceImpl *This = impl_from_IXAudio2SourceVoice(iface);
    HRESULT hr;
	//<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add begin
    int i;
    UINT32 defChannels = 1;
    XAUDIO2_VOICE_DETAILS *pVoiceDetails;

    pVoiceDetails = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(*pVoiceDetails));
    FAudioVoice_GetVoiceDetails(This->faudio_voice, (FAudioVoiceDetails *)pVoiceDetails);
    if(pVoiceDetails != NULL){
        TRACE("YYYXXXRRR---> XA2SRC_SetEffectChain get audio voice detail %u\n",pVoiceDetails->InputChannels);
        defChannels = pVoiceDetails->InputChannels;
    }
    HeapFree(GetProcessHeap(), 0, pVoiceDetails);
    //<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add end
    TRACE("%p, %p\n", This, pEffectChain);
	//<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add begin
    if(pEffectChain != NULL && pEffectChain->EffectCount > 0){
        TRACE("YYYXXXRRR---> XA2SRC_SetEffectChain with(ZERO) channel=%u count=%u\n",pEffectChain->pEffectDescriptors[0].OutputChannels,pEffectChain->EffectCount);
        // i = pEffectChain->EffectCount - 1;
        // TRACE("YYYXXXRRR---> XA2SRC_SetEffectChain channel=%u index=%u\n",pEffectChain->pEffectDescriptors[i].OutputChannels,i);
        // if(pEffectChain->pEffectDescriptors[i].OutputChannels == 0){
        //     TRACE("YYYXXXRRR---> XA2SRC_SetEffectChain force effect channed = 1 channel=%u\n",pEffectChain->pEffectDescriptors[i].OutputChannels);
        //     pEffectChain->pEffectDescriptors[i].OutputChannels = defChannels;
        // }

        for(i = 0; i < pEffectChain->EffectCount; ++i){
            TRACE("YYYXXXRRR---> XA2SRC_SetEffectChain channel=%u index=%u\n",pEffectChain->pEffectDescriptors[i].OutputChannels,i);
            if(pEffectChain->pEffectDescriptors[i].OutputChannels == 0){
                TRACE("YYYXXXRRR---> XA2SRC_SetEffectChain force effect channed = 1 channel=%u\n",pEffectChain->pEffectDescriptors[i].OutputChannels);
                pEffectChain->pEffectDescriptors[i].OutputChannels = defChannels;
        ...
        TRACE("YYYXXXRRR---> XA2SRC_SetEffectChain zero pEffectChain->EffectCount ");
    }
    //<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add end
    free_effect_chain(This->effect_chain);
    This->effect_chain = wrap_effect_chain(pEffectChain);
	//<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add begin
    TRACE("YYYXXXRRR---> XA2SRC_SetEffectChain before faudio_voice = %p ",This->faudio_voice);
    //<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add end
        ...
        const XAUDIO2_EFFECT_CHAIN *pEffectChain)
{
    ...
    //<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add begin
    if(pEffectChain != NULL && pEffectChain->EffectCount > 0){
        TRACE("YYYXXXRRR---> XA2SUB_SetEffectChain with(ZERO) channel=%u\n", pEffectChain->pEffectDescriptors[0].OutputChannels);
    }else{
        TRACE("YYYXXXRRR---> XA2SUB_SetEffectChain zero pEffectChain->EffectCount ");
    }
    //<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add end
    free_effect_chain(This->effect_chain);
    This->effect_chain = wrap_effect_chain(pEffectChain);
        ...
        const XAUDIO2_EFFECT_CHAIN *pEffectChain)
{
    ...
    //<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add begin
    if(pEffectChain != NULL && pEffectChain->EffectCount > 0){
        TRACE("YYYXXXRRR---> XA2M_SetEffectChain with(ZERO) channel=%u\n",pEffectChain->pEffectDescriptors[0].OutputChannels);
    }else{
        TRACE("YYYXXXRRR---> XA2M_SetEffectChain zero pEffectChain->EffectCount ");
    }
    //<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add end
    free_effect_chain(This->effect_chain);
    This->effect_chain = wrap_effect_chain(pEffectChain);
        ...
        const XAUDIO2_EFFECT_CHAIN *pEffectChain)
{
    ...
    //<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add begin
    if(pEffectChain != NULL && pEffectChain->EffectCount > 0){
        TRACE("YYYXXXRRR---> IXAudio2Impl_CreateSourceVoice with(ZERO) channel=%u\n", pEffectChain->pEffectDescriptors[0].OutputChannels);
    }else{
        TRACE("YYYXXXRRR---> IXAudio2Impl_CreateSourceVoice zero pEffectChain->EffectCount ");
    }
    //<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add end
        ...
        const XAUDIO2_EFFECT_CHAIN *pEffectChain)
{
    ...
    //<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add begin
    if(pEffectChain != NULL && pEffectChain->EffectCount > 0){
        TRACE("YYYXXXRRR---> IXAudio2Impl_CreateSubmixVoice with(ZERO) channel=%u\n", pEffectChain->pEffectDescriptors[0].OutputChannels);
    }else{
        TRACE("YYYXXXRRR---> IXAudio2Impl_CreateSubmixVoice zero pEffectChain->EffectCount ");
    }
    //<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add end
        ...
        AUDIO_STREAM_CATEGORY streamCategory)
{
    ...
    *ppMasteringVoice = &This->mst.IXAudio2MasteringVoice_iface;
    ...
    //<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add begin
    if(pEffectChain != NULL && pEffectChain->EffectCount > 0){
        TRACE("YYYXXXRRR---> IXAudio2Impl_CreateMasteringVoice with(ZERO) channel=%u\n", pEffectChain->pEffectDescriptors[0].OutputChannels);
    }else{
        TRACE("YYYXXXRRR---> IXAudio2Impl_CreateMasteringVoice zero pEffectChain->EffectCount ");
    }
    //<xinruo><2020-08-05><for ARMA3 support explosion sound effects crash> add end
