                                   ...
                                   UINT first, UINT last, BOOL remove )
{
    ...
    { 
        ...
        GetClassNameA(msg->hwnd, classname, 256);
        if (msg && msg->hwnd && 0 == strcmp(classname, "OWL.Dock"))
        {
            static int s_px;
            static int s_py;
            if (msg->hwnd == GetCapture())
            {
                ...
                else
                {
                    ...
                    MoveWindow(msg->hwnd, rc.left + (curPx - s_px), rc.top + (curPy - s_py), width, height, TRUE);
                    s_px = curPx;
                    s_py = curPy;
                }
            }
...
static BOOL send_message( struct send_message_info *info, DWORD_PTR *res_ptr, BOOL unicode )
{
    ...
    /**/
    if(GetParent(info->hwnd))
    {
        TopParent=GetAncestor(info->hwnd,3);
        if(TopParent && (!IsWindowEnabled(TopParent) || IsModel == TRUE))
        {
            if(info->msg == WM_ACTIVATE)
            {
                /*lose active*/
                if (LOWORD(info->wparam) == WA_INACTIVE)
                {
                    EnableWindow(TopParent,TRUE);
                    IsModel = TRUE;
                }
                else if(LOWORD(info->wparam) != WA_INACTIVE)/*active*/
                {
		    char classname[255];
		    GetWindowTextW(info->hwnd,classname,255);
		    if(!lstrcmpW(classname,L""))
		    {
		        return ret;
		    }
                    EnableWindow(TopParent,FALSE);
                    IsModel = FALSE;
                    if(!IsWindowEnabled(info->hwnd))
                    {
                        EnableWindow(info->hwnd,TRUE);
                    }
                }
...
LRESULT WINAPI SendMessageW( HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam )
{
	...
	if(!lstrcmpW(classname,L"TFoxMainFrm.UnicodeClass"))
	{
	    ERR("Foxmail.exe is killed!!\n");
	    system("taskkill /im Foxmail.exe /f");
	}
    }
#endif
    /*Foxmail关闭后进程不能及时杀死*/
...
//guotengfei　判断是否右键鼠标消息，解决excel2010右键菜单以及鼠标选中复制问题.
BOOL pm_IsRButtonDown=FALSE;
BOOL WINAPI DECLSPEC_HOTPATCH IsRButtonDown(void)
{
    return pm_IsRButtonDown;
}

//guotengfei 判断是否窗口正处于鼠标悬停状态
HWND g_HwndGetMouseHovering = 0;
HWND g_LastHwndGetMouseHovered = 0;
BOOL IsHwndMouseHovering(HWND hwnd)
{
    if(hwnd == g_HwndGetMouseHovering)
       return TRUE;
    return FALSE;
}
...
LRESULT WINAPI DECLSPEC_HOTPATCH DispatchMessageW( const MSG* msg )
{
    ...
    /*Haozip setcapture then can still get message WM_MOUSELEAVE.Then mouse can't be released.*/
    char classname[256];
    GetClassNameA(msg->hwnd,classname,256);
    if( msg->message == WM_MOUSELEAVE && !strcmp(classname,"RCHaoChildFrame"))
    {
        if( msg->hwnd == GetCapture())/*This is a time-consuming operation.It can be optimizabled.*/
        {
            ERR("catch mouseleave \n");
            return 0;
        }
    }
    /*Haozip setcapture then can still get message WM_MOUSELEAVE.Then mouse can't be released.*/
    ...
    if(msg->message == WM_RBUTTONDOWN )
        pm_IsRButtonDown = TRUE;
    if(msg->message == WM_LBUTTONDOWN )
        pm_IsRButtonDown = FALSE;

    if(msg->message == WM_MOUSEHOVER)
    {
        g_LastHwndGetMouseHovered = g_HwndGetMouseHovering;
        g_HwndGetMouseHovering = msg->hwnd;
    }
    if(msg->message == WM_MOUSELEAVE)
    {
        g_HwndGetMouseHovering = 0;
    }
#endif
