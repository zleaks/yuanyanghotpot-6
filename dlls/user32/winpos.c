 ...
 */
HWND WINPOS_WindowFromPoint_internal( HWND hwndScope, POINT pt, INT *hittest ,UINT uFlags)
{
    int i, res;
    HWND ret, *list;
    POINT win_pt;
    ...
    *hittest = HTNOWHERE;
    ...
    for (i = 0; list[i]; i++)
    {
        ...
        {
            LONG style = GetWindowLongW( list[i], GWL_STYLE );
            if ((uFlags & CWP_SKIPINVISIBLE) && !(style & WS_VISIBLE)){
                continue;
            }
            if ((uFlags & CWP_SKIPDISABLED) && (style & WS_DISABLED)){
                continue;
            }
        }
        if (uFlags & CWP_SKIPTRANSPARENT)
        {
            if (GetWindowLongW( list[i], GWL_EXSTYLE ) & WS_EX_TRANSPARENT){
                continue;
            }
        }
...
HWND WINPOS_WindowFromPoint( HWND hwndScope, POINT pt, INT *hittest){
    return WINPOS_WindowFromPoint_internal(hwndScope,pt,hittest,0);
}
...
HWND WINAPI WindowFromPoint_internal( POINT pt ,UINT uFlags)
{
    INT hittest;
    return WINPOS_WindowFromPoint_internal( 0, pt, &hittest,uFlags );
}
...
//<yangxinruo><20200826><for RTX talk window maximize to full screen and overlap input rect> add begin
...
//<yangxinruo><20200826><for RTX talk window maximize to full screen and overlap input rect> add end

//<yangxinruo><2020-09-27><for wechat minimum window size restriction not work> add begin
void internal_GetMinMaxInfo( HWND hwnd,LPMINMAXINFO MinMax ){
...
//<yangxinruo><2020-09-27><for wechat minimum window size restriction not work> add end
/*******************************************************************
...
MINMAXINFO WINPOS_GetMinMaxInfo( HWND hwnd )
{
    ...
    {
        RECT rc_work;
        //<yangxinruo><20200826><for RTX talk window maximize to full screen and overlap input rect> delete begin
        //MONITORINFO mon_info;
        //<yangxinruo><20200826><for RTX talk window maximize to full screen and overlap input rect> delete end
    ...
    //<yangxinruo><20200826><for RTX talk window maximize to full screen and overlap input rect> add begin
    char processnm[256];
    int fullWorkHeight=(mon_info.rcWork.bottom-mon_info.rcWork.top);
    GetModuleFileNameA(NULL,processnm,256);
    if(EndWith(processnm,"RTX.exe")){
        if((MinMax.ptMaxSize.y+MinMax.ptMaxPosition.y)>fullWorkHeight){
            MinMax.ptMaxSize.y=fullWorkHeight-MinMax.ptMaxPosition.y;
        }
        TRACE("YYYXXXRRR---> WINPOS_GetMinMaxInfo force #32770 window in monitor Y hwnd=%p %s %d\n", hwnd,processnm,fullWorkHeight);
    }
    //<yangxinruo><20200826><for RTX talk window maximize to full screen and overlap input rect> add end
...
static BOOL show_window( HWND hwnd, INT cmd )
{
	...
	case SW_SHOW:
            ...
            if (style & WS_CHILD) swp |= SWP_NOACTIVATE | SWP_NOZORDER;
#if defined __HOTPOT_OFFICE2010__
            /* 20220125 modify for mask activation window  start*/
            WCHAR titlenm[256] = {0};
            GetWindowTextW(hwnd, titlenm, 256);
            if(!strncmp(debugstr_w(titlenm), "L\"Microsoft Office \\6fc0\\6d3b\\5411\\5bfc\"", strlen("L\"Microsoft Office \\6fc0\\6d3b\\5411\\5bfc\"")))
                SendMessageW(hwnd, WM_CLOSE, 0, 0);
            /* 20220125 modify for mask activation window  end*/
#endif
	    break;
...
#ifdef __HOTPOT_XJCAD__
	int cx = GetSystemMetrics(SM_CXSCREEN);
	int cy = GetSystemMetrics(SM_CYSCREEN);

	if( cmd == SW_SHOWMAXIMIZED )
		SetWindowPos( hwnd , HWND_TOP , 0,0,cx,cy , swp );
#endif
    }
    if (cmd == SW_HIDE)
    {
...
BOOL WINAPI GetWindowPlacement( HWND hwnd, WINDOWPLACEMENT *wndpl )
{
    ...
    //<yangxinruo><20201029><for wechat restore button act as min (locate off the screen) After maximize> add begin
    if(pWnd){
        if(pWnd->normal_rect.left<=0&&pWnd->normal_rect.right<=0){
            TRACE("YYYXXXRRR---> GetWindowPlacement hwnd=%p force normal RECT X locate in screen\n",hwnd);
            LONG w=pWnd->normal_rect.right-pWnd->normal_rect.left;
            pWnd->normal_rect.left=0;
            pWnd->normal_rect.right=pWnd->normal_rect.left+w;
        }
        if(pWnd->normal_rect.top<=0&&pWnd->normal_rect.bottom<=0){
            TRACE("YYYXXXRRR---> GetWindowPlacement hwnd=%p force normal RECT Y locate in screen\n",hwnd);
            LONG h=pWnd->normal_rect.bottom-pWnd->normal_rect.top;
            pWnd->normal_rect.top=0;
            pWnd->normal_rect.bottom=pWnd->normal_rect.top+h;
        }
    }
    //<yangxinruo><20201029><for wechat restore button act as min (locate off the screen) After maximize> add end
    ...
    {
        //<yangxinruo><20201029><for wechat restore button act as min (locate off the screen) After maximize> modify begin
        // pWnd->normal_rect = pWnd->window_rect;
        if((pWnd->window_rect.left<0&&pWnd->window_rect.right<0)||(pWnd->window_rect.top<0&&pWnd->window_rect.bottom<0)){
            TRACE("YYYXXXRRR---> avoid to set notmal_rect off screen!! window_rect(for new normal_rect)=%s\n",wine_dbgstr_rect(&pWnd->window_rect));
        }else{
            pWnd->normal_rect = pWnd->window_rect;
        }
        //<yangxinruo><20201029><for wechat restore button act as min (locate off the screen) After maximize> modify end
    }
    ...
    *new_client_rect = *old_client_rect;
    ...
    if (!(pWinpos->flags & SWP_NOSIZE))
    {
        ...
        /*20210801 renzhengyi modify for autoCAD 浮动工具栏显示不正常 begin*/
            ...
            char classnm[256];
            GetClassNameA(pWinpos->hwnd, classnm, 256);
            if(strstr(classnm, "Afx:00400000:8:00")
                && (GetWindowLongW( pWinpos->hwnd, GWL_STYLE ) == 0x84c84000
                || GetWindowLongW( pWinpos->hwnd, GWL_STYLE ) == 0x94c84000)){
                new_window_rect->bottom = new_window_rect->top + pWinpos->cy + 20;
            }
            else{
...
/*start, add by zhuyi, 20210903，解决微信小程序主窗口背景覆盖内容问题*/
                if (swp_flags == 0x00001817 || swp_flags == 0x00000815)
                {
                    return ret;
                }
...
BOOL USER_SetWindowPos( WINDOWPOS * winpos, int parent_x, int parent_y )
{
    ...
    //<yangxinruo><20200826><for RTX ANR when window manager exec show desktop> add begin
    //0083 for minimaize
    if(winpos->flags == 0x8174){
		char classnm[256];
        // WCHAR wndnm[256];
		if(winpos->hwnd)
		{
            GetClassNameA(winpos->hwnd,classnm,256);
            // GetWindowTextW(winpos->hwnd,wndnm,256);
            // LONG style=GetWindowLongW( winpos->hwnd, GWL_STYLE );
            LONG exStyle=GetWindowLongW( winpos->hwnd, GWL_EXSTYLE );
            TRACE("YYYXXXRRR---> USER_SetWindowPos 0024 hwnd=%p winpos->flags=%04x classname=%s exstyle=%08x\n", winpos->hwnd,winpos->flags,classnm,exStyle);
            if(strcmp(classnm,"#32770") == 0&&(exStyle&WS_EX_CLIENTEDGE))
		    {
                 winpos->flags=(winpos->flags|SWP_NOSIZE);
                TRACE("YYYXXXRRR---> USER_SetWindowPos force return skip 0024 hwnd=%p winpos->flags=%08x\n", winpos->hwnd,winpos->flags);
                // goto done;
		    }
		}
    }
    //<yangxinruo><20200826><for RTX ANR when window manager exec show desktop> add end

#ifdef __HOTPOT_QQ__
    if(GetWindowLongW( winpos->hwnd, GWL_STYLE ) == 0x960c0000 || GetWindowLongW( winpos->hwnd, GWL_EXSTYLE ) == 0x80308){
        winpos->hwndInsertAfter = HWND_NOTOPMOST;
    }
#endif 

    /*start add by lidong 解决企业微信微文档无法转发问题*/
#ifdef __HOTPOT_WECOM__
    char classnm[256];
    GetClassNameA(winpos->hwnd,classnm,256);
	if(strcmp("PerryShadowWnd", classnm) == 0 && GetWindowLongW( winpos->hwnd, GWL_STYLE ) == 0x84880000 && GetWindowLongW( winpos->hwnd, GWL_EXSTYLE ) == 0x80800a8)
    {
        winpos->hwndInsertAfter = HWND_NOTOPMOST;
        winpos->flags |= SWP_NOZORDER;
    }
#endif
    /*end add by lidong 解决企业微信微文档无法转发问题*/
    ...
    /* Common operations */
    /*20210816 renzhengyi modify for catia 工作窗白屏、花屏问题的修复 begin*/
    #if defined (__HOTPOT_CATIAV5__) || defined (__HOTPOT_PHOTOSHOP7__)
    if((GetWindowLongW( winpos->hwnd, GWL_STYLE ) & 0xcf0000) == 0xcf0000) { //修复photoshop新建窗口关闭显示异常的问题
        RedrawWindow( winpos->hwnd, NULL, 0, RDW_INVALIDATE | RDW_ERASE );
    }
    #endif
    /*20210816 renzhengyi modify for catia 工作窗白屏、花屏问题的修复 end*/
    ...
    //<yangxinruo><20200826><for RTX ANR when window manager exec show desktop> modify begin
    //TRACE("\tstatus flags = %04x\n", winpos->flags & SWP_AGG_STATUSFLAGS);
    TRACE("\tstatus flags = %04x hwnd=%p orig_flags=%04x\n", winpos->flags & SWP_AGG_STATUSFLAGS,winpos->hwnd,orig_flags);
    //<yangxinruo><20200826><for RTX ANR when window manager exec show desktop> modify end    
                          ...
                          INT x, INT y, INT cx, INT cy, UINT flags )
{
    ...
    //<yangxinruo><for right click menu too wide> add begin
    char classnm[256];
    if(hwnd)
    {
        GetClassNameA(hwnd,classnm,256);
        if(strcmp(classnm,"CMenuWnd") == 0&&cx>200)
        {
            TRACE("MODIFY---> TOO WIDE FOR CMenuWnd ,DO NOT LET THIS PASS!!, hwnd %p\n",hwnd);
            SetLastError( ERROR_INVALID_PARAMETER );
            return FALSE;
        }
    }
    //<yangxinruo> add end
	//禁止微云检测升级,add by zhangyanhui 2021.01.19
#if defined __HOTPOT_WEIYUN__
	extern HWND wy_proxyHwnd;
	if (hwnd&&(hwnd == wy_proxyHwnd))
	{
		flags&=~(SWP_NOZORDER);
		hwndInsertAfter=0;
    ...
    if(strcmp(classnm,"#32770")==0)
    {
        WCHAR lv_WindowText[256];
        GetWindowTextW(hwnd,lv_WindowText,256);
        if(strcmp(lv_WindowText,L"新建")==0)
        {
            cx*=2;
            cy*=2;
        }
    ...
    //Fix acad window of AutoCAD.
    if (cy > 0 && cy < 100 && hwnd && (flags & SWP_NOACTIVATE)) {
        LONG targetStyle = WS_POPUP | WS_SYSMENU | WS_THICKFRAME;
        LONG style = GetWindowLongA(hwnd, GWL_STYLE);
        if ((style & targetStyle) == targetStyle) {
	    winpos.cy += cy/2;
	    FIXME("NFS: Add cy %d->%d, flags:0x%x, style:0x%x \n", cy, winpos.cy, flags, style);
	}
    }

    //<yangxinruo><20201104><for wxwork blue frame shadow window> modify begin
    //<yangxinruo><20200930><for BUGFIX popupshadow picture edit bar failed .etc> add begin
    if(hwnd)
    {
        if(strcmp(classnm,"DuiShadowWnd")==0)
        {
            winpos.flags|=SWP_HIDEWINDOW;
        }
#if defined __HOTPOT_ZYXPDF__
        if(strcmp(classnm,"PerryShadowWnd")==0)
        {
            TRACE("MODIFY---> no need for zhuanyixia PDF  PerryShadowWnd window,ignore it style=%08x\n",  winpos.flags);
            winpos.flags|=SWP_HIDEWINDOW;
 	}
#endif

#if defined __HOTPOT_WECOM__
        if(strcmp(classnm,"PerryShadowWnd")==0)
        {
            TRACE("MODIFY---> no need for WECOM PerryShadowWnd window,ignore it style=%08x\n",  winpos.flags);
            winpos.flags|=SWP_HIDEWINDOW;
        }
#endif
        //<yangxinruo><20201231><for wechat3.0 temp wechatapp loading splash never disapper> add begin
#if defined __HOTPOT_WECHAT__
        if(strcmp(classnm,"popupshadow") == 0)
        {
            TRACE("MODIFY---> no need for WECHAT popupshadow window,ignore it style=%08x\n",  winpos.flags);
            winpos.flags|=SWP_HIDEWINDOW;
        }

        if(strcmp(classnm,"Chrome_WidgetWin_0") == 0)
        {
            LONG kStyle = GetWindowLongW( hwnd, GWL_STYLE );
            TRACE("YYYXXXRRR---> MODIFY---> try to hide loading window,ignore it style=%08x hwnd=%p kStyle=%08x target=%08x\n",  winpos.flags,hwnd,kStyle,(WS_VISIBLE|WS_CLIPSIBLINGS|WS_CLIPCHILDREN|WS_SYSMENU));
            if(kStyle==(WS_VISIBLE|WS_CLIPSIBLINGS|WS_CLIPCHILDREN|WS_SYSMENU)){
                winpos.flags|=SWP_HIDEWINDOW;
            }
        }
#endif
        //<yangxinruo><20201231><for wechat3.0 temp wechatapp loading splash never disapper> add end
    }
    //<yangxinruo><20200930><for BUGFIX popupshadow picture edit bar failed .etc> add end
    //<yangxinruo><20201104><for wxwork blue frame shadow window> modify end

    map_dpi_winpos( &winpos );

    if (WIN_IsCurrentThread( hwnd ))
    {
        BOOL ret = USER_SetWindowPos( &winpos, 0, 0 );

#ifdef __HOTPOT_WEMEET__
        char clsname[MAX_PATH] = {0};

        GetClassNameA(hwnd, clsname, MAX_PATH);
        if (strstr(clsname, "TXGuiFoundation") && GetWindowLongW(hwnd, GWL_STYLE) == 0x96040000)
            PostMessageW(hwnd, WM_MOUSEMOVE, 0, MAKELPARAM(0, 0));
#endif
        return ret;
    }
...
void WINPOS_SysCommandSizeMove( HWND hwnd, WPARAM wParam )
{
    ...
    #if defined __HOTPOT_CATIAV5__
    if(style == 0x54cf0000 ){
        if(syscommand == SC_MOVE) RedrawWindow( hwnd, NULL, 0, RDW_INVALIDATE | RDW_ERASE );
    }
    #endif
    /*20210816 renzhengyi modify for catia 工作窗白屏、花屏问题的修复 end*/   
}
