				    ...
				    const char *postfix)
{
    ...
    if (mp) {
        UINT flags = mp->fType;
        //TRACE( "{ ID=0x%lx,rect=%u,%u", mp->wID,mp->rect.left,mp->rect.top);
        if ( mp->hSubMenu)
                               ...
                               MENUITEM *lpitem, BOOL menuBar, UINT odaction )
{
    ...
    //added by lijing gray full-screen menu in autocad begin
    if(lpitem->wID == 596)
	    lpitem->fState = MF_GRAYED;
    //added by lijing gray full-screen menu in autocad end
...
static BOOLEAN is_disable_menu(MENUITEM *item) {
    //For ACDSee 5.0 disable wallpaper menu 2021.06.01 YYY
    unsigned int *mtxt;
    mtxt = (unsigned int *)item->text;
    if(mtxt != NULL && ((item->wID == 0x100c6 && *mtxt == 0x7f6e8bbe)
            || (item->wID == 0x4eef && *mtxt == 0x539f8fd8)
            || (item->wID == 0x4eed && *mtxt == 0x4e2d5c45)
            || (item->wID == 0x4eee && *mtxt == 0x94fa5e73)) )
        return TRUE;

#ifdef __HOTPOT_ADOBEREADER10__			    
    if( item->wID==6168  || item->wID == 6150 || item->wID == 6062 || item->wID == 6148 )
        return TRUE;
#endif
		...
		DrawEdge (hdc, &rect, EDGE_RAISED, BF_RECT);
            ...
            if (menu)
            {
                ...
                if (menu->nItems)
                {
                    ...
                    {
                        if (is_disable_menu(item)) EnableMenuItem(hmenu, item->wID, MF_GRAYED | MF_DISABLED);
...
static INT MENU_ButtonUp( MTRACKER* pmt, HMENU hPtMenu, UINT wFlags)
{
        ...
        if (pos != NO_SELECTED_ITEM && (ptmenu->FocusedItem == pos))
	{
            ...
            MENUITEM * item = &ptmenu->items[pos];
	    // 判断是否是当前下拉菜单按下，如果窗口存在，就return
            if(strcmp("L\"\\5185\\5bb9...\\t?\"", debugstr_w(item->text)) == 0 || 
                strcmp("L\"\\641c\\5bfb...\"", debugstr_w(item->text)) == 0 ||
                strcmp("L\"&\\7d22\\5f15...\"", debugstr_w(item->text)) == 0 ||
                strcmp("L\"\\5feb\\901f\\6338\\89c8...\"", debugstr_w(item->text)) == 0 )
            {
                HWND hq = FindWindowA("HH Parent",NULL);
                if(IsWindow(hq)){
                    if(IsWindowVisible(hq))
                        return 0;
                    else 
                        ShowWindow(hq, SW_SHOW);
                }
            }
#endif
...
{
    //added for sourceinsight windows_focus 20210317
    SetFocus(hWnd); 
    //added end
    POPUPMENU *menu;
...
{
#if defined __HOTPOT_NOTEPAD__
    if(0x9cf1 == id) return FALSE;
#endif
    /*HaoZip3.1 need to delete item of menu*/
#if defined __HOTPOT_HAOZIP3__
    if( !strcmp( debugstr_w(str),"L\"2345\\770b\\56fe\\738b          \\tAlt+J\"") ||
        !strcmp( debugstr_w(str),"L\"\\7f29\\7565\\56fe(&H)\"") ||
        !strcmp( debugstr_w(str),"L\"\\53cc\\9762\\677f(&S) \\tF9\"") ||
	    !strcmp( debugstr_w(str),"L\"\\786c\\4ef6\\6027\\80fd\\8bc4\\6d4b(&G)  \\tAlt+G\"" ) )
    {
        return TRUE;
    }
#endif
    ...
    LPWSTR text1=L"添加到收藏夹...";
    LPWSTR text2=item->text;//菜单标题
    int m_res=lstrcmpW(text1,text2);//若m_res=0,则标题为添加到收藏夹...
    if(m_res == 0)
    {
	int m_res3 = EnableMenuItem(hMenu,0,MF_BYPOSITION | MF_GRAYED/*|MF_GRAYED*/);
        if(m_res3 == -1)
        {
                ERR("HMENU is not exists!\n");
        }
    }
#endif
    /**/
    ...
    /*set up menu Click invalid for yangyanpeng*/
#if defined __HOTPOT_SOURCEINSIGHT__
    static char classname[255];
    GetClassNameA(menu->hWnd,classname,255);
    if(!strcmp(classname,"si_Frame"))
    {
        menu->items[newpos].rect=menu->items[newpos+1].rect;
    }
#endif
    /*end to menu Click invalid for yangyanpeng*/
    release_menu_ptr(menu);
    return ret;
}
                         ...
                         UINT_PTR id, LPCWSTR str )
{
    ...
    //add by xdl
#if defined __HOTPOT_AUTOCAD2008__
    LPCWSTR text_paste_special = L"选择性粘贴(&S)...";
    LPCWSTR text_ole_object = L"OLE 对象(&O)...";
#endif
    //end
    ...
    //add by xdl
#if defined __HOTPOT_AUTOCAD2008__
    if (lstrcmpW(text_paste_special, str) == 0)
    {
        TRACE("str:%s, item_pos:%d\n", debugstr_w(str), item_pos);
	EnableMenuItem(hMenu, 11, MF_BYPOSITION | MF_GRAYED);
    }
    if (lstrcmpW(text_ole_object, str) == 0)
    {
        TRACE("str:%s, item_pos:%d\n", debugstr_w(str), item_pos);
	EnableMenuItem(hMenu, 13, MF_BYPOSITION | MF_GRAYED);
    }
#endif
    //end
