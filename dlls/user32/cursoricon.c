...
BOOL WINAPI DestroyIcon( HICON hIcon )
{
    ...
    int userProcessMsg = 0;
    SPY_GetUserProcessingMessage(&userProcessMsg);
    if(userProcessMsg == 6||userProcessMsg == 1026){
        TRACE_(icon)("YYYXXXRRR---> do not delete icon when wxwork app processing message(%d) icon=%p \n",userProcessMsg ,hIcon);
        return FALSE;
    }
#endif
