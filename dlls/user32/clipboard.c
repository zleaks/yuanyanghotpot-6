...
BOOL WINAPI EmptyClipboard(void)
{
    ...
    //wlz,chenchen,begin,2018/11
    /*HANDLE handle = GetClipboardData(CF_BITMAP);
    if(handle)
    {
	increase_sel_count(handle,-1);
    }*/
    //end
...
HANDLE WINAPI SetClipboardData( UINT format, HANDLE data )
{
    ...
    if (data)
    {
        ...
        if (format == CF_BITMAP)
        {
            BITMAP *bm = handle;
            if (size >= sizeof(*bm) &&
                size >= bm->bmWidthBytes * abs( bm->bmHeight ) &&
                !bm->bmBits)
            {
                bm->bmBits = bm + 1;
                cache->handle = CreateBitmapIndirect( bm );
            }
        }
    }
    ...
    //changed by lijing begin
    HWND hwnd = FindWindowA("WeChatMainWndForPC",NULL);
    if(hwnd != NULL)
        increase_sel_count(data,1);
    //end
...
BOOL WINAPI IsClipboardFormatAvailable( UINT format )
{
    ...
    //added by lijjing begin 
    HWND hwnd = FindWindowA("WeChatMainWndForPC",NULL);
    //<yangxinruo><for WECHAT copy html data from clipboard but can not parse it> add begin
    WCHAR buffer[256];
    WCHAR htmlFormatW[] = { 'H','T','M','L',' ','F','o','r','m','a','t',0 };
    if (GetClipboardFormatNameW( format, buffer, 256 )){
        if(lstrcmpW(buffer,htmlFormatW) == 0 ){
		if(hwnd != NULL)
       	        {
		    	TRACE("Do not let wechat use HTML Format %u \n",format);
		    	return FALSE;
		}
...
HANDLE WINAPI GetClipboardData( UINT format )
{
    ...
    for (;;)
    {
        ...
        //<yangxinruo><20201106><for wxwrok screenshot paste bug> add begin
#if defined __HOTPOT_WECOM__
        if(format==CF_BITMAP){
            cache=NULL;
        }
#endif
        //<yangxinruo><20201106><for wxwrok screenshot paste bug> add end
