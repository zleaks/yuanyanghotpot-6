...
//<yangxinruo><20201104><for wxwork systray icon not show> add begin
static int user_message_tls_index = TLS_OUT_OF_INDEXES;
//<yangxinruo><20201104><for wxwork systray icon not show> add end
#endif
...
#if defined __HOTPOT_WECOM__
    //<yangxinruo><20201104><for wxwork systray icon not show> add begin
    if (user_message_tls_index == TLS_OUT_OF_INDEXES)
    {
        DWORD index = TlsAlloc();
        if (InterlockedCompareExchange( &user_message_tls_index, index, TLS_OUT_OF_INDEXES ) != TLS_OUT_OF_INDEXES)
            TlsFree( index );
    }
    if(user_message_tls_index != TLS_OUT_OF_INDEXES && iFlag != SPY_DEFWNDPROC){
        char classnm[256];
        GetClassNameA(hWnd,classnm,256);
        if (strcmp(classnm,"WeWorkWindow") == 0) TlsSetValue(user_message_tls_index,msg);
    }
    //<yangxinruo><20201104><for wxwork systray icon not show> add end
#endif
...
#if defined __HOTPOT_WECOM__
    //<yangxinruo><20201104><for wxwork systray icon not show> add begin
    if(GetCurrentThreadId()==0x0024){
        if(user_message_tls_index != TLS_OUT_OF_INDEXES){
            TlsSetValue(user_message_tls_index,NULL);
        }
    }
    //<yangxinruo><20201104><for wxwork systray icon not show> add end
#endif
...
#if defined __HOTPOT_WECOM__
//<yangxinruo><20201104><for wxwork systray icon not show> add begin
void SPY_GetUserProcessingMessage(LPINT result){
    if(!result){
        return;
    }
    if(user_message_tls_index != TLS_OUT_OF_INDEXES){
        int ret= TlsGetValue(user_message_tls_index);
        *result=ret;
        if (ret != 0) TlsFree(user_message_tls_index);
    }
}
//<yangxinruo><20201104><for wxwork systray icon not show> add end
#endif