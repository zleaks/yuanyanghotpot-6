    ...
    case WM_CLOSE:
 	{
 	    ...
 	    LPSTR title = HeapAlloc(GetProcessHeap(), 0, 16);
 	    if(title){
 	    ...
 	    LONG style = GetWindowLongW(hwnd, GWL_STYLE);
             ...
             if(lstrcmpW(title, L"视图") == 0 && strncmp(className, "Afx:00400000:", 12) == 0 && (style & WS_POPUP && style & WS_SYSMENU))
 	    {
 			ShowWindow(hwnd, 0);
 	    } else {
 #endif
    ...
    case WM_GETMINMAXINFO:
        {
            if(hwnd){
                MINMAXINFO oldMMI;
                internal_GetMinMaxInfo(hwnd,&oldMMI);
                MINMAXINFO *lpMMI=(MINMAXINFO *)lParam;
                TRACE("YYYXXXRRR---> window(%p) want to limit window MIN size to %dx%d FROM %dx%d\n",hwnd,lpMMI->ptMinTrackSize.x,lpMMI->ptMinTrackSize.y,oldMMI.ptMinTrackSize.x,oldMMI.ptMinTrackSize.y);
                if(lpMMI->ptMinTrackSize.x!=oldMMI.ptMinTrackSize.x||lpMMI->ptMinTrackSize.y!=oldMMI.ptMinTrackSize.y){
                    oldMMI.ptMinTrackSize.x=lpMMI->ptMinTrackSize.x;
                    oldMMI.ptMinTrackSize.y=lpMMI->ptMinTrackSize.y;
                }else{
                    oldMMI.ptMinTrackSize.x=0;
                    oldMMI.ptMinTrackSize.y=0;
                }
                if(lpMMI->ptMaxTrackSize.x!=oldMMI.ptMaxTrackSize.x||lpMMI->ptMaxTrackSize.y!=oldMMI.ptMaxTrackSize.y){
                    oldMMI.ptMaxTrackSize.x=lpMMI->ptMaxTrackSize.x;
                    oldMMI.ptMaxTrackSize.y=lpMMI->ptMaxTrackSize.y;
                }else{
                    oldMMI.ptMaxTrackSize.x=0;
                    oldMMI.ptMaxTrackSize.y=0;
                }
                if(USER_Driver->pSetWindowSizeLimit){
                    USER_Driver->pSetWindowSizeLimit(hwnd,&oldMMI);
                }else{
                    TRACE("YYYXXXRRR---> no SetWindowSizeLimit func in current user driver\n");
                }
            }
        }
    ...
    LPARAM lParam )   /* [in] second message parameter */
{
    ...
    default:
        /* 20210720 renzy modify for 云视讯远程控制窗口不正常显示 begin*/
#if defined __HOTPOT_YUNSHIXUN__
        if (msg == WM_MOUSELEAVE || msg == WM_CLOSE) {
            GetClassNameA(hwnd, classnm, 256);
            if(strstr(classnm, "RCAssignWndClass")) {
                if(msg == WM_MOUSELEAVE) msg = WM_CLOSE;
                else if(msg == WM_CLOSE) break;
            }
        }
#endif
        /* 20210720 renzy modify for 云视讯远程控制窗口不正常显示 end*/
        result = DEFWND_DefWinProc( hwnd, msg, wParam, lParam );
        break;
    }
    SPY_ExitMessage( SPY_RESULT_DEFWND, hwnd, msg, result, wParam, lParam );
