...
static BOOL set_active_window( HWND hwnd, HWND *prev, BOOL mouse, BOOL focus )
{
...
#if defined (__HOTPOT_360BROWSER__) || defined (__HOTPOT_360SAFEBROWSER__)
    if(last_hwnd != NULL && previous != NULL &&  previous == last_hwnd && mouse == 0)
    {
	return TRUE;
    }
#endif
    //20211214 zhangyanhui 解决有道云笔记登录窗口输入用户名时，窗口激活异常问题
#if defined (__HOTPOT_YDYNOTE__)
     if(hwnd&&yn_login_hwnd != NULL &&  previous == yn_login_hwnd && mouse == 0) {
	TRACE("have got yn_login_hwnd!\n ");
	return TRUE;
     }
#endif
    ...
    char title_name[256];
    GetWindowTextA(hwnd, title_name, 256);
    if(strcmp(class_name, "SkinDialog") == 0 && strlen(title_name) == 0)
    {
        return TRUE;
    }
#endif
