...
void flush_window_surfaces( BOOL idle )
{
    ...
    /* 20210728 zhengyi add for catia project窗口频繁出现黑屏/白屏现象 --> */
#if defined __HOTPOT_CATIAV5__
    idle = 0;
#endif
    /* 20210728 zhengyi add for catia project窗口频繁出现黑屏/白屏现象 <-- */
...
#if defined __HOTPOT_QQ__
    /* renzy modify for kill qq risidul process*/
    if(GetWindowLongA(hwnd, GWL_STYLE) == 0X860C0000 && GetWindowLongA(hwnd, GWL_EXSTYLE) == 0x80308){
 ...
 */
//解决360浏览器窗口弹出后立刻消失的问题2021.11.9 nannan
#if defined (__HOTPOT_360BROWSER__) || defined (__HOTPOT_360SAFEBROWSER__)
HWND last_hwnd;
#endif
//20211214 zhangyanhui 解决有道云笔记登录窗口输入用户名时，窗口激活异常问题
#if defined (__HOTPOT_YDYNOTE__)
HWND yn_login_hwnd=NULL;
#endif
//激活微云登录窗口密码响应,add by zhangyanhui 2021.01.19
#if defined __HOTPOT_WEIYUN__
VOID CALLBACK enableLoginProc( 
...
{ 
		...
		HWND login_hwnd=GetParent(hwnd);
		if(!login_hwnd){
			ERR("login_hwnd null\n");
			return;
		}
		if((GetWindow(login_hwnd,GW_HWNDPREV)!=hwnd)){
			TRACE("exe SetWindowPos\n");
			SetWindowPos( hwnd, GetWindow(login_hwnd,GW_HWNDPREV), 0, 0, 0, 0,
...
HWND WIN_CreateWindowEx( CREATESTRUCTW *cs, LPCWSTR className, HINSTANCE module, BOOL unicode )
{
    ...
    /* add by yujun, 解决notepad++联想弹窗窗口位置覆盖文本编辑行字体问题*/
#if defined __HOTPOT_NOTEPADPLUS__
    char class_name[256] = {0};
    GetClassNameA(cs->hwndParent, class_name, 256);
    if (strcmp(class_name, "Scintilla") == 0)
    {
        cs->style &= ~(WS_THICKFRAME | WS_CAPTION);
    }
#endif
   /* end add by yujun, 解决notepad++联想弹窗窗口位置覆盖文本编辑行字体问题*/

	/*start, add by zhuyi, 20210615，解决creo3.0软件启动时无法最大化以及没有taskbar的问题*/
#if defined __HOTPOT_CREO3__
	char Lstr_Reference_point[256] = {"L\"\\57fa\\51c6\\70b9\""};
	char Lstr_Reference_aixs[256] = {"L\"\\57fa\\51c6\\8f74\""};
	char Lstr_Datum_plane[256] = {"L\"\\57fa\\51c6\\5e73\\9762\""};
	char Lstr_Coordinate_System[256] = {"L\"\\5750\\6807\\7cfb\""};
	char Lstr_Sketching[256] = {"L\"\\8349\\7ed8\""};
	char Lstr_File_open[256] = {"L\"\\6587\\4ef6\\6253\\5f00\""};
	char Lstr_Warning_message[256] = {"L\"\\8b66\\544a\\6d88\\606f\\5217\\8868\""};
	char Lstr_Search[256] = {"L\"\\68c0\\7d22\""};

	if ((cs->style == (WS_POPUP|WS_CLIPCHILDREN|WS_CAPTION|WS_SYSMENU|WS_THICKFRAME|WS_MINIMIZEBOX|WS_MAXIMIZEBOX)) 
		&& (cs->dwExStyle == WS_EX_APPWINDOW))
	{
		cs->style &= ~(WS_POPUP|WS_CLIPCHILDREN);
	}
	
	if ((cs->style == (WS_POPUP|WS_CLIPCHILDREN|WS_CAPTION|WS_SYSMENU|WS_THICKFRAME)) 
		&& (cs->dwExStyle == 0))
	{
		if (	(strncmp(Lstr_Reference_point, debugstr_w(cs->lpszName), strlen(Lstr_Reference_point)) == 0) 
			||	(strncmp(Lstr_Reference_aixs, debugstr_w(cs->lpszName), strlen(Lstr_Reference_aixs)) == 0) 
			||	(strncmp(Lstr_Datum_plane, debugstr_w(cs->lpszName), strlen(Lstr_Datum_plane)) == 0) 
			||	(strncmp(Lstr_Coordinate_System, debugstr_w(cs->lpszName), strlen(Lstr_Coordinate_System)) == 0) 
			||	(strncmp(Lstr_Sketching, debugstr_w(cs->lpszName), strlen(Lstr_Sketching)) == 0) 
			||	(strncmp(Lstr_File_open, debugstr_w(cs->lpszName), strlen(Lstr_File_open)) == 0) 
			||	(strncmp(Lstr_Warning_message, debugstr_w(cs->lpszName), strlen(Lstr_Warning_message)) == 0) 
			||	(strncmp(Lstr_Search, debugstr_w(cs->lpszName), strlen(Lstr_Search)) == 0) 
		)
		{
...
#endif
	/*end, add by zhuyi, 20210615，解决creo3.0软件启动时无法最大化以及没有taskbar的问题*/

     /*start add by lidong, 20220121,解决photoshop14全屏后卡死，界面异常问题*/
#if defined (__HOTPOT_PHOTOSHOP14__)
      if(strstr(debugstr_w(className),"OWL.ShadowView"))
	 {
           if((cs->style == WS_POPUP) && (cs->dwExStyle == WS_EX_LAYERED|WS_EX_NOACTIVATE))
             {
	      cs->dwExStyle &= ~WS_EX_LAYERED;
	     }
	 }
#endif
    /*end add by lidong, 20220121,解决photoshop14全屏后卡死，界面异常问题*/

#if defined (__HOTPOT_WECHAT__)
    /*start, add by zhuyi, 20210903，解决微信小程序主窗口背景覆盖内容问题*/
	//微信小程序主窗口
	if ( cs->style == (WS_CLIPSIBLINGS|WS_CLIPCHILDREN|WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX))
	{
		//cs->style = (WS_VISIBLE|WS_CAPTION|WS_SYSMENU|WS_THICKFRAME|WS_MINIMIZEBOX);
		//cs->dwExStyle = WS_EX_TOPMOST;
	}

	//微信小程序内容窗口
	if ( cs->style == (WS_CLIPSIBLINGS|WS_CLIPCHILDREN|WS_SYSMENU))
	{
    ...
    /*end, add by zhuyi, 20210903，解决微信小程序主窗口背景覆盖内容问题*/
#endif

    className = CLASS_GetVersionedName(className, NULL, NULL, TRUE);

    if(strncmp(debugstr_w(className),"L\"ImagePreviewWnd\"",18)==0||strncmp(debugstr_w(className),"L\"TalkRoomWnd\"",14)==0){
        TRACE("YYYXXXRRR---> force add style WS_OVERLAPPEDWINDOW for ImagePreviewWnd,TalkRoomWnd\n");
        cs->style|=WS_MINIMIZEBOX | WS_MAXIMIZEBOX;
    }

    //<yangxinruo><20210121><for wechat confirm dialog behind window> add begin
    if(strncmp(debugstr_w(className),"L\"ConfirmDialog\"",16)==0){
        TRACE("YYYXXXRRR---> force add style WS_POPUP for ConfirmDialog\n");
        // cs->style|=WS_POPUP;
        cs->dwExStyle|=WS_EX_TOPMOST;
    }
    //<yangxinruo><20210121><for wechat confirm dialog behind window> add end

#if defined __HOTPOT_PROJECT2007__
    if (!strcmp(debugstr_w(className), "L\"MS-SDIa\"")) {
        FIXME("MS-SDIa window, return\n");
        return 0;
    }
#endif

#if defined __HOTPOT_OFFICE2007__
    /*start, add by zhuyi, 20210927, 解决office2007多开文件时，任务栏切换导致的死循环，解决方法禁用任务栏切换*/
	if ((strcmp(debugstr_w(className), "L\"MS-SDIb\"") == 0) ||
        (strcmp(debugstr_w(className), "L\"MS-SDIa\"") == 0))
    {
         return 0;
    }
	/*end, add by zhuyi, 20210927, 解决office2007多开文件时，任务栏切换导致的死循环，解决方法禁用任务栏切换*/
#endif

#if defined __HOTPOT_CREO5__
    /* 20211203 renzy modify for creo5.0.2.0 窗口无法最大化、taskbar不显示，以及弹出窗口被覆盖的问题 start */
    char processnm[256];
    GetModuleFileNameA(NULL, processnm, 256);
    if (!strncmp(processnm, "C:\\Program Files\\PTC\\Creo 5.0.2.0\\Common Files\\x86e_win64\\obj\\xtop.exe", strlen(processnm)) )
    {
        if (cs->style == (WS_POPUP | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX) 
                && cs->dwExStyle == WS_EX_APPWINDOW )
        {
            cs->style &= ~WS_POPUP;//窗口无法最大化、taskbar不显示
        }
        else if(cs->style == (WS_POPUP | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME) 
                && cs->dwExStyle == WS_EX_APPWINDOW )
        {
            cs->dwExStyle |= WS_EX_TOPMOST;//弹出窗口被覆盖
        }
    }
    /* 20211203 renzy modify for creo5.0.2.0 窗口无法最大化、taskbar不显示，以及弹出窗口被覆盖的问题 end */
#endif

#if defined __HOTPOT_CREO2__
    /* 20211203 lidong modify for creo2.0 窗口无法最大化、taskbar不显示，以及弹出窗口被覆盖的问题 start */ 
    char processnm[256];
    GetModuleFileNameA(NULL, processnm, 256);
    if (!strncmp(processnm, "C:\\Program Files\\PTC\\Creo 2.0\\Common Files\\M100\\i486_nt\\obj\\xtop.exe", strlen(processnm)) )
    {
        if (cs->style == (WS_POPUP | WS_CLIPCHILDREN | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX) && cs->dwExStyle == WS_EX_APPWINDOW) 
        {
            cs->style &= ~WS_POPUP;//窗口无法最大化、taskbar不显示
        }       
        if (cs->style == (WS_POPUP | WS_CLIPCHILDREN | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME)) 
    ...
    /* 20211203 lidong modify for creo2.0 窗口无法最大化、taskbar不显示，以及弹出窗口被覆盖的问题 end */
#endif

#ifdef __HOTPOT_FINEREADER15__
    /* 20211210 renzy hide for internal error popup of fineReader15 start */
    if(cs->style == 0x80c801c4 && cs->dwExStyle == (WS_EX_DLGMODALFRAME | WS_EX_CONTROLPARENT))
        return FALSE;
    /* 20211210 renzy hide for internal error popup of fineReader15 end */
#endif

#ifdef __HOTPOT_ZSZY__

    if(cs->style == 0x80000044 && cs->x == 3 && cs->y == 22)
    {
    	    return FALSE;
    }
...
#if defined __HOTPOT_OFFICE201X__
    /* 20211105 renzhengyi modify for office word 同义词界面卡死 start*/
    if(strstr(debugstr_w(className), "AS_WindowTransition"))
        return 0;
    /* 20211105 renzhengyi modify for office word 同义词界面卡死 end*/
#endif

    //add by xdl 修复拖动dwg文件到应用界面卡死问题
#if defined __HOTPOT_CAXA__
    //if(lstrcmpW(cs->lpszName, L"指定形文件") == 0)
    if(strcmp(debugstr_w(cs->lpszName), "L\"\\6307\\5b9a\\5f62\\6587\\4ef6\"") == 0)
    {
        FIXME("create shape file window stub\n");
	return 0;
    }
#endif
    //end
#if defined __HOTPOT_CAXA__
    /* 20220119 renzy modify for the shrotcut menu is invalid after pressing the S key start*/
    if(strstr(debugstr_w(className), "BCGPVisualCtrl")){
        cs->dwExStyle &= ~WS_EX_LAYERED;
    }
    /* 20220119 renzy modify for the shrotcut menu is invalid after pressing the S key end*/
#endif

#if defined __HOTPOT_PHOTOSHOPCC2018__
    if (unicode && cs->style == 0x80000000 && cs->dwExStyle == 0x8080000)
    {
      if (0 == strcmp(debugstr_w(p), "L\"OWL.PlatformUserFeedbackWindow\""))
        {  
            return 0;
        }
    }
    ...
    //add by xdl ZY环境对话框下方按钮不展示
#if defined __HOTPOT_CAXA__ || defined __HOTPOT_CARSIM8__
    //if(lstrcmpW(cs->lpszName, L"选择文件夹") == 0 && cx != 0 && cy !=0)
    if(strcmp(debugstr_w(cs->lpszName), "L\"\\9009\\62e9\\6587\\4ef6\\5939\"") == 0 && cx != 0 && cy !=0)
    {
        cx += 30;
	cy += 30;
    }
    ...
    /*20210801 renzhengyi modify for autoCAD 浮动工具栏显示不正常 begin*/
    if(strstr(debugstr_w(className), "Afx:00400000:8:00") && (GetWindowLongW( hwnd, GWL_STYLE ) == 0x840c4000)){
        char processnm[256];
        GetModuleFileNameA(NULL, processnm, 256);
        if(strstr(processnm, "acad.exe")){
            WIN_SetStyle( hwnd, WS_CAPTION, WS_THICKFRAME);
            SetWindowLongW( hwnd, GWL_EXSTYLE, WS_EX_TOOLWINDOW);
        }
    }
    /*20210801 renzhengyi modify for autoCAD 浮动工具栏显示不正常 end*/

#if defined __HOTPOT_CAXA__
    /* 20210927 renzhengyi modify for caxa_capp Title Block overlap start*/
    if(cs->style == 0xcf8000) WIN_SetStyle( hwnd, 0, WS_MAXIMIZE);
    /* 20210927 renzhengyi modify for caxa_capp Title Block overlap end*/
#endif
    ...
    //add by xdl caxa cad修改任务栏图标
#if defined __HOTPOT_CAXA__
    //if(lstrcmpW(cs->lpszName, L"选择文件夹") == 0)
    if(strcmp(debugstr_w(cs->lpszName), "L\"\\9009\\62e9\\6587\\4ef6\\5939\"") == 0)
    {
        const int OIC_CAXACAD = 32519;
	HICON hIcon = LoadIconW(0, MAKEINTRESOURCEW(OIC_CAXACAD));
...
//解决360浏览器窗口弹出后立刻消失的问题2021.11.9 nannan
    ...
    if( 0 == strcmp(class_name,"Chrome_WidgetWin_2") )
    {
    	last_hwnd  = hwnd;
    }
#endif
//20211214 zhangyanhui 解决有道云笔记登录窗口输入用户名时，窗口激活异常问题
    ...
    if(!strcmp(class_name,"LoginForm") ) {
	 yn_login_hwnd=hwnd;
         TRACE("yn_login_hwnd %p\n", yn_login_hwnd);
    }
#endif
		//激活微云登录窗口密码响应及禁止微云检测升级,add by zhangyanhui 2021.01.19
#if defined __HOTPOT_WEIYUN__
	static int rightDown_MainMenu_cx=0;
	if((GetWindowLongW(hwnd,GWL_STYLE)==0x86000000)&&(GetWindowLongW(hwnd,GWL_EXSTYLE)==0x08000008)&&!(GetParent(hwnd))){
			if(cs->cx<cs->cy){
				rightDown_MainMenu_cx=cs->cx;
			}
			else if(cs->cx>rightDown_MainMenu_cx){
				SetPropA(hwnd,"checkupgradewin",0xff);
				TRACE("has catch is_upgrade_win\n"); 
			}
	}
	if((GetWindowLongW(hwnd,GWL_STYLE)==0x84000000)&&(GetWindowLongW(hwnd,GWL_EXSTYLE)==0x00080088)&&(GetParent(hwnd))){
		UINT_PTR uResult = SetTimer(hwnd,
		7777,			 
		0,					  
		(TIMERPROC) enableLoginProc);
		TRACE("has catch wy_PWHwnd\n"); 
	}
#endif
...
{
    CREATESTRUCTW cs;
    WCHAR wechats1[]={'E','m','o','t','i','o','n','T','i','p','W','n','d',0};
    if(className && lstrcmpA(debugstr_w(className),debugstr_w(wechats1)) == 0 && x == 0 && y == 0 && width == 0 && height == 0)
    {
        TRACE("find wechat special window:%s(0,0,0x0),set invisible\n",debugstr_w(className));
        style = 0x80000000;
    }
...
/*lidong add for 160压缩选择存储路径后确定按钮无效 20211108*/
#if defined __HOTPOT_160ZIPTOOL__
    {
        char classnm[256];
        GetClassNameA(hwnd,classnm,256);
        if(strcmp(classnm,"Button") == 0 && GetParent(GetFocus()) == GetParent(hwnd)) return FALSE;
    }
#endif
/*lidong add for 160压缩选择存储路径后确定按钮无效 20211108*/
 ...
 */
//调整微云代理服务器设置窗口位置,add by zhangyanhui 2021.01.19 
#if defined __HOTPOT_WEIYUN__
	 HWND wy_proxyHwnd=NULL;
#endif
...
BOOL WINAPI DECLSPEC_HOTPATCH SetWindowTextW( HWND hwnd, LPCWSTR lpString )
{
	...
	//调整微云代理服务器设置窗口位置,add by zhangyanhui 2021.01.19 
#if defined __HOTPOT_WEIYUN__
	if(!strcmp(debugstr_w(lpString),"L\"\\4ee3\\7406\\8bbe\\7f6e\"")){
		  wy_proxyHwnd=hwnd;
		  TRACE("has catch proxy,(%p)\n",wy_proxyHwnd);
	}
#endif
