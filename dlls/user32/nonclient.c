...
static void  NC_DoNCPaint( HWND  hwnd, HRGN  clip )
{
    ...
    if(dwStyle == 0x54cf0000 || dwStyle == 0x55cf0000) {
...
LRESULT NC_HandleSysCommand( HWND hwnd, WPARAM wParam, LPARAM lParam )
{
    ...
    case SC_MINIMIZE:
        ...
        if(hwnd)
        {
            GetClassNameA(hwnd,classnmShadow,256);
            if(strcmp(classnmShadow,"TalkRoomLayerWnd") == 0)
            {
                HWND voipParent = FindWindowA("TalkRoomWnd",NULL);
                if(voipParent){
                    TRACE("YYYXXXRRR--->TalkRoomLayerWnd handle SC_MINIMIZE we may intercept this hwnd=%p to voipParent=%p\n",hwnd,voipParent);
                    return NC_HandleSysCommand(voipParent,wParam,lParam);
                }
            }
        }
    ...
    case SC_MAXIMIZE:
        ...
        if(hwnd)
        {
            ...
            if(strcmp(classnmShadow,"TalkRoomLayerWnd") == 0)
            {
                HWND voipParent = FindWindowA("TalkRoomWnd",NULL);
                if(voipParent){
                    TRACE("YYYXXXRRR--->TalkRoomLayerWnd handle SC_MAXIMIZE we may intercept this hwnd=%p to voipParent=%p\n",hwnd,voipParent);
                    return NC_HandleSysCommand(voipParent,wParam,lParam);
                }
            }
        }
    ...
    case SC_RESTORE:
        ...
        if(hwnd)
        {
            ...
            if(strcmp(classnmShadow,"TalkRoomLayerWnd") == 0)
            {
                HWND voipParent = FindWindowA("TalkRoomWnd",NULL);
                if(voipParent){
                    TRACE("YYYXXXRRR--->TalkRoomLayerWnd handle SC_RESTORE we may intercept this hwnd=%p to voipParent=%p\n",hwnd,voipParent);
                    return NC_HandleSysCommand(voipParent,wParam,lParam);
                }
            }
        }
