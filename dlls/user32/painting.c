...
static void update_visible_region( struct dce *dce )
{
    ...
    {
#ifdef __HOTPOT_TENCENTTIM__
	RECT tim_win;
	char classname[260]={0};
	GetClassNameA(dce->hwnd,classname,260);
	tim_win = get_virtual_screen_rect();
	int hight = win_rect.bottom - win_rect.top;
	int wight = win_rect.right  - win_rect.left;

	if( 725 == width && 548 == hight && 0 == strcmp(classname ,"TXGuiFoundation"))
	{
    	    __wine_set_visible_region( dce->hdc, vis_rgn, &win_rect, &top_rect, 0 );
	}
#endif
