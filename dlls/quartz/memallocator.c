...
//<yangxinruo><20210401><for womeeting no video preview> add begin
...
//<yangxinruo><20210401><for womeeting no video preview> add end
...
static HRESULT WINAPI BaseMemAllocator_Commit(IMemAllocator * iface)
{
    ...
    EnterCriticalSection(This->pCritSect);
    {
        ...
        if(EndWith(processnm,"ViLin.exe")){
            void* callstack[3];
            int i, frames = backtrace(callstack, 3);
            char** stackstrs = backtrace_symbols(callstack, frames);
            if(frames>1){
                if(strncmp(stackstrs[1],"[0x",3)==0){
                    TRACE("YYYXXXRRR---> call request from external strs[1]=%s strs[0]=%s\n",stackstrs[1],stackstrs[0]);
                    isInternal=FALSE;
                }
            }
            free(stackstrs);
        ...
        if (!isInternal) {
            TRACE("YYYXXRRR---> call request from external abort commit\n");
            hr = S_OK;
        } else
        //<yangxinruo><20210401><for womeeting no video preview> add end
