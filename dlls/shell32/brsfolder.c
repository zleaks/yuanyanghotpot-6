...
static void BrsFolder_CheckValidSelection( browse_info *info, LPTV_ITEMDATA lptvid )
{
    ...
    {
//add by xdl 格式工厂浏览文件夹对话框无法点击确定按钮
				          ...
				          LPARAM lParam )
{
    ...
    case BFFM_ENABLEOK:
        TRACE("Enable %ld\n", lParam);
	//liuyanhui:patch for mtxx
        // EnableWindow(GetDlgItem(hWnd, 1), lParam != 0);
        if(FindWindowA("MTXXMainWnd",NULL) != NULL)
       		EnableWindow(GetDlgItem(hWnd, 1), TRUE /*lParam != 0*/);
       	else
       		EnableWindow(GetDlgItem(hWnd, 1), lParam != 0);
       	//patch for mtxx end
        break;
...
static LPITEMIDLIST (CDECL *pGTKSHBrowseForFolderW)(LPBROWSEINFOW lpbi,LPCWSTR path) = NULL;
static LPITEMIDLIST callSystemSHBrowseForFolderW(LPBROWSEINFOW lpbi){
        //<yangxinruo><20210220><for wechat ungrabpointer before call GTK file dialog> add begin
        LPITEMIDLIST lpid;
        if(!pGTKSHBrowseForFolderW){
            HMODULE module = load_graphics_driver();
            pGTKSHBrowseForFolderW = (void *)GetProcAddress(module, "GTKSHBrowseForFolderW");
        }
        if(!pGTKSHBrowseForFolderW){
            TRACE("YYYXXXRRR---> load system func GTKSHBrowseForFolderW failedf!!\n");
            return NULL;
        }
        WCHAR folderPathW[1024];
        pGTKSHBrowseForFolderW(lpbi,folderPathW);
        TRACE("YYYXXXRRR---> get a shell folder path = %s\n",debugstr_w(folderPathW));
...
LPITEMIDLIST WINAPI SHBrowseForFolderW (LPBROWSEINFOW lpbi)
{
    ...
    {
        case FILESTYLE_GTK:
            return  callSystemSHBrowseForFolderW(lpbi);
        case FILESTYLE_WINE:
        default:
        {
            ...
            TRACE("cccxxxyyy--------------------- %s %s\n",debugstr_w(templateName), debugstr_w(lpbi->pszDisplayName));
...
//    TRACE("cccxxxyyyW------------- %s %s\n",debugstr_w(lpbi->pszDisplayName),debugstr_w(lpbi->lpszTitle));
    }
}
