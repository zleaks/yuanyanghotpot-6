...
//find the qq txupd file

static int search_txupd(char*file_path,char*qq_str)
{	
	int size_t=strlen(qq_str),size_s=strlen(file_path),res,i,j;
	res=0;
	char *tmp_pos=NULL;
...
//<yangxinruo><20200201><for wechat redirect request toward explorer to default approch(nemo) > add begin
static BOOL openPathByNemo(LPCWSTR filePath){
    char *unix_path,*unix_cmd;
    char nemo[] = "nemo";
    int cmdlen,syscmdlen,ret;
    unix_path = wine_get_unix_file_name(filePath);
    if(unix_path != NULL)
    {
...
/*File manager Style, 2: IE style  1:Linux Style*/
static int FILESTYLE =-1;
#define FILESTYLE_GTK 1
#define FILESTYLE_WINE 2

static void get_FileManage_Style()
{
	static const WCHAR szKeyStyle[] = {'S','o','f','t','w','a','r','e','\\','W','i','n','e','\\','F','i','l','e','S','t','y','l','e',0}; 
	int res;
	HKEY style_key;
	DWORD styleID = FILESTYLE_WINE;
	...
	if(style_key)
	{
        ...
        if((res = RegQueryValueExW(style_key, key_name, NULL, NULL,(BYTE *)&styleID, &data_size))){
            styleID = FILESTYLE_WINE;
        }
	}
	FILESTYLE = styleID;
}
//<yangxinruo><20200201><for wechat redirect request toward explorer to default approch(nemo) > add end
/*************************************************************************
...
static BOOL SHELL_execute( LPSHELLEXECUTEINFOW sei, SHELL_ExecuteW32 execfunc )
{
    ...
    sei_tmp = *sei;
    //<yangxinruo><20200201><for wechat redirect request toward explorer to default approch(OpenByLinux.exe) > add begin
    if(FILESTYLE == -1)
    {
    	get_FileManage_Style();
    }

    static const WCHAR nameExp[]={'e','x','p','l','o','r','e','r','.','e','x','e',0};
    static const WCHAR nameExpN[]={'e','x','p','l','o','r','e','r',0};
    static const WCHAR nameSel[]={'/','s','e','l','e','c','t',',',0};
    static const WCHAR nameCd[]={'/','c','d',',',0};
    static const WCHAR nameSelDel[]={',',0};
    static const WCHAR nameSelSpl[]={'\"',0};
    if(sei_tmp.lpFile){
        if(strcmpW(sei_tmp.lpFile,nameExp)==0||strcmpW(sei_tmp.lpFile,nameExpN)==0){
            //char processnm[256];
            //GetModuleFileNameA(NULL,processnm,256);
            //if(FindWindowA("WeChatMainWndForPC",NULL)||EndWith(processnm,"WeChat.exe")||EndWith(processnm,"QQ.exe")){
                WCHAR *availCmd=NULL;
                if(availCmd=StrStrW(sei_tmp.lpParameters,nameSel)){
                    WCHAR *onePath=StrStrW(availCmd,nameSelDel);
                    if(onePath){
                        onePath+=2;
                        WCHAR *pSpl=StrRChrW(onePath,NULL,L'\"');
                        if(pSpl){
                            *pSpl=0;
                        }
                        if(FILESTYLE == FILESTYLE_GTK){
                            return openPathByNemo(onePath);
                        }
                    }
                }else if(availCmd=StrStrW(sei_tmp.lpParameters,nameCd)){
                    WCHAR *onePath=StrStrW(availCmd,nameSelDel);
                    if(onePath){
                        onePath+=1;
                        if(FILESTYLE == FILESTYLE_GTK){
    ...
    //<yangxinruo><20200201><for wechat redirect request toward explorer to default approch(OpenByLinux.exe) > add end

    TRACE("we begin to find the txupd file %s\n",sei_tmp.lpFile);
    int res_find=search_txupd(debugstr_w(sei_tmp.lpFile),"txupd")+search_txupd(debugstr_w(sei_tmp.lpDirectory),"txupd");
 
    if(res_find>0){//find the txupd file
	return 0;
	...
	*wszDir = '\0';
    ...
    retval = SHELL_quote_and_execute( wcmd, wszParameters, wszEmpty,
                                      ...
                                      sei, execfunc );
    //<yangxinruo><20210206><for qq can not open some file with extension end by 'x'> add begin
    WCHAR *extensionPP = strrchrW(wszApplicationName, '.');
    if(extensionPP){
        extensionPP++;
        if(extensionPP){
            static const WCHAR wlDocx[]={'d','o','c','x'};
            static const WCHAR wlXlsx[]={'x','l','s','x'};
            static const WCHAR wlPptx[]={'p','p','t','x'};
            if(strncmpW(extensionPP,wlDocx,4)==0
                ||strncmpW(extensionPP,wlXlsx,4)==0
                ||strncmpW(extensionPP,wlPptx,4)==0){
                TRACE("YYYXXXRRR---> we should recheck ext here !! ori ret=%d ext=%s\n",retval,debugstr_w(extensionPP));
                if(retval>32){
                    retval=11;
                }
            }
    ...
    //<yangxinruo><20210206><for qq can not open some file with extension end by 'x'> add end
    if (retval > 32) {
        heap_free(wszApplicationName);
        if (wszParameters != parametersBuffer)
            heap_free(wszParameters);
    ...
    if (retval > 32)  /* Found */
    {
        TRACE("YYYXXXRRR---> to call SHELL_quote_and_execute #0\n");
    ...
    else if (PathIsDirectoryW(lpFile))
    {
        ...
        if (lpQuotedFile)
        {
            ...
            if (retval > 32)
            {
                ...
                //<yangxinruo><20210207><for qq setting open NEMO> add begin
                //char processnm[256];
                //GetModuleFileNameA(NULL,processnm,256);
                // if(EndWith(processnm,"QQ.exe")){
                if(FILESTYLE == FILESTYLE_GTK){
                    return openPathByNemo(lpFile);
                }
                //<yangxinruo><20210207><for qq setting open NEMO> add end
    ...
    if(retval < 33 && retval != ERROR_FILE_NOT_FOUND && retval != ERROR_PATH_NOT_FOUND
            && retval != SE_ERR_ACCESSDENIED && retval != SE_ERR_DLLNOTFOUND
            && retval != SE_ERR_FNF && retval != SE_ERR_OOM
            && retval != SE_ERR_PNF && retval != SE_ERR_SHARE)
    {
        WCHAR verb[] = {'o','p','e','n',0};
        if(lstrcmpW(verb,sei->lpVerb) == 0)
        {
            char *unix_path,*unix_cmd;
            char xdg[] = "xdg-open";
            int cmdlen,syscmdlen,ret;
            unix_path = wine_get_unix_file_name(sei->lpFile);
            if(unix_path != NULL)
            {
                cmdlen = lstrlenA(xdg);
                syscmdlen = cmdlen+1+1+lstrlenA(unix_path)+1+1;//cmd "unix_path"\0
                ...
                *(unix_cmd+syscmdlen-1)=0;
                ...
                if(ret != -1 && ret != 127)
                {
                    ...
                    retval = 33;
                }else if(FILESTYLE == FILESTYLE_GTK){
                    TRACE("system exec failed,try show in NEMO\n");
                    openPathByNemo(unix_path);
                }
                HeapFree(GetProcessHeap(),0,unix_cmd);
            }
        }
    ...
    // TODO yujun 解决360安全浏览器双击打开html文件dde报错问题
    // 修改注册表不启用dde后注册表项被加回，此处特殊处理
#if defined (__HOTPOT_360SAFEBROWSER__)
    WCHAR msg[2048];
    int error_code=GetLastError();
 ...
 */
#define HINSTANCE_OK (HINSTANCE)33
...
HINSTANCE OpenLinuxFileOrFloder(LPCWSTR lpFile)
{
    if( !PathIsDirectoryW(lpFile) && !_isFileExists(lpFile) )//check the path
    {
        ERR("lpFile is not a dir or file !\n");
        return ERROR_PATH_NOT_FOUND;//pathname error
    }
    char *unix_file_name;
    unix_file_name = wine_get_unix_file_name(lpFile);
    pid_t pid = vfork();
    if( pid < 0)
    {
        ERR( "fork error\n");
    ...
    else if(pid == 0)
    {
        ...
        strcat(unix_file_name,"\"");
        strcat(command,unix_file_name);
        execl( "/usr/bin/flock","flock","-x","-n","/tmp","-c",command,NULL);
    }
    return HINSTANCE_OK;
}
...
{
    /*Haozip3.1 open floder error*/
#if defined __HOTPOT_HAOZIP3__
    if( lpVerb && !lpParameters && strstrW(lpVerb,L"open") )//only open a file or dir not a program
        return OpenLinuxFileOrFloder(lpFile);
#endif
    /*Haozip3.1 open floder error*/
