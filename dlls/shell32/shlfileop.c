...
int WINAPI SHCreateDirectoryExW(HWND hWnd, LPCWSTR path, LPSECURITY_ATTRIBUTES sec)
{
    ...
    if(strlenW(path)>13){
        endOfPath=path;
        endOfPath+=(strlenW(path)-13);
        if(strncmp(debugstr_w(endOfPath),"L\"WeChat\\\\patch\\\\\"",18)==0){
            TRACE("YYYXXXRRR--->skip create path for wechat path =%s\n",(debugstr_w(endOfPath)));
...
static HRESULT parse_file_list(FILE_LIST *flList, LPCWSTR szFiles)
{
    ...
    /*Haozip3.1 rename error*/
#if defined __HOTPOT_HAOZIP3__
    WCHAR parent_path_file[256];
    lstrcpyW(parent_path_file,szFiles);
    PathRemoveFileSpecW(parent_path_file);
#endif
    /*Haozip3.1 rename error*/
    while (*ptr)
    {
        ...
        /*Haozip3.1 rename error*/
#if defined __HOTPOT_HAOZIP3__
        if( !PathIsDirectoryW(ptr) && !_isFileExists(ptr) )//check the path
        {
        ...
        {
            WCHAR parent_path_ptr[256];
            lstrcpyW(parent_path_ptr,ptr);
            PathRemoveFileSpecW(parent_path_ptr);
            if( lstrcmpW(parent_path_file,parent_path_ptr))
            {
                ERR("lpFile is not in same floder !\n");
                break;
            }
        }
#endif
