	...
	UINT lLength)
{
	...
	UINT i = 0;
	DROPFILES *lpDropFileStruct = GlobalLock(hDrop);
    char processnm[256]={0};
    char check_path[256]={"C:\\Program Files\\foxmail7.2\\Foxmail.exe"};

     /*start, add by zhuyi, 20210722，解决foxmail因为拖拽而导致的死锁问题，解决方案禁止拖拽*/
    GetModuleFileNameA(NULL,processnm,256);
    if(strncmp(check_path,processnm,strlen(check_path)) == 0)
    {
        TRACE("[zhuyi]====> Prohibit foxmail from using the drag and drop function\n");
        return 0;
    }
    /*end, add by zhuyi, 20210722，解决foxmail因为拖拽而导致的死锁问题，解决方案禁止拖拽*/
