...
static HANDLE import_gnome_cp_files( Atom type, const void *data, size_t size );
...
//<yangxinruo><20201028><for wechat copy file to desktop> add begin
static BOOL export_gnome_cp_files( Display *display, Window win, Atom prop, Atom target, HANDLE handle );
//<yangxinruo><20201028><for wechat copy file to desktop> add end
...
} builtin_formats[] =
{
    ...
    //<yangxinruo><20201028><for wechat copy file to desktop> add begin
    { 0, CF_HDROP,           XATOM_x_special_gnome_copied_files,       import_gnome_cp_files, export_gnome_cp_files },
    //<yangxinruo><20201028><for wechat copy file to desktop> add end
...
INT find_atom_in_buildin_format(INT id, Atom *supportAtoms, INT max){
    int size = sizeof(builtin_formats)/sizeof(builtin_formats[0]);
    int i;
    int count=0;
    for(i=0;i<size;++i){
        if(builtin_formats[i].id==id){
            if(count>=max){
                break;
            }
            supportAtoms[count]=GET_ATOM(builtin_formats[i].data);
            count++;
        }
    }
    return count;
...
static HANDLE import_gnome_cp_files( Atom type, const void *data, size_t size )
{
    ...
    TRACE( "YYYXXXRRR---> import_gnome_cp_files uriList =%s\n",uriList);

    if (!(out = HeapAlloc(GetProcessHeap(), 0, capacity * sizeof(WCHAR)))) return 0;

    while (end < size)
    {
        while (end < size && uriList[end] != '\n')
            ++end;
        ...
        if(StrCmpA(uri,"copy")!=0&&StrCmpA(uri,"cut")!=0){
            path = uri_to_dos(uri);
...
static BOOL export_gnome_cp_files( Display *display, Window win, Atom prop, Atom target, HANDLE handle )
{
    TRACE( "YYYXXXRRR---> export_gnome_cp_files\n");
    UINT i;
    UINT numFiles;
    char *textUriList;
    ...
    TRACE( "YYYXXXRRR---> export_gnome_cp_files file numbers=%d\n",numFiles);
    for (i = 0; i < numFiles; i++)
    {
        ...
        uriSize += 8 + /* file:/// */
                ...
                (lstrlenA(unixFilename) - 1) + /* "%xy" per char except first '/' */
                1; /* \n */
        ...
        if(i==0){
            lstrcpyA(&textUriList[next], "copy\n");
            next += 5;
        ...
        {
            textUriList[next++] = unixFilename[u];
        ...
        TRACE( "YYYXXXRRR---> export_gnome_cp_files file name=%s\n",unixFilename);
        HeapFree(GetProcessHeap(), 0, unixFilename);
    }
    textUriList[next]='\0';
...
BOOL export_buildIn_target( Display *display, Window win, Atom prop, Atom target, HANDLE handle )
{
    int size = sizeof(builtin_formats)/sizeof(builtin_formats[0]);
    int i;
    for(i=0;i<size;++i){
        if(GET_ATOM(builtin_formats[i].data)==target&&builtin_formats[i].export){
            if(builtin_formats[i].export(display,win,prop,target,handle)){
                return TRUE;
            }else{
...
BOOL X11DRV_SelectionRequest( HWND hwnd, XEvent *xev )
{
    ...
    if(event->selection == x11drv_atom(XdndSelection)){
        return  X11DRV_XDND_SelectionRequest(hwnd, xev);
    }
