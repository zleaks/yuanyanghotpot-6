...
//wlz,begin,2019/5
static void active_window(Display *disp, Window *win)
{
    ...
    ev.window = *win;
    ev.message_type = XInternAtom(disp, "_NET_ACTIVE_WINDOW", TRUE);
    ev.format = 32;
    ev.data.l[0] = 1;
    ev.data.l[1] = CurrentTime;
    ev.data.l[2] = ev.data.l[3] = ev.data.l[4] = 0;
    XSendEvent (disp, DefaultRootWindow(disp), FALSE, SubstructureRedirectMask |SubstructureNotifyMask, (XEvent*)&ev);
    XFlush (disp);
}
//wlz,end
#endif
...
static int EndWith(const char *str,const char *suffix){
    if(!str||!suffix){
        return 0;
    }
...
static BOOL is_window_managed_for_hotpot( HWND hwnd, UINT swp_flags, const RECT *window_rect )
{
/* start 20210713 zhuyi add for foxmail用户信息窗口闪退 */
#if defined __HOTPOT_FOXMAIL__
    {
        char classnm[256];
        GetClassNameA(hwnd,classnm,256);
        if(strcmp(classnm,"FMShadowWnd") == 0)
        {
            return FALSE;
        }
    }
#endif
/* end 20210713 zhuyi add for foxmail用户信息窗口闪退 */

/* by liuyanhui 2020.12.03 && zhangchunguang foxmail minimize caton , so only for QQ 2021.05 begin*/
#if defined __HOTPOT_QQ__
    if (swp_flags & SWP_NOACTIVATE) return FALSE;
#endif
/* by liuyanhui 2020.12.03 && zhangchunguang foxmail minimize caton , so only for QQ 2021.05 end*/

/* 20211029 zhangxuguang add for office2013\2016\2019功能区菜单显示和下拉框显示问题，EXCEL2013右键不能粘贴问题 --> */
#if defined __HOTPOT_OFFICE201X__
    {
        char classnm[256];
        GetClassNameA(hwnd,classnm,256);

        if (strcmp(classnm,"devcat_launcher") == 0)
            return FALSE;

        if (strcmp(classnm, "Net UI Tool Window") == 0)
            return FALSE;

        if (strcmp(classnm, "Net UI Tool Window Layered") == 0)
            return FALSE;
    }
#endif
/* 20211029 zhangxuguang add for office2013\2016\2019功能区菜单显示和下拉框显示问题，EXCEL2013右键不能粘贴问题 <-- */

/*start, add by zhuyi, 20211122,解决photoshop14鼠标上浮弹出提示信息导致卡死问题*/
#if defined __HOTPOT_PHOTOSHOP14__
    char classnm[256];
    GetClassNameA(hwnd,classnm,256);

    if (strcmp(classnm,"tooltips_class32") == 0)
        return FALSE;
#endif
    ...
    if (strcmp(classnm,"tooltips_class32") == 0)
        return FALSE;
#endif
/*end, add by zhuyi, 20211122,解决photoshop14鼠标上浮弹出提示信息导致卡死问题*/

    return TRUE;
}

//处理需要返回TRUE的情况
static BOOL is_window_managed_for_hotpot_v2( HWND hwnd, UINT swp_flags, const RECT *window_rect )
{
    //20211227 （1）office2010 载入图片右键菜单旋转菜单栏消失，（2）文字ctrl＋c－》右键[A]粘贴失败。
    //(3)右键菜单图片裁剪无效(2016、2019也有这个问题，由于在2013、2019、2016识别类Net UI Tool Window Layered需要返回FALSE所以这里只修复2010)
#if defined __HOTPOT_OFFICE2010__
    {
        char classnm[256];
        GetClassNameA(hwnd,classnm,256);
        if (strcmp(classnm, "Net UI Tool Window Layered") == 0 && IsRButtonDown() )
        {
            return TRUE;
        }
    }
...
static BOOL is_window_managed( HWND hwnd, UINT swp_flags, const RECT *window_rect )
{
    ...
    if (!is_window_managed_for_hotpot(hwnd, swp_flags, window_rect)) return FALSE;
    if( is_window_managed_for_hotpot_v2(hwnd, swp_flags, window_rect) ) return TRUE;
...
static int get_window_attributes( struct x11drv_win_data *data, XSetWindowAttributes *attr )
{
	...
	if((data->hwnd)&&(GetPropA(data->hwnd,"checkupgradewin")==(HANDLE)(0xff))){
		attr->event_mask		= (ExposureMask | KeymapStateMask | StructureNotifyMask);
		TRACE("has catch upgrade win\n");
	}
	else
#endif
...
static void set_size_hints( struct x11drv_win_data *data, DWORD style )
{
    ...
    if (!(style & (WS_MINIMIZE | WS_MAXIMIZE)))
    {
        ...
        {
            if(data->min_height>0&&data->min_width>0){
                TRACE("YYYXXXRRR---> try to set a min LIMIT %dx%d\n",data->min_width,data->min_height);
                size_hints->min_height=data->min_height;
                size_hints->min_width=data->min_width;
                size_hints->flags|=PMinSize;
            }
            if(data->max_height>0&&data->max_width>0){
                TRACE("YYYXXXRRR---> try to set a max LIMIT %dx%d\n",data->max_width,data->max_height);
                size_hints->max_height=data->max_height;
                size_hints->max_width=data->max_width;
...
void update_net_wm_states( struct x11drv_win_data *data )
{
    ...
    else  /* ask the window manager to do it for us */
    {
        ...
        {
            //<yangxinruo><20210305><for wemeet new window show behind ,revert c5ec1585f6e5211a2b63e3435748210552250534> add begin
            if (!((data->net_wm_state ^ new_state) & (1 << i))) continue;  /* unchanged */
            //<yangxinruo><20210305><for wemeet new window show behind ,revert c5ec1585f6e5211a2b63e3435748210552250534> add end
...
static void unmap_window( HWND hwnd )
{
    ...
    if (data->mapped)
    {
        ...
        if (data->embedded) set_xembed_flags( data, 0 );
            ...
            //<yangxinruo><20210201><for wechat hide main window when screenshot> add begin
            char classnm[256];
            if(hwnd)
            {
                GetClassNameA(hwnd,classnm,256);
                if(strcmp(classnm,"WeChatMainWndForPC") == 0 || strcmp(classnm,"ChatWnd") == 0)
                {
                    XSync(data->display,FALSE);
                }
            }
            //<yangxinruo><20210201><for wechat hide main window when screenshot> add end
        }
...
Window create_client_window( HWND hwnd, const XVisualInfo *visual )
{
    ...
    if (!data)
    {
        ...
        //<yangxinruo><20200125><for wechat browser user GPU accelerate> modify begin
        // if (parent == GetDesktopWindow() || GetAncestor( parent, GA_PARENT )) return 0;
        HWND pParent;
        char classnmPparent[256];
        if(parent){
            pParent = GetAncestor( parent, GA_PARENT );
            if(pParent){
                GetClassNameA(pParent,classnmPparent,256);
            }
        }
        if (parent == GetDesktopWindow() || (pParent && strcmp(classnmPparent,"CefWebViewWnd") != 0)) return 0;
        //<yangxinruo><20200125><for wechat browser user GPU accelerate> modify end
...
/* 20210412 wanghg add for 应用被前台窗口覆盖后，残留一个阴影框的问题 --> */
static BOOL is_popupshadow(HWND hwnd) {
    char classnm[256];
...
#if 0
    if(EndWith(processnm,"HaoZip.exe")){
        char classnm[256];
        GetClassNameA(hwnd,classnm,256);
        TRACE("wanghg---> FIND WINDOW FOR DST className=%s\n",classnm);
        if(strcmp(classnm,"RCShadowWindow") == 0) {
            return TRUE;
        }
    }
#endif
...
static void create_whole_window( struct x11drv_win_data *data )
{
    ...
    if (is_popupshadow(data->hwnd)) return; /* 20210412 wanghg add for 应用被前台窗口覆盖后，残留一个阴影框的问题 */
...
BOOL CDECL X11DRV_CreateWindow( HWND hwnd )
{
    ...
    LPSTR clsname = HeapAlloc(GetProcessHeap(), 0, 12);
    LPSTR title = HeapAlloc(GetProcessHeap(), 0, 24);
    if (title && clsname)
    {
        GetWindowTextW(hwnd, title, 10);
        GetClassNameA(hwnd, clsname, 11);
        if ((strcmp(clsname, "Afx:004000") == 0 || strcmp(clsname, "BCGPRibbon") == 0) && strstrW(title, L"CAXA CAD"))
        {
        ...
        HeapFree(GetProcessHeap(), 0 ,clsname);
    }
#endif

#if defined (__HOTPOT_E32010__)
   LPSTR clsname = HeapAlloc(GetProcessHeap(), 0, 17);
   if(clsname)
   {
        GetClassNameA(hwnd, clsname, 16);
        if(!strcmp(clsname, "Afx:00400000:84"))
        {   
            LONG style = GetWindowLongW(hwnd, GWL_STYLE);
            if(style == 0x84000000)
    ...
    //add by xdl 解决Carsim启动后无任务栏图标
#if defined __HOTPOT_CARSIM8__
    char class_name[256];
    GetClassNameA(hwnd, class_name, 256);
    if(strcmp(class_name, "CVIRTLVDChild4194304") == 0)
    {
        LONG dwExStyle = GetWindowLongW(hwnd, GWL_EXSTYLE);
	SetWindowLongW(hwnd, GWL_EXSTYLE, dwExStyle | WS_EX_APPWINDOW);
    }
#endif
    //end
                                    ...
                                    struct window_surface *surface )
{
    ...
    /*start, add by zhuyi, 202107002，解决腾讯视频会议异常窗口问题*/
#if defined __HOTPOT_WEMEET__
    GetClassNameA(hwnd,classnm,256);
    if (strncmp(classnm, "TXGuiFoundation", strlen("TXGuiFoundation")) == 0)
    {
        if(new_style==0x960a0000 && rectClient->left == 5 && rectClient->top == 5)
        {
            if (swp_flags == 0x0000003c || swp_flags == 0x0000180f 
            || swp_flags == 0x0000180b || swp_flags == 0x00003c1f)
            {
                //TRACE("====== zhuyi[555] %s processnm[%s]\n", classnm, processnm);
                //return;
                data->managed = 0;
            }
        }
...
#endif
    /*end, add by zhuyi, 202107002，解决腾讯视频会议异常窗口问题*/
        ...
        ((new_style & WS_MINIMIZE) || is_window_rect_mapped( rectWindow )))
    {
            ...
            if (needs_map) 
	    {
            ...
            //wlz,begin,2019/5	
            usleep(200000);
  	        active_window(data->display, &data->whole_window);
            //wlz,end
#endif
	    }
            return;
...
#if defined __HOTPOT_WECHAT__
                //wlz,begin,2019/5
                if(!(swp_flags & SWP_NOACTIVATE) && !(swp_flags & SWP_NOZORDER) && (data->iconic==0))
                {
                    active_window(data->display, &data->whole_window);
                }
                //wlz,end
...
#ifdef __HOTPOT_QQ__
    if(new_style == 0xb60f0000 && (swp_flags == 0x0000180b || swp_flags & 0x1a00 )) {
                                       ...
                                       const RECT *window_rect )
{
...
#if defined __HOTPOT_WECHAT__
    GetClassNameA(hwnd,classnm,256);
    if (strcmp("QuoteMsgLayerWnd", classnm) == 0)
    {
        rect = *window_rect;
        rect.left = 0;
        rect.bottom = 0;
    ...
    {
        //<yangxinruo><for wechat systray icon can not flash> modify begin
        // data->surface = create_surface( data->whole_window, &data->vis, &rect,
        //                         color_key, data->use_alpha);
        data->surface = create_surface1( data->whole_window, &data->vis, &rect,
                                        color_key, data->embedded?False:data->use_alpha, data->embedded);
        //<yangxinruo> modify end
...
//<yangxinruo><2020-09-27><for wechat minimum window size restriction not work> add begin
void CDECL X11DRV_SetWindowSizeLimit( HWND hwnd, MINMAXINFO *mmi )
{
    struct x11drv_win_data *data;
    ...
    data->min_width=mmi->ptMinTrackSize.x;
    data->min_height=mmi->ptMinTrackSize.y;
    data->max_width=mmi->ptMaxTrackSize.x;
    data->max_height=mmi->ptMaxTrackSize.y;
    TRACE("YYYXXXRRR---> SET A MAXMIN LIMIT %dx%d\n",data->min_width,data->min_height);
done:
    release_win_data( data );
}
//<yangxinruo><2020-09-27><for wechat minimum window size restriction not work> add end

//<yangxinruo><20210220><for wechat ungrabpointer before call GTK file dialog> add begin
static void my_gtk_realize(GtkWidget* widget, gpointer user)
{
    gtk_widget_set_window(widget, (GdkWindow*)user);
}

LPVOID CDECL X11DRV_GetGTKWindowHandle(HWND hwnd){
    struct x11drv_win_data *data;

    if (!(data = get_win_data( hwnd ))) return NULL;
    GdkDisplay *ddp=gdk_x11_lookup_xdisplay(data->display);
    GdkWindow* gw = gdk_x11_window_foreign_new_for_display(ddp, data->whole_window);
    ...
    TRACE("YYYXXXRRR---> call X11DRV_GetGTKWindowHandle ddp=%p hwnd=%p dp=%p x11=%08x gdk=%p gtk=%p isPointergrabbed=%d\n",ddp,hwnd,data->display,data->whole_window,gw,gtk,gdk_display_pointer_is_grabbed (ddp));
    if (is_window_rect_full_screen( &data->window_rect )){
        TRACE("YYYXXXRRR---> to force ungrab DISPLAY=%p\n",data->display);
...
//<yangxinruo><20210220><for wechat ungrabpointer before call GTK file dialog> add end
