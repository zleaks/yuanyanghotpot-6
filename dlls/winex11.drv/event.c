...
static void handle_wm_protocols( HWND hwnd, XClientMessageEvent *event )
{
    ...
    else if (protocol == x11drv_atom(WM_TAKE_FOCUS))
    {
        ...
        //<yangxinruo><20201027><for wechat ContactProfileWnd dialog cannot dismiss when click outside> add begin
        HWND oriFocusTarget = hwnd;
        //<yangxinruo><20201027><for wechat ContactProfileWnd dialog cannot dismiss when click outside> add end
        ...
        //<yangxinruo><20210309><for wxwork mainWnd and dialog get focue by turns rapidly> add begin
        BOOL flagRecheck=TRUE;
        char processnm[256];
        GetModuleFileNameA(NULL,processnm,256);
        if(EndWith(processnm,"WXWork.exe")){
            if(last_focus==hwnd){
                TRACE("YYYXXXRRR---> old win may raise again?\n %p",hwnd);
                flagRecheck=FALSE;
            }
        }

        //解决office2010右键粘贴菜单鼠标悬停在粘贴选项时粘贴菜单在焦点会被转移引起闪烁和消失。
        //增加只对右键菜单做处理的逻辑（右键菜单窗口类名是NetUIHWND）
        #if defined __HOTPOT_OFFICE2010__
        if( IsHwndMouseHovering(GetFocus()) )
        {
            char lv_className[256];
            GetClassNameA(GetFocus(),lv_className,256);
            if( strcmp(lv_className,"NetUIHWND") == 0 )//右键菜单窗口
            {
                FIXME("没有执行setFouce，因为判断在焦点变化前的窗口有鼠标悬停操作\n");
                return;
            }
        }
        #endif
        //<yangxinruo><20210309><for wxwork mainWnd and dialog get focue by turns rapidly> add end

        if (can_activate_window(hwnd)&&flagRecheck)
        {
        ...
        if (!hwnd) hwnd = last_focus;
        //<yangxinruo><20201027><for wechat ContactProfileWnd dialog cannot dismiss when click outside> modify begin
        // if (hwnd && can_activate_window(hwnd)) set_focus( event->display, hwnd, event_time );
        if (hwnd && can_activate_window(hwnd)) {
            if(oriFocusTarget!=hwnd){
                char classnm[256];
                GetClassNameA(hwnd,classnm,256);
                if(strcmp(classnm,"ContactProfileWnd") == 0)
		        {
                    TRACE("YYYXXXRRR---> tell ContactProfileWnd should have lose focus\n");
                    SendMessageW( hwnd, WM_NCACTIVATE, FALSE, (LPARAM)oriFocusTarget );
                    SendMessageW( hwnd, WM_ACTIVATE,
                        MAKEWPARAM( WA_INACTIVE, IsIconic(hwnd) ), (LPARAM)oriFocusTarget );
                    }
            }
            set_focus( event->display, hwnd, event_time );
        }
        //<yangxinruo><20201027><for wechat ContactProfileWnd dialog cannot dismiss when click outside> modify end
    }
...
static const struct client_message_handler client_messages[] =
{
    ...
    { XATOM_XdndStatus,   X11DRV_XDND_StatusEvent },
    { XATOM_XdndFinished, X11DRV_XDND_FinishedEvent }
};
