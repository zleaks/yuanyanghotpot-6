...
struct gl_drawable
{
    ...
    BOOL                           mutable_pf;      //20211210 lidong add for photoshop14 
#endif
};
...
#if defined __HOTPOT_PHOTOSHOP14__
static void (*pglXWaitGL)( void );          //20211210 lidong add for photoshop14 
#endif
...
#if defined __HOTPOT_PHOTOSHOP14__
    LOAD_FUNCPTR(glXWaitGL);              //20211210 lidong add for photoshop14 
#endif
 ...
 */
#if defined __HOTPOT_PHOTOSHOP14__
static struct gl_drawable *create_gl_drawable( HWND hwnd, const struct wgl_pixel_format *format, BOOL known_child, BOOL mutable_pf )
#else
...
#endif
{
    ...
    gl->ref = 1;
#if defined __HOTPOT_PHOTOSHOP14__
    gl->mutable_pf = mutable_pf;           // 20211210 lidong add for photoshop14 
#endif
...
static BOOL set_pixel_format(HDC hdc, int format, BOOL allow_change)
{
    ...
    if (!allow_change)
    {
        ...
        #if defined __HOTPOT_PHOTOSHOP14__
            BOOL mutable_pf = gl->mutable_pf;              // 20211210 lidong add for photoshop14
        #endif
            release_gl_drawable( gl );
        #if defined __HOTPOT_PHOTOSHOP14__
            if (!mutable_pf) return prev == format;  /* cannot change it if already set */
        #else
...
#if defined __HOTPOT_PHOTOSHOP14__
    return set_win_format( hwnd, fmt, allow_change );            // 20211210 lidong modify for photoshop14
#else
...
static BOOL WINAPI glxdrv_wglShareLists(struct wgl_context *org, struct wgl_context *dest)
{
    ...
    {
    //added by jiangqian begin---meituxiuxiu---face reshaping
    HWND hwnd=FindWindowA("MTXXMainWnd",NULL);
    if(hwnd!=NULL)
...
#if defined __HOTPOT_PHOTOSHOP14__
    else if (escape.gl_drawable && pglXWaitGL)
        pglXWaitGL();
#endif
