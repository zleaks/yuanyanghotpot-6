...
static const char * const atom_names[NB_XATOMS - FIRST_XATOM] =
{
    ...
    "text/richtext",
    "text/uri-list",
    //<yangxinruo><20201028><for wechat copy file to desktop> add begin
    "x-special/gnome-copied-files"
    //<yangxinruo><20201028><for wechat copy file to desktop> add end
};
...
/***********************************************************************
<zyh><2021-10-22> <add gtk funcs> begin
init_dpi() 
根据分辨率初始化DPI
steps:
...
static  BOOL set_dpi_by_winecfg(void)
{
...
static  int get_dpi_by_pixels(void)
{
	//const UINT dpi_values[] = { 96, 120, 144, 168, 192, 216, 240, 288, 336, 384, 432, 480 };
	unsigned int width,height,dpi_data;
	const WCHAR dpi_2k_keyname[] = {'2','k','d','p','i',0};
	const WCHAR dpi_4k_keyname[] = {'4','k','d','p','i',0};
	const WCHAR dpi_8k_keyname[] = {'8','k','d','p','i',0};
	WCHAR *dpi_keyname=NULL;
	const int pixels_2k=2000;//2000;//2k分辨率
	const int pixels_4k=3000;//3000;//4k分辨率
	const int pixels_8k=7000;//7000;//8k分辨率
    height=DisplayHeight( gdi_display, default_visual.screen );
    width=DisplayWidth( gdi_display, default_visual.screen );//只判断横向分辨率
    if(width>(height*2))//判断是否是双屏显示
		width=width/2;
	if(width>=pixels_8k){
		dpi_data=216;
		dpi_keyname=dpi_8k_keyname;
	}
	else if(width>=pixels_4k){
		dpi_data=168;
		dpi_keyname=dpi_4k_keyname;
	}
	else if(width>=pixels_2k){
		dpi_data=120;
		dpi_keyname=dpi_2k_keyname;
	}
	else
		dpi_data=96;
	
	while(dpi_keyname!=NULL){
		const WCHAR dpis_regname[] = {'C','o','n','t','r','o','l',' ','P','a','n','e','l','\\','D','e','s','k','t','o','p','\0'};
		HKEY dpi_key;
		int res;
		DWORD data_size;
		DWORD reg_dpi_data;
		data_size = sizeof(reg_dpi_data);

		res=RegOpenKeyExW(HKEY_CURRENT_USER,dpis_regname,0,KEY_READ|KEY_WRITE|KEY_WOW64_64KEY, &dpi_key);
		if(ERROR_SUCCESS!=res){
			res=RegOpenKeyExW(HKEY_CURRENT_USER,dpis_regname,0,KEY_READ|KEY_WRITE|KEY_WOW64_32KEY, &dpi_key);
		}
		if(ERROR_SUCCESS!=res)
			break;
		res=RegQueryValueExW(dpi_key, dpi_keyname, NULL, NULL,(BYTE *)&reg_dpi_data, &data_size);
		if(ERROR_SUCCESS!=res)
			break;
		dpi_data=reg_dpi_data;//取注册表的值
		break;
	}
...
{
    EnterCriticalSection( &x11drv_section );//防止多线程冲突
	static BOOL init_dpi=FALSE;
	if(init_dpi)
		goto END;
	init_dpi=TRUE;

	if(set_dpi_by_winecfg()){
		goto END;
	}
	
	const WCHAR logpixels_reg[] = {'C','o','n','t','r','o','l',' ','P','a','n','e','l','\\','D','e','s','k','t','o','p','\0'};
	const WCHAR logpixels[] = {'L','o','g','P','i','x','e','l','s',0};
	HKEY dpi_key;
	...
	DWORD data_size = sizeof(dpi_data);
	
	res=RegQueryValueExW(dpi_key, logpixels, NULL, NULL,(BYTE *)&reg_dpi_data, &data_size);
	
	TRACE("RegQueryValueExW(%d)(%d)(%d)\n",res,reg_dpi_data,dpi_data);
	/*if(ERROR_SUCCESS!=res){//查询失败，创建值
		res= RegSetValueExW(dpi_key,logpixels,0,REG_DWORD,(BYTE *)&dpi_data,data_size);
	}*/
	//4.设置键值
	if(reg_dpi_data==dpi_data)
		goto END;
	reg_dpi_data=dpi_data;

	res= RegSetValueExW(dpi_key,logpixels,0,REG_DWORD,(BYTE *)&reg_dpi_data,data_size);
	if(ERROR_SUCCESS!=res){
		ERR("(%d)\n",res);//设置错误
		goto END;
	}
...
//
//<zyh><2021-10-22> <add gtk funcs> end
...
static BOOL process_attach(void)
{
    ...
    //<yangxinruo><20210220><for wechat ungrabpointer before call GTK file dialog> modify begin
    // if (!(display = XOpenDisplay( NULL ))) return FALSE;
    EnterCriticalSection( &x11drv_section );
    gtk_init(NULL, NULL);
    const char* dName = getenv("DISPLAY");
    GdkDisplay* gd = gdk_display_open(dName);
    display = GDK_DISPLAY_XDISPLAY(gd);
    LeaveCriticalSection( &x11drv_section );
    if (!display) return FALSE;
    //<yangxinruo><20210220><for wechat ungrabpointer before call GTK file dialog> modify end
...
struct x11drv_thread_data *x11drv_init_thread_data(void)
{
    ...
    const char* dName = getenv("DISPLAY");
    GdkDisplay* gd = gdk_display_open(dName);
    data->display = GDK_DISPLAY_XDISPLAY(gd);
    LeaveCriticalSection( &x11drv_section );
    if (!data->display)
    {
