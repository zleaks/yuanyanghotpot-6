...
//解决内网通截图卡死以及截图黑屏问题
#if defined __HOTPOT_SHIYELINE__
        TRACE("solve shiyeline screenshot black window problem!\n");
#else
...
struct window_surface *create_surface1( Window window, const XVisualInfo *vis, const RECT *rect,
                                       COLORREF color_key, BOOL use_alpha,BOOL use_embedded )
//<yangxinruo><for wechat systray icon can not flash> modify end
{
    const XPixmapFormatValues *format = pixmap_formats[vis->depth];
    ...
    //<yangxinruo><for wechat systray icon can not flash> modify begin
    //remove it to prevent black background
    if(!use_embedded){
...
//<yangxinruo><for wechat systray icon can not flash> add begin
struct window_surface *create_surface( Window window, const XVisualInfo *vis, const RECT *rect,
    ...
    return create_surface1(window,vis,rect,color_key,use_alpha,False);
}
//<yangxinruo><for wechat systray icon can not flash> add end
