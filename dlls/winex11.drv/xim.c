 ...
 */
static int STYLE_OVERTHESPOT_OPEN = 0;
BOOL X11DRV_InitXIM( const char *input_style )
{
	...
	else
	{
	    ...
	    const char *xim_style=getenv( "XIM_STYLE" );
	    if( xim_style && !strcmp(xim_style,"overthespot"))
	    {
			STYLE_OVERTHESPOT_OPEN = 1;
	    }
	    ximStyleRequest = STYLE_OVERTHESPOT;
	    /*Fix Input Method always at lower left corner*/
	}
...
void X11DRV_UpdateCandidatePos(HWND hwnd, XIC xic)
{
	...
	static POINT CursorAbstractPos;
    if (ximStyle & XIMPreeditPosition)
    {
        ...
        if (focus)
        {
            ...
            {
				win = ((struct x11drv_win_data *)data)->whole_window;
            ...
            GetCaretPos(&point);/*跨进程*/
            /* the caret's position */
            ClientToScreen(focus, &point);/*跨进程*/
            TRACE("point.x = %d point.y = %d\n", point.x, point.y);

            if( CursorAbstractPos.x == point.x && CursorAbstractPos.y == point.y)
            {
                return;
            }

            xpoint.x = point.x - origin_x;
            /* FIXME: use the height of the font instead of 30 */
            xpoint.y = point.y - origin_y - 30 ;
            ...
            XFree(preedit);
            CursorAbstractPos.x = point.x;
            CursorAbstractPos.y = point.y;
        }
    }
}
