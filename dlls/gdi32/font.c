...
#if defined (__HOTPOT_WECHAT__) || defined (__HOTPOT_WECOM__)
static struct gdi_font_subst *get_gdi_font_subst_wine520( const WCHAR *from_name, int from_charset, int *to_charset )
{
    struct gdi_font_subst *subst;
...
static BOOL add_gdi_font_subst( const WCHAR *from_name, int from_charset, const WCHAR *to_name, int to_charset )
{
    ...
    /*start, add by zhuyi, 20210805，解决微信3.3.0小程序无法打开和微信企业版部分功能异常的问题，字体连接引起的，沿用线上旧版本的字体修正方案*/
#if defined (__HOTPOT_WECHAT__) || defined (__HOTPOT_WECOM__)
    TRACE("====> WXWork || WeChat 330 chg_font_subst_toname\n");
    struct gdi_font_subst *subst_toname = get_gdi_font_subst_wine520(to_name, to_charset, NULL);
    if (subst_toname) {
        lstrcpyW( get_subst_to_name(subst), get_subst_to_name(subst_toname) );
    } else {
    ...
    lstrcpyW( get_subst_to_name(subst), to_name );
#endif
    /*end, add by zhuyi, 20210805，解决微信3.3.0小程序无法打开和微信企业版部分功能异常的问题，字体连接引起的，沿用线上旧版本的字体修正方案*/
...
static void load_gdi_font_subst(void)
{
    ...
    /* renzhengyi modify for QQ群公告字体乱码与字体大小显示不正常 begin */
#if defined __HOTPOT_QQ__
    RegSetKeyValueW( hkey, 0, L"Helv", REG_SZ, L"SimSun", sizeof(L"SimSun"));
    RegSetKeyValueW( hkey, 0, L"Helvetica", REG_SZ, L"SimSun", sizeof(L"SimSun"));
#endif
    /* renzhengyi modify for QQ群公告字体乱码与字体大小显示不正常 end */    
...
static BOOL get_face_enum_data( struct gdi_font_face *face, ENUMLOGFONTEXW *elf, NEWTEXTMETRICEXW *ntm )
{
    ...
    if (font_funcs->set_outline_text_metrics( font ))
    {
        ...
        memcpy((char *)&ntm->ntmTm + offsetof( TEXTMETRICW, tmWeight ),
               (const char *)&font->otm.otmTextMetrics + offsetof( TEXTMETRICW, tmWeight ),
...
HFONT WINAPI CreateFontIndirectExW( const ENUMLOGFONTEXDVW *penumex )
{
    HFONT hFont;
    FONTOBJ *fontPtr;
/* 20211117 renzy modify for font of CAXA-3D display blank squares start*/
#if defined __HOTPOT_CAXA__
    LOGFONTW *plf;
#else
    const LOGFONTW *plf;
#endif
/* 20211117 renzy modify for font of CAXA-3D display blank squares end*/
                         ...
                         const RECT *lprect, LPCSTR str, UINT count, const INT *lpDx )
{
    ...
    if(strstr(str, "Procedure or Misc. Item: Factor 'A'")
      || strstr(str, "Vehicle or Misc. Item: Factor 'B'")
      || strstr(str, "Create Matrix and Make All Runs")
      || strstr(str, "Available Variables")
      || strstr(str, "Existing run used as template for new runs"))
    {
	TEXTMETRICW lptm;
	...
	logfont.lfWidth = lptm.tmAveCharWidth;
	logfont.lfWeight = lptm.tmWeight;
	sprintf(logfont.lfFaceName, "%s", "Times New Roman");
