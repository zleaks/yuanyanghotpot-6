...
typedef DWORD (WINAPI *GetModuleFileNameA_funcptr)( HMODULE,LPSTR,DWORD );
...
HBITMAP WINAPI CreateCompatibleBitmap( HDC hdc, INT width, INT height)
{
    ...
    TRACE("(%p,%d,%d)\n", hdc, width, height);
    //<yangxinruo><for wechat 2.9.5 get black screen when snapshot> begin
    HBITMAP resBitmap;
    HDC dcMem;
    if (GetObjectType( hdc ) != OBJ_MEMDC){
        char processnm[256];
        GetModuleFileNameA(NULL,processnm,256);
        if(EndWith(processnm,"WeChat.exe")){
            HMODULE mod;
            ...
            if (mod)
            {
                ...
                if (pWindowFromDC)
                {
                    ...
                    if (pGetClassNameA)
                    {
                        ...
                        if(wndSrc)
                        {
                            pGetClassNameA(wndSrc,classnm,256);
                            TRACE("YYYXXXRRR---> FIND WINDOW FOR DST className=%s\n",classnm);
                            //#32769 is classname of wechat snapshot src wnd
                            if(strcmp(classnm,"#32769") == 0)
                            {
                                resBitmap = CreateBitmap( width, height,
                                                    GetDeviceCaps(hdc, PLANES), GetDeviceCaps(hdc, BITSPIXEL), NULL );
                                if(hdc&&width>1&&height>1){
                                    TRACE("YYYXXXRRR--> let init new bitmap with content in srchdc hdc=%p newBitmap=%p\n",hdc,resBitmap);
                                    dcMem = CreateCompatibleDC( hdc );
                                    SelectObject(dcMem, resBitmap);
                                    BitBlt(dcMem,0,0,width,height,hdc,0,0,SRCCOPY);
                                    DeleteDC(dcMem);
                                }
                                return resBitmap;
                            }
    ...
    //<yangxinruo><for wechat 2.9.5 get black screen when snapshot> end
