...
//<yangxinruo><20200929><for wechat snapscreen black screen fix #2> add begin
typedef INT (WINAPI *GetSystemMetrics_funcptr)(INT);
//<yangxinruo><20200929><for wechat snapscreen black screen fix #2> add end
/******************************************************************************
 *           GdiAlphaBlend [GDI32.@]
 */
                          ...
                          BLENDFUNCTION blendFunction)
{
    ...
    //<yangxinruo><20200929><for wechat snapshot black screen fix #2> add begin
    char processnm[256];
    ...
    if(EndWith(processnm,"WeChat.exe")){
        ...
        if (mod)
        {
            GetSystemMetrics_funcptr pGetSystemMetrics=(GetSystemMetrics_funcptr)GetProcAddress(mod,"GetSystemMetrics");
            if(pGetSystemMetrics&&widthDst==pGetSystemMetrics(0)){
                void* callstack[128];
                int i, frames = backtrace(callstack, 128);
                char** strs = backtrace_symbols(callstack, frames);
                for (i = 0; i < frames; ++i) {
                    // TRACE("YYYXXXRRR---> %s\n", strs[i]);
                    if(strcmp(strs[i],"[0x1082e92b]")==0){
                        free(strs);
                        TRACE("YYYXXXRRR---> set fullscreen suspectableSnapDc->%p\n",hdcDst);
                        return TRUE;
                    }
                }
                free(strs);
    ...
    //<yangxinruo><20200929><for wechat snapshot black screen fix #2> add end
