...
static char *get_unix_file_name( LPCWSTR dosW )
{
    ...
    for (;;)
    {
        ...
        //status = wine_nt_to_unix_file_name( &nt_name, buffer, &size, FILE_OPEN_IF, FALSE);//zcg
...
static BOOL CDECL freetype_get_glyph_index( struct gdi_font *font, UINT *glyph, BOOL use_encoding )
{
    ...
    if (ft_face == NULL || ft_face->charmap == NULL) return FALSE; /* 20210521 wanghg add for 数码大方在专用机上不能启动 */
