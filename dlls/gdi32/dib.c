...
//<yangxinruo><for wechat voip rotate 90> add begin
LPVOID GetRotatedBitmap( LPBITMAPINFO hDIB, LPVOID lpDIBBits, float degree, COLORREF clrBack ,LPBITMAPINFO newInfo)
{
	// Get source bitmap info
    float radians=degree*(3.14159265f/180);
	BITMAPINFO bmInfo = *hDIB;
	int bpp = bmInfo.bmiHeader.biBitCount;		// Bits per pixel

	int nColors = bmInfo.bmiHeader.biClrUsed ? bmInfo.bmiHeader.biClrUsed :
					1 << bpp;
	int nWidth = abs(bmInfo.bmiHeader.biWidth);
	int nHeight = abs(bmInfo.bmiHeader.biHeight);
    ...
    // int nHeaderSize = sizeof(bmInfo);
	// HANDLE hDIBResult = GlobalAlloc(GMEM_FIXED,len+nHeaderSize);
	// Initialize the header information
	// memcpy( (void*)hDIBResult, (void*)&bmInfo, nHeaderSize);
    ...
    // int nHeaderSize = sizeof(bmInfo);
	// memcpy( newInfo, (void*)&bmInfo, nHeaderSize);
    
    // BITMAPINFO bmInfoResult = *newInfo ;
    // bmInfoResult.bmiHeader.biWidth = w;
	// bmInfoResult.bmiHeader.biHeight = h;
	// bmInfoResult.bmiHeader.biSizeImage = len;
    TRACE("YYYXXXRRR---> computed attrs: %dx%d size=%d\n",w,h,len);
    newInfo->bmiHeader.biBitCount=bmInfo.bmiHeader.biBitCount;
    newInfo->bmiHeader.biClrImportant=bmInfo.bmiHeader.biClrImportant;
    newInfo->bmiHeader.biClrUsed=bmInfo.bmiHeader.biClrUsed;
    newInfo->bmiHeader.biCompression=bmInfo.bmiHeader.biCompression;
    newInfo->bmiHeader.biHeight=h;
    newInfo->bmiHeader.biPlanes=bmInfo.bmiHeader.biPlanes;
    newInfo->bmiHeader.biSize=bmInfo.bmiHeader.biSize;
    newInfo->bmiHeader.biSizeImage=len;
    newInfo->bmiHeader.biWidth=w;
    newInfo->bmiHeader.biXPelsPerMeter=bmInfo.bmiHeader.biXPelsPerMeter;
    newInfo->bmiHeader.biYPelsPerMeter=bmInfo.bmiHeader.biYPelsPerMeter;

    // lpDIBBitsResult = GlobalAlloc(GMEM_FIXED,len);
    LPVOID lpDIBBitsResult = HeapAlloc( GetProcessHeap(), HEAP_ZERO_MEMORY, len );
    TRACE("YYYXXXRRR---> computed attrs NEW BITS P : %p\n",lpDIBBitsResult);
    ...
    return lpDIBBitsResult;
	// return hDIBResult;
}

typedef HWND (WINAPI *WindowFromDC_funcptr)( HDC );
typedef INT (WINAPI *GetClassNameA_funcptr)( HWND,LPSTR,INT );
//<yangxinruo><for wechat voip rotate 90> add end
/***********************************************************************
 *           StretchDIBits   (GDI32.@)
 */
                                            ...
                                            DWORD rop )
{
    ...
    if (mod)
    {
        ...
        if (pWindowFromDC)
        {
            HWND wndDst=pWindowFromDC(hdc);
            GetClassNameA_funcptr pGetClassNameA = (GetClassNameA_funcptr)GetProcAddress(mod,"GetClassNameA");
            if (pGetClassNameA)
            {
                char classnm[256];
                if(wndDst)
                {
                    pGetClassNameA(wndDst,classnm,256);
                    TRACE("YYYXXXRRR---> FIND WINDOW FOR DST className=%s\n",classnm);
                    if(strcmp(classnm,"VoipWnd") == 0)
                    {
   
                        TRACE("YYYXXXRRR---> set a DC rotate\n");
                        BITMAPINFO newInfo;
                        BYTE *newBits = GetRotatedBitmap(bmi,bits,270,RGB(255,255,255),&newInfo);
                        BITMAPINFO* resInfo=&newInfo;
                        TRACE("YYYXXXRRR---> set a DC rotate info count=%d, w=%d, h=%d ,size=%d\n",resInfo->bmiHeader.biBitCount,resInfo->bmiHeader.biWidth,resInfo->bmiHeader.biHeight,resInfo->bmiHeader.biSizeImage);

                        if ((dc = get_dc_ptr( hdc )))
                        {
                            update_dc( dc );
                            physdev = GET_DC_PHYSDEV( dc, pStretchDIBits );
                            ret = physdev->funcs->pStretchDIBits( physdev, xDst, yDst+heightDst, widthDst, -heightDst,
                                                                0,0, resInfo->bmiHeader.biWidth,resInfo->bmiHeader.biHeight, newBits, &newInfo, coloruse, rop );
                            release_dc_ptr( dc );
                        }
                        if(newBits){
                            TRACE("YYYXXXRRR---> FREE NEW BITS %p\n",newBits);
