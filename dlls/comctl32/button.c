...
#if defined __HOTPOT_PHOTOSHOP7__
    // yujun PhotoShopV7.0滤镜_视频-逐行,弹出逐行框，消除项 单选按钮点击没反应
    // 理论上窗口内存在多个组的RadioButton都存在问题
    WCHAR title_text[1024];
    if(GetWindowTextW( parent, title_text, ARRAY_SIZE(title_text))) {
        if(!lstrcmpW(title_text, L"逐行")) return;
    }
#endif
