...
/*  start add by tongzhe, 20211217 极压缩软件点击“网络”后无法返回上一层目录，修改为“网络”点击无反应 */
#ifdef __HOTPOT_SPEEDZIP__
    item.mask = LVIF_PARAM | LVIF_IMAGE;
#else	
    item.mask = LVIF_PARAM;
#endif // __HOTPOT_SPEEDZIP__
...
#ifdef __HOTPOT_SPEEDZIP__
    if (-3 == code && 4 == item.iImage)
    {
        FIXME("Double click network icon in SpeedZip App\n");
        return FALSE;
    }
#endif
/*  end add by tongzhe, 20211217 极压缩软件点击“网络”后无法返回上一层目录，修改为“网络”点击无反应 */
...
static BOOL LISTVIEW_SetItemState(LISTVIEW_INFO *infoPtr, INT nItem, const LVITEMW *item)
{
    ...
    if ((item->state & item->stateMask & LVIS_STATEIMAGEMASK) && infoPtr->himlState == NULL) {
        FIXME("Can't set state image, himlState is null\n");
        return FALSE;
    }
...
static INT LISTVIEW_SetView(LISTVIEW_INFO *infoPtr, DWORD nView)
{
  ...
  if (nView == LV_VIEW_TILE)
  {
      ...
      // use list view to replace tile view
      infoPtr->uView = LV_VIEW_LIST;
      return -1;
  }
...
static LRESULT LISTVIEW_Paint(LISTVIEW_INFO *infoPtr, HDC hdc)
{
	...
	if (!hdc) return 1;
    /*haozip listview black srceen and page residue*/
#if defined __HOTPOT_HAOZIP3__
    LISTVIEW_Refresh(infoPtr, hdc, &(infoPtr->rcList));// Erase the rect then make listview paint all listview's client
#else
 ...
 *  None
 */
static int FillBackGround(HDC hdc,RECT *old,RECT *new)
{
    RECT need_right,need_bottom;
    int flag = 0;
    /*fill the right part*/
    need_right.left=old->right;
    need_right.right=new->right;
    need_right.top=old->top;
    need_right.bottom=new->bottom;

    /*fill the bottom part*/
    need_bottom.left=old->left;
    need_bottom.right=new->right;
    need_bottom.top=old->bottom;
    need_bottom.bottom=new->bottom;

    HBRUSH hbr_white = GetStockObject(WHITE_BRUSH);
    if( (need_right.right - need_right.left)>0 && (need_right.bottom - need_right.top)>0 )
    {
      FillRect(hdc,&need_right,hbr_white);
      flag = 1;
    }
    if( (need_bottom.right - need_bottom.left)>0 && (need_bottom.bottom - need_bottom.top)>0 )
    {
      if(flag == 1 )
      {
        need_bottom.right=old->right;
      }
      FillRect(hdc,&need_bottom,hbr_white);
    }
...
#if defined __HOTPOT_HAOZIP3__
    /*haozip black screen change to white late.*/
    RECT start,end;
    start.bottom=infoPtr->rcList.bottom;
    start.top=infoPtr->rcList.top;
    start.left=infoPtr->rcList.left;
    start.right=infoPtr->rcList.right;
    /*haozip black screen change to white late.*/
#endif
...
#if defined __HOTPOT_HAOZIP3__
    /*haozip black screen change to white late.*/
    end.bottom=infoPtr->rcList.bottom;
    end.top=infoPtr->rcList.top;
    end.left=infoPtr->rcList.left;
    end.right=infoPtr->rcList.right;
    HDC hdc = GetDC(infoPtr->hwndSelf);
    FillBackGround(hdc,&start,&end);
    ReleaseDC(infoPtr->hwndSelf, hdc);
    /*haozip black screen change to white late.*/
#endif
}
...
//add by xdl 暂时处理格式工厂listview列表闪烁的问题
#if defined __HOTPOT_FORMATFACTORY__
    if(strcmp(debugstr_w(((LPLVCOLUMNW)lParam)->pszText), "L\"\\622a\\53d6\\7247\\65ad\"") == 0)
    {
        if( ((LPLVCOLUMNW)lParam)->cx > 10 )
	{
	    ((LPLVCOLUMNW)lParam)->cx -= 10;
	}
    }
#endif
//end
  ...
  /*20210618 renzy modify for CAXA_CAPP cannot create a new file directly begin */
    lParamDefault = (LPLVITEMW)lParam;
    GetModuleFileNameA(NULL, buff, sizeof(buff));
    if(strstr(buff, "CDRAFT_P.exe") && !lParamDefault->iItem)
        infoPtr->nFocusedItem = LVIR_BOUNDS;      
    return LISTVIEW_InsertItemT(infoPtr, lParamDefault, uMsg == LVM_INSERTITEMW);
    //return LISTVIEW_InsertItemT(infoPtr, (LPLVITEMW)lParam, uMsg == LVM_INSERTITEMW);
  /*20210618 renzy modify for CAXA_CAPP cannot create a new file directly end */
  ...
  case LVM_SETITEMSTATE:
    {
	/*listview反选功能问题修复*/
	if((INT)wParam != -1)//-1表示全选，需要过滤
	{
	   if((((LPLVITEMW)lParam)->state & ((LPLVITEMW)lParam)->stateMask) == LVIS_DROPHILITED)//反选选项中未选中的item
	   {
	        ((LPLVITEMW)lParam)->state |= LVIS_SELECTED;//将反选未选中的item选中
	   }
	}
	/*listview反选功能问题修复*/
