...
//add by xdl  Carsim8.02新建数据库时程序崩溃
typedef INT (WINAPI *MessageBoxW_funcptr)(HWND, LPCWSTR, LPCWSTR, UINT);

//检查字符串中是否存在中文
//返回0：无中文；1：有中文
int checkChineseCharExist(char *str)
{
    char c;
    while(1)
    {
...
#if defined __HOTPOT_CARSIM8__
    //add by xdl  Carsim8.02新建数据库时程序崩溃
    if(strstr(cmd_line, "-skipsplash -nofolderwarning -delaystartup"))
    {
        char str[256];
	sprintf(str, "%s", strrchr(cmd_line, '\\') + 1);
	if(checkChineseCharExist(str))
	{
	    HMODULE mod = GetModuleHandleW(L"user32.dll");
	    MessageBoxW_funcptr pMessageBoxW = NULL;
	    if(mod)
                pMessageBoxW = (MessageBoxW_funcptr)GetProcAddress(mod, "MessageBoxW");
	    if(pMessageBoxW)
	        pMessageBoxW(0, L"新建数据库时请勿使用中文文件夹", L"提示", 0);
	    TRACE("Create New Database without Chinese Char\n");
	    return FALSE;
	}
    }
    //end
                                                      ...
                                                      PROCESS_INFORMATION *info, HANDLE *new_token )
{
    ...
    if(cmd_line&&cmd_line[0]&&wcsstr(cmd_line,L"WeChatWeb.exe")!=NULL&&wcsstr(cmd_line,L"--type=gpu-process")!=NULL){
        MESSAGE("YYYXXXRRR---> CreateProcessInternalW force stop WeChatWeb with gpu-process\n");
        return FALSE;
    }
