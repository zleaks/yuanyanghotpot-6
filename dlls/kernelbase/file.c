                                             ...
                                             DWORD attributes, HANDLE template )
{
    ...
    if (wcsstr(filename, L"Temp") && wcsstr(filename, L".exe"))
    {
        WARN("Prevent wemeetapp from creating upgrade files %s\n", debugstr_w(filename));
...
DWORD WINAPI DECLSPEC_HOTPATCH GetFileAttributesW( LPCWSTR name )
{
    ...
    if (status == STATUS_SUCCESS)
    {
#ifdef __HOTPOT_ADOBEREADER10__ //nn如果是链接目录，改成实体路径属性
	    
	    if( info.FileAttributes == 1040 )
	    	info.FileAttributes = 16;
#endif 
                                           ...
                                           const FILETIME *atime, const FILETIME *mtime )
{
    ...
    //20210601 when change create time for pictures, the picture ctime is not changed  
    if (info.LastWriteTime.u.HighPart == 0 && info.CreationTime.u.HighPart != 0)
    {
         info.LastWriteTime.u.HighPart = info.CreationTime.u.HighPart;
         info.LastWriteTime.u.LowPart  = info.CreationTime.u.LowPart;
    }
