...
{
    //add by xdl
    static WCHAR tmp_server_name[1024] = {'\0'};
    static int tmp_success_flag = 0;
    //end
    ...
    //add by xdl
    if (lstrlenW(tmp_server_name) == 0)
    {
        lstrcpynW(tmp_server_name, server->name, 1024);
        TRACE("tmp_server_name:%s\n", debugstr_w(tmp_server_name));
    }
    else
    {
        if (lstrcmpW(tmp_server_name, server->name) == 0 && tmp_success_flag == 1)
        {
            tmp_success_flag = 0;
	...
	DWORD dwContentLength, BOOL bEndRequest)
{
	...
	//add by xdl
        NETCON_set_timeout( request->netconn, TRUE, tmp_timeout);
	//end
