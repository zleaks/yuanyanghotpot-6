...
RPC_STATUS WINAPI I_RpcBindingInqLocalClientPID(RPC_BINDING_HANDLE ClientBinding, ULONG *ClientPID)
{
    ...
    //<yangxinruo><20210706><temporary solution for zoomSDK(vilin etc.) can not share desktop> modify begin
    // return RPC_S_INVALID_BINDING;
    char processnm[256];
    GetModuleFileNameA(NULL,processnm,256);
    if(EndWith(processnm,"zcscpthost.exe")){
      ...
      endC=cmdline;
      BOOL pidReady=FALSE;
      while(endC-cmdline<strlen(cmdline)){
        if(endC[0]==' '){
          if(pidReady){
            break;
          }
          else if(strncmp(startC,"-pid",4)==0){
            pidReady=TRUE;
          }
          endC++;
          startC=endC;
      ...
      CHAR pidStr[endC-startC+1];
      memcpy(pidStr,startC,endC-startC);
      pidStr[endC-startC]='\0';
      int pid=atoi(pidStr);
      TRACE("YYYXXXRRR--->TODO:fixit Get pid from cmdline for now pid=[%d]\n",pid);
      *ClientPID=pid;
      return RPC_S_OK;
    }else{
      return RPC_S_INVALID_BINDING;
    }
    //<yangxinruo><20210706><temporary solution for zoomSDK(vilin etc.) can not share desktop> modify end
}
