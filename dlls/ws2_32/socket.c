...
int __hotpot_ipv6=-1;// nan nan 是否支持IPv6选项，从环境变量读取

// nannan 
// 1 支持  0 不支持

static int getHotPotIPv6()
{
    char *ipv6 = getenv("WINEIPV6");

    if( NULL == ipv6 )
	return 1;

    if( 0 == strcmp(ipv6,"enable") )
	return 1;

    if( 0 == strcmp(ipv6,"disable") )
	return 0;
    ...
    if (fd != -1)
    {
        ...
        if (ret == WSAEINPROGRESS)
        {
            ...
            if (!ret)
            {
                ...
                {
                    //add by xdl
                    do_block(fd, POLLIN | POLLOUT, tmp_timeout);
                    //end
    ...
    case WS_SOL_SOCKET:
        ...
        case WS_SO_SNDTIMEO:
            ...
            if (optval && optlen == sizeof(UINT32)) {
                ...
                optval = (char*)&tval;
                //<yangxinruo><wechat 2.9.5 can not upload big file> begin
                if(optname==WS_SO_RCVTIMEO){
                    TRACE("YYYXXXRRR---> Ignoring WS_SO_RCVTIMEO, !!! tv_sec=%d,tv_usec=%d\n",tval.tv_sec,tval.tv_usec);
                    return 0;
                }
                //<yangxinruo><wechat 2.9.5 can not upload big file> end
...
int WINAPI WS_getaddrinfo(LPCSTR nodename, LPCSTR servname, const struct WS_addrinfo *hints, struct WS_addrinfo **res)
{
    ...
    *res = NULL;
	...
	if( __hotpot_ipv6 == -1 )
	{
	     __hotpot_ipv6 = getHotPotIPv6();
	}

        *xai = NULL;
        while (xuai) {
            //<yangxinruo><20210208><for QQ do NOT return ipv6 addr to QQ.exe> add begin
            if( (xuai->ai_family == AF_INET6 && EndWith(processnm,"QQ.exe")) 
		|| (xuai->ai_family == AF_INET6 && EndWith(processnm,"TIM.exe")) 
		|| (xuai->ai_family == AF_INET6 && __hotpot_ipv6 == 0 ) )
	    {
                TRACE("YYYXXXRRR---> do NOT tell QQ we got a ipv6 addr!!\n");
                goto skipaddr;
            }
            //<yangxinruo><20210208><for QQ do NOT return ipv6 addr to QQ.exe> add end
...
//检查网络是否连通
int WINAPI check_network_status(void)
{
    char cmd[2048] = "";
    char retval[2048] = "";
    sprintf(cmd, "/bin/ping 114.114.114.114 -c 1 -w 1");
    cmdSystem(cmd, retval);
    TRACE("retval:%s\n", retval);
    if(strstr(retval, "1 packets transmitted, 1 received"))
    {
        TRACE("network normal\n");
        return TRUE;
    }
    TRACE("network unnormal\n");
    return FALSE;
}
//end
#endif
...
#if defined __HOTPOT_OFFICE201X__
    //add by xdl  检查网络是否连通，解决office2019在无网络环境下dns造成的卡顿问题
    if(!check_network_status())
    {
        TRACE("network status unnormal, donot getaddrinfo\n");
        return -1;
    }
    //end
#endif
