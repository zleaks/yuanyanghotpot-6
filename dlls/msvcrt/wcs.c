...
wchar_t* CDECL wcsstr(const wchar_t *str, const wchar_t *sub)
{
    ...
    if (strcmp(filename, "C:\\Program Files\\Microsoft Office\\Office12\\WINPROJ.EXE") == 0)
    {
        regex_t regex;
        const char* pattern  = "(^([0-9]{4})\u5e74([0-9]|1[0-2])\u6708([0-9]|1[0-9]|2[0-9]|3[01])\u65e5$)"
                               "|(^([0-9]{4})\u5e74([0-9]|1[0-2])\u6708([0-9]|1[0-9]|2[0-9]|3[01])\u65e5\u661f\u671f([\u4e00\u4e8c\u4e09\u56db\u4e94\u516d\u65e5])$)"
                               "|(^([0-9]|1[0-2])\u6708([0-9]|1[0-9]|2[0-9]|3[01])\u65e5$)"
                               "|(^([0-9]|1[0-2])\u6708([0-9]|1[0-9]|2[0-9]|3[01])\u65e5\u661f\u671f([\u4e00\u4e8c\u4e09\u56db\u4e94\u516d\u65e5])$)";
        regcomp(&regex, pattern, REG_EXTENDED | REG_NOSUB);
        const wchar_t* str1 = str;
        size_t size = WideCharToMultiByte(CP_UTF8, 0, str1,  -1, NULL, 0, NULL, NULL);
        char* tmp  = (char *)malloc(size);
        WideCharToMultiByte(CP_UTF8, 0, str1, -1, tmp, size, NULL, NULL);
        if (tmp && regexec(&regex, tmp, 0, NULL, 0) == 0)
        {
            wchar_t* ptr = str;
            ptr--;
            *ptr = L"\0";
