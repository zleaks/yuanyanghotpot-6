...
{
    struct strmbase_pin *pin = impl_from_IPin(iface);
    //<yangxinruo><20210107><for wechat qcap cannot open device twice,cut off pin ref to filter,because wechat may not release pins> modify begin
    // return IBaseFilter_AddRef(&pin->filter->IBaseFilter_iface);
    ULONG refCount = InterlockedIncrement(&pin->refCount);

    TRACE("(%p)->() AddRef from %d\n", iface, refCount - 1);

    return refCount;
    //<yangxinruo><20210107><for wechat qcap cannot open device twice,cut off pin ref to filter,because wechat may not release pins> modify end
}
...
static ULONG WINAPI pin_Release(IPin *iface)
{
    ...
    ULONG refCount = InterlockedDecrement(&pin->refCount);
        ...
        const WCHAR *name, const struct strmbase_source_ops *func_table)
{
    ...
    //<yangxinruo><20210107><for wechat qcap cannot open device twice,cut off pin ref to filter,because wechat may not release pins> add begin
    pin->pin.refCount = 1;
    //<yangxinruo><20210107><for wechat qcap cannot open device twice,cut off pin ref to filter,because wechat may not release pins> add end
