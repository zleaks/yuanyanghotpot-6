                             ...
                             const xmlstr_t *version, const xmlstr_t *lang, const void *data, DWORD len )
{
    ...
    if(!lstrcmpW( path,L"c:\\windows\\winsxs\\manifests\\x86_microsoft.windows.common-controls_6595b64144ccf1df_6.0.2600.2982_none_deadbeef.manifest"))
    {
	return 1;
    }
#endif
...
#if __HOTPOT_E32010__
    //start, chg by zhuyi, 20211009, 解决e3使用oleaut32时无法打开office_excel的问题。
    if (strstr(debugstr_w(file), "oleaut32.dll") != 0)
    {
        //TRACE("----> dest[%s] file[%s]\n", debugstr_w(dest), debugstr_w(file));
        return 0;
    }
    //end, chg by zhuyi, 20211009, 解决e3使用oleaut32时无法打开office_excel的问题。
#endif
